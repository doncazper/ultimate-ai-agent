...
def test_context_requirement_larger_than_model_window_is_rejected() -> None:
...
def test_request_tokens_must_fit_profile_context_window() -> None:
    request = route_request(
...
def test_exhausted_available_history_budget_rejects_otherwise_capable_profile() -> None:
    request = route_request(
...
def test_input_tokens_larger_than_available_history_budget_rejects_profile() -> None:
    request = route_request(
        profiles=[local_profile()],
        context_budget=ContextBudget(
            model_context_limit=4096,
...
def test_input_tokens_within_available_history_budget_can_select_profile() -> None:
    request = route_request(
        profiles=[local_profile()],
        context_budget=ContextBudget(
            model_context_limit=4096,
