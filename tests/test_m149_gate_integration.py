...
def test_m149_foundation_gate_criteria_registered() -> None:
    ids = {criterion.criterion_id for criterion in default_foundation_gate_criteria()}

    assert "m149_alpha_release_candidate_freeze_contracts" in ids
    assert "m149_alpha_release_candidate_freeze_static_safety" in ids
    assert "m149_alpha_release_candidate_freeze_route_boundary" in ids
    assert "m149_roadmap_currentness" in ids


def test_m149_foundation_gate_evaluator_accepts_current_repo() -> None:
...
def test_m149_route_boundary_rejects_release_candidate_routes() -> None:
    paths = {
        "/api/manifest": {},
    ...
    failures = gate_evaluators.m149_openapi_route_failures(
        paths,
    ...
    assert not gate_evaluators.m149_openapi_route_failures(app.openapi().get("paths", {}))


def test_m149_static_safety_detects_release_candidate_fragments(
    tmp_path: Path,
) -> None:
    ...
    criterion = next(
        ...
        if item.criterion_id == "m149_alpha_release_candidate_freeze_static_safety"
    )
    result = (
        gate_evaluators.FoundationGateEvaluator(tmp_path)
        .check_m149_alpha_release_candidate_freeze_static_safety(criterion)
    )
