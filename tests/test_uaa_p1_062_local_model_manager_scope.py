...
def test_uaa_p1_062_scope_doc_pins_docs_only_non_goals() -> None:
    ...
    assert "Candidate Implementation Roadmap" in text
    assert "Read-only inventory" in text
    assert "uaa local-model status" in text
    assert "uaa local-model switch --to <model-ref> --dry-run" in text
    assert "Implement UAA-P1-064 Local Model Inventory Read-Only Backend + CLI" in text
