...
IDEMPOTENCY_HEADERS = {"X-UAA-Idempotency-Key": "idempotency:kernel-api-test"}
...
def test_kernel_task_run_endpoint_is_dry_run_only_for_test_prefixed_approval(tmp_path: Path) -> None:
    response = client.post(
        ...
        json=request(tmp_path).model_dump(mode="json"),
    )
...
def test_kernel_task_run_endpoint_does_not_mutate_with_arbitrary_approval(tmp_path: Path) -> None:
    ...
    response = client.post("/kernel/tasks/run", headers=IDEMPOTENCY_HEADERS, json=payload)
