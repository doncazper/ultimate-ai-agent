...
WRAPPER = ROOT / "scripts" / "dev" / "run_uaa_goatcitadel_runtime_parity_prompt_pack.sh"
...
def test_verifier_accepts_pack() -> None:
    ...
    assert data["bundle_id"] == "uaa-goatcitadel-runtime-parity-001"
...
def test_wrapper_dry_run_emits_combined_prompt(tmp_path: Path) -> None:
    ...
    assert "UAA GoatCitadel Runtime Parity Prompt Pack Combined Run" in text
