...
def test_m163_builds_structured_loopback_argv_without_shell_string():
...
def test_m163_supervisor_uses_process_factory_and_redacts_result():
...
def test_m163_supervisor_stop_terminates_managed_process():
    factory = FakeM163ProcessFactory()
    supervisor = M163LlamaCppSupervisor(process_factory=factory)
...
def test_m163_preset_rejects_unsafe_shape(update, reason):
    with pytest.raises(ValueError, match=reason):
        validate_m163_llama_cpp_server_preset(_preset(**update))


def test_m163_result_validation_rejects_unsafe_mutation():
