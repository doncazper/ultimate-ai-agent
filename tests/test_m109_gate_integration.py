...
def test_m109_foundation_gate_criteria_registered() -> None:
    ids = {criterion.criterion_id for criterion in default_foundation_gate_criteria()}

    assert "m109_mobile_sensor_audit_ledger_contracts" in ids
    assert "m109_mobile_sensor_audit_ledger_static_safety" in ids
    assert "m109_mobile_sensor_audit_ledger_route_boundary" in ids
    assert "m109_roadmap_currentness" in ids


def test_m109_foundation_gate_evaluator_accepts_current_repo() -> None:
...
def test_m109_route_boundary_rejects_sensor_audit_runtime_routes() -> None:
    failures = gate_evaluators.m109_openapi_route_failures(
        {
            "/api/manifest": {},
            "/mobile/sensor-audit": {},
    ...
    assert not gate_evaluators.m109_openapi_route_failures(app.openapi().get("paths", {}))


def test_m109_static_safety_detects_sensor_runtime_fragments(
    tmp_path: Path,
) -> None:
    ...
    criterion = next(
        ...
        if item.criterion_id == "m109_mobile_sensor_audit_ledger_static_safety"
    )
    result = (
        gate_evaluators.FoundationGateEvaluator(tmp_path)
        .check_m109_mobile_sensor_audit_ledger_static_safety(criterion)
    )
