...
def test_m17_evidence_file_memory_viewer_criterion_exists_and_passes() -> None:
    ...
    assert (
        "summary-only refs"
        in criteria_by_id["m17_evidence_file_memory_viewer_safe"].pass_condition
    )
    assert (
        "memory as recall and not authority"
        in criteria_by_id["m17_evidence_file_memory_viewer_safe"].pass_condition
    )
    assert (
        "raw file"
    ...
    report = FoundationGateEvaluator().evaluate(
        [criteria_by_id["m17_evidence_file_memory_viewer_safe"]]
    )
...
def test_m17_openapi_route_guard_rejects_backend_knowledge_viewer_expansion() -> None:
    ...
    assert EXPECTED_M17_OPENAPI_PATH_COUNT == 80
