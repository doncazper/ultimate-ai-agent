...
def test_capability_surface_generated_overlay_is_current() -> None:
    ...
    assert overlay["source_truth_counts"]["covered_release_route_count"] == 46
    assert overlay["source_truth_counts"]["covered_visible_action_count"] == 49
    finance = next(item for item in overlay["capabilities"] if item["capability_id"] == "finance_synthetic_workspace")
    assert finance["source_truth_status"] == "current"
    assert len(finance["api_routes"]) == 4
    assert finance["ui_routes"][0]["release_status"] == "partial"
    today = next(
        item
...
def test_capability_surface_read_model_is_safe_bounded_and_source_backed() -> None:
    ...
    assert payload["summary"]["capability_count"] == 34
...
def test_capability_surface_api_route_and_cli_expose_same_safe_model() -> None:
    ...
    assert body["data"]["summary"]["covered_release_route_count"] == 46
    ...
    assert cli_payload["summary"]["covered_visible_action_count"] == 49
...
def test_capability_surface_manifest_covers_current_visible_routes_and_actions() -> (
    ...
    assert messenger["cli_paths"] == ["scripts/dev/uaa_communications.py"]
