...
def test_m123_foundation_gate_criteria_registered() -> None:
    ids = {criterion.criterion_id for criterion in default_foundation_gate_criteria()}

    assert "m123_contacts_connector_contract_refresh_contracts" in ids
    assert "m123_contacts_connector_contract_refresh_static_safety" in ids
    assert "m123_contacts_connector_contract_refresh_route_boundary" in ids
    assert "m123_roadmap_currentness" in ids


def test_m123_foundation_gate_evaluator_accepts_current_repo() -> None:
...
def test_m123_route_boundary_rejects_contacts_connector_runtime_routes() -> None:
    failures = gate_evaluators.m123_openapi_route_failures(
        {
            "/api/manifest": {},
    ...
    assert not gate_evaluators.m123_openapi_route_failures(app.openapi().get("paths", {}))


def test_m123_static_safety_detects_contacts_runtime_fragments(tmp_path: Path) -> None:
    ...
    criterion = next(
        ...
        if item.criterion_id == "m123_contacts_connector_contract_refresh_static_safety"
    )
    result = (
        gate_evaluators.FoundationGateEvaluator(tmp_path)
        .check_m123_contacts_connector_contract_refresh_static_safety(criterion)
    )
