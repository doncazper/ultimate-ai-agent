...
def test_m25_foundation_gate_criteria_are_registered() -> None:
    ...
    truth_criterion = next(
        ...
        if criterion.criterion_id == "m25_truth_source_router_contracts_valid"
    )
    ...
    v0292_criterion = next(
        criterion
        for criterion in criteria
        if criterion.criterion_id == "v0292_local_dev_api_authority_and_preview_safe"
    )
...
def test_m25_openapi_route_guard_rejects_truth_execution_routes() -> None:
    ...
    assert EXPECTED_M25_OPENAPI_PATH_COUNT == 80
...
def test_m25_foundation_gate_evaluator_passes_current_contracts() -> None:
    ...
    criteria = [
        ...
        if criterion.criterion_id.startswith("m25_")
        or criterion.criterion_id.startswith("v0292_")
    ]
