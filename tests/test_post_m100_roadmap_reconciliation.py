...
def test_post_m100_documentation_integrity_guard_rejects_missing_row_status(
    ...
    roadmap.write_text(
        roadmap.read_text(encoding="utf-8").replace(
            ...
            "| Checkpoint M135 | pre-alpha checkpoint | M135 | Autonomous Recovery Planner | Deferred |",
        ),
        encoding="utf-8",
    )
    ...
    assert any("m135" in failure.lower() for failure in failures)
...
def test_post_m100_foundation_gate_rejects_missing_row_status(tmp_path: Path) -> None:
    ...
    assert any("m135" in failure.lower() for failure in result.failures)
