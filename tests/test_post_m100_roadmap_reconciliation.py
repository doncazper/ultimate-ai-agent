...
def test_post_m100_documentation_integrity_guard_rejects_missing_row_status(
    ...
    roadmap.write_text(
        roadmap.read_text(encoding="utf-8").replace(
            ...
            "| Checkpoint M134 | pre-alpha checkpoint | M134 | Human Checkpoint Scheduling | Deferred |",
        ),
        encoding="utf-8",
    )
    ...
    assert any("m134" in failure.lower() for failure in failures)
...
def test_post_m100_foundation_gate_rejects_missing_row_status(tmp_path: Path) -> None:
    ...
    assert any("m134" in failure.lower() for failure in result.failures)
