...
def test_append_first_run_storage_writes_run_and_receipt_atomically(tmp_path: Path):
    ...
    assert validate_receipt_summary_hash_ref(
        receipt_entry.receipt_summary,
    ...
    replay = reloaded.validate_receipt_replay(
        "run:p1-025",
        "receipt:p1-025-summary",
    )
...
def test_append_first_run_storage_validates_receipt_replay(tmp_path: Path):
    ...
    reloaded = AppendFirstRunStorage(tmp_path / "runs.jsonl")
    replay_entry = reloaded.validate_receipt_replay(
        "run:p1-029",
        "receipt:p1-029-replay",
    )
...
def test_append_first_run_storage_receipt_hash_detects_summary_mismatch():
    ...
    with pytest.raises(
        DurableRunStorageCorruptionError,
        match="DURABLE_RUN_STORAGE_RECEIPT_HASH_MISMATCH",
    ):
...
def test_append_first_run_storage_preserves_prior_file_when_replace_fails(
    ...
    with pytest.raises(
        DurableRunStorageWriteError,
        match="DURABLE_RUN_STORAGE_ATOMIC_WRITE_FAILED",
    ):
        store.append_receipt_summary(
            run_id="run:p1-025",
            ...
            receipt_summary={
                "status": "blocked",
                "safe_summary": "Atomic replace failed before commit.",
            },
        )
...
def test_append_first_run_storage_rejects_corrupted_hash_chain(tmp_path: Path):
    ...
    with pytest.raises(
        DurableRunStorageCorruptionError,
        match="DURABLE_RUN_STORAGE_ENTRY_HASH_MISMATCH",
    ):
...
def test_append_first_run_storage_blocks_duplicate_idempotency_keys(
    ...
    with pytest.raises(
        DurableRunStorageDuplicateError,
        match="DURABLE_RUN_STORAGE_IDEMPOTENCY_REPLAY_DENIED",
    ):
        store.append_receipt_summary(
            run_id="run:p1-025",
            receipt_ref="receipt:p1-025-duplicate",
            ...
            receipt_summary={
                "status": "blocked",
                "safe_summary": "Duplicate idempotency key denied.",
            },
        )
...
def test_append_first_run_storage_recovers_restart_visibility(tmp_path: Path):
    ...
    recovered = AppendFirstRunStorage(tmp_path / "runs.jsonl").latest_run_record(
        "run:p1-025"
    )
