...
def test_action_decision_storage_records_receipts_replay_and_conflict(
    ...
    request = FounderLoopActionDecisionRequest(
        expected_revision_ref=str(
            repo.action_revision("setup-assistant-hardening")["revision_ref"]
        ),
    ...
    assert (
        repo.latest_action_receipt("setup-assistant-hardening")["receipt_ref"]
        == (receipt["receipt_ref"])
    )
...
def test_approval_decision_records_backend_owned_exact_local_approval(
    ...
    assert backend_owned["approval_ref"].startswith("approval-ref:founder-loop-action:")
...
def test_action_decision_api_requires_idempotency_and_returns_receipt(
    ...
    body = {
        "expected_revision_ref": str(
            next(
                item
                for item in client.get("/control-center/actions/inbox").json()["data"][
                    "items"
                ]
    ...
    assert (
        conflict.json()["detail"]["code"] == "FOUNDER_LOOP_ACTION_IDEMPOTENCY_CONFLICT"
    )
    ...
    response = client.post(
        ...
        json={
            "expected_revision_ref": str(
                next(
                    item
                    for item in client.get("/control-center/actions/inbox").json()[
                        "data"
                    ]["items"]
                    if item["item_ref"] == "founder-action:local-task-create-scorecard"
                )["action_revision_ref"]
...
def test_fcc_v1_002_verifier_flags_release_overclaim() -> None:
    ...
    actions = next(
        route for route in release_surface["routes"] if route["path"] == "/actions"
    )
    actions["status"] = "ship"
    ...
    actions["proof_lanes"] = [
        proof
        for proof in actions["proof_lanes"]
        if proof != verifier.FOUNDER_LOOP_V1_PROOF_REF
    ]
    ...
    assert any(
        "/actions ship status requires FCC-V1-007 proof lane" in failure
