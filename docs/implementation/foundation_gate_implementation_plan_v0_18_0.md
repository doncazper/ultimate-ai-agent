# Foundation Gate Implementation Plan v0.18.0

Status: Active for M14 Web Control Center Local Backend Connection Stabilization.

v0.18.0 does not add runtime capability or backend API authority. It adds Foundation Gate coverage for local-only Web Control Center backend connection behavior.

M14 criteria:

- `VERSION.md`, `pyproject.toml`, and `src/ultimate_ai_agent/__init__.py` agree on `0.18.0`.
- README points to `README_IMPORT_v0_18_0.md` and `ultimate_ai_agent_master_plan_v0_18_0.md`.
- active import, master plan, release notes, and Foundation Gate implementation plan exist for v0.18.0.
...
- secret-like API base URL strings are rejected without exposing secret-like values.
- backend online, degraded, offline-safe, and mock fallback states are visible.
...
v0.18.0 does not change the Skill Package Security Rule. The web shell may display plugin governance summaries only; it cannot install, enable, configure, or execute plugins or skills.
