...
- Runtime session continuity, MCP catalog filtering, worktree-per-agent, staged
  ...
  interrupt redirect, voice/media posture, messaging gateway posture, remote
  execution posture, plugin metadata posture, and skill marketplace posture consume that
  ...
  requirement, background jobs expose the delegated-mission
  ...
  Center authority minting remain blocked. Logging profile exposes the Read-only
  `workspace/read` posture decision while verbose logging, material persistence,
  telemetry export, background streams, and Control Center authority minting
  ...
  Runtime checkpoint rollback exposes the Read-only `workspace/read`
  checkpoint, rollback-plan, receipt, approval-scope, and idempotency posture
  inspection decision while broad snapshots, rollback execution, Git mutation,
  raw content/path persistence, shell/browser execution, and production
