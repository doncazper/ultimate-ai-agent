...
contract-only review-only messages connector contract review records. M125-M150
are planned/provisional only in
`docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md`.
Future M125-M149 conveyor milestones use checkpoint labels instead of product
...
M34-M124 implemented/released and rely on documentation-integrity checks to
guard against superseded-roadmap drift, stale current-baseline labels, route
drift, authority-boundary drift, alpha/beta versioning drift, and M125-M150
future-status drift.
