...
| Tool-aware cognition and chat quality queue | `docs/strategy/UAA_TOOL_AWARE_COGNITION_AND_CHAT_QUALITY_PLAN.md`, `docs/roadmap/UAA_TOOL_AWARE_COGNITION_QUEUE_INSERTION.md`, `docs/roadmap/UAA_REMAINING_QUEUE_MANIFEST.json`, `scripts/verify_tool_aware_cognition_plan.py`, `tests/test_tool_aware_cognition_plan.py` |
...
| Local developer work coordinator | `docs/developer/LOCAL_DEVELOPER_WORK_COORDINATOR.md`, `scripts/dev/uaa_developer_orchestrator/`, `scripts/dev/uaa_developer_queue.py`, `tests/test_developer_orchestrator.py` (separate local developer coordination plane: canonical queue catalog, explicit registered-node claims and heartbeats, isolated-worktree metadata, terminal scope-packet archive gate, verifier/merge gates, and fixed read-only Git hygiene scouting; no agent launch, Git mutation, remote dispatch, or UAA product-runtime authority) |
