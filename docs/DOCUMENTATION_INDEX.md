...
v0.37.2 adds local developer launcher tooling for prototype testing. The
launcher is localhost-only and developer-only, starts the existing FastAPI
backend plus CCC Web Control Center, stores ignored PID/log state under
`.uaa/dev/`, and adds no production installer, backend route, execution
authority, dependency, M34 work, or production authority.

## Release Notes Index

Current release notes: `docs/release_notes/v0_37_2.md`
