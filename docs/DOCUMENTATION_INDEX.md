...
| Medical knowledge and local Knowledge Dump | `docs/knowledge/MEDICAL_KNOWLEDGE_BASELINE.md`, `docs/knowledge/KNOWLEDGE_DUMP.md`, `src/ultimate_ai_agent/core/medical_knowledge/`, `src/ultimate_ai_agent/core/knowledge_dump/`, `scripts/dev/uaa_knowledge.py`, `tests/test_medical_knowledge_catalog.py`, `tests/test_knowledge_dump.py` (15 registered medical source records plus exact-approved local TXT/Markdown/HTML/EPUB ingestion, source kind/category/collection/tag inventory and recategorization, filtered FTS5 retrieval, and explicit cited context preparation; PDF parsing, network ingestion, automatic Chat injection, embeddings/vector search, model training, diagnosis, prescribing, and clinical authority remain blocked) |
