...
| ECO-010 deterministic proposal intelligence | `docs/architecture/ECO_010_PROPOSAL_INTELLIGENCE.md`, `src/ultimate_ai_agent/core/ecosystem/proposals.py`, `scripts/inspect_eco_010_proposals.py`, `scripts/verify_queue_v2_q27_proposal_intelligence.py`, `tests/test_queue_v2_q27_proposal_intelligence.py` (bounded cited proposal-only event, task, person, commitment, and meeting candidates with source-revision, privacy, uncertainty, and canonical-owner posture; no source read, model authority, ChangeSet, target record, or direct commit) |
