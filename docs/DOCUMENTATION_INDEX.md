...
and complete usage/cost receipts. The contracts are backed by
...
by `tests/test_exact_approved_provider_fallback.py`. It adds no secret resolution
...
billing authority, no provider output authority, no unknown paid-cost bypass, no
vault runtime authority, no invocation authority from vault presence, and no
broad callable runtime authority.
