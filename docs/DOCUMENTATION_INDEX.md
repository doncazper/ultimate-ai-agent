...
| ECO-001 shared local-data foundation | `docs/architecture/ECO_001_SHARED_LOCAL_DATA_FOUNDATION.md`, `src/ultimate_ai_agent/core/ecosystem/local_data.py`, `scripts/verify_eco_001_local_data.py`, `tests/test_eco_001_local_data.py` (approval-bound encrypted generic repository, encrypted exact replay, tombstoned record refs, atomic local unit of work, blind-index search/rebuild, deep size-bounded backup/restore, integrity, archive/delete, and bounded read-only migration preview; no production key backend, app cutover, route, connector, cloud sync, or production authority) |
