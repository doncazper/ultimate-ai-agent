...
readiness, budget, redaction, and authority posture refs only. They do not
write files, apply patches, read or persist raw file content, run
shell/subprocess commands, mutate Git state, call providers or models, automate
browsers, write connectors, launch background agents, persist raw paths or raw
content, or grant production authority.
