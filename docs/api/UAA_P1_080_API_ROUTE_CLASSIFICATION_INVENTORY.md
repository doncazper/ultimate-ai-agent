...
| `local_readonly` | 31 |
| `local_sensitive` | 223 |
| `mutating_requires_authority` | 137 |

The current OpenAPI path count is `392` and `/api/manifest` currently reports
`394` route operations after later scoped FCC-V1-003 through
...
backup, and exact restore routes, plus the founder-private News and Signals
adoption read, preview, approval, and commit routes, plus the synthetic-only
Finance workspace read, preview, refresh, and exact-confirmed commit routes,
plus the protected
...
It also includes the MSG-MX-010 content-free intelligence posture and proposal
routes plus six exact request-scoped local policy/context/proposal operations;
provider/model invocation and attachment analysis remain blocked and absent.
