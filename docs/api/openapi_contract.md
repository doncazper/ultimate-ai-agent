...
v0.29.0 preserves the FastAPI OpenAPI boundary and documents the current route contract without adding public real execution routes. M25 Truth Source Router + Evidence Claim Checker adds no backend API route; OpenAPI path count remains `74`.
...
.venv/bin/python scripts/export_openapi.py --output /tmp/openapi_v0_29_0.json
```
