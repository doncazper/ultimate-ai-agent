...
v0.28.2 preserves the FastAPI OpenAPI boundary and documents the current route contract without adding public real execution routes. M24 Roadmap Row Cleanup adds no backend API route; OpenAPI path count remains `74`.
...
.venv/bin/python scripts/export_openapi.py --output /tmp/openapi_v0_28_2.json
```
