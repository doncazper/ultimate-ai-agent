...
v0.28.1 preserves the FastAPI OpenAPI boundary and documents the current route contract without adding public real execution routes. M24 Contract Repair + Memory Safety Hardening adds no backend API route; OpenAPI path count remains `74`.
...
.venv/bin/python scripts/export_openapi.py --output /tmp/openapi_v0_28_1.json
```
