...
|---|---|
| OpenAPI path count | 127 paths. |
| `/api/manifest` route count | 127 routes. |
...
| Control Center route-status manifest | 105 backend route refs checked against `/api/manifest`; 0 missing and 0 path/method/operation/side-effect mismatches. |
| Route inventory doc | Current count matches 127; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `docs/api/route_inventory.md` vs `/api/manifest` | Count and high-level groups agree. The doc intentionally summarizes broad groups rather than listing all 127 rows. | This UAA-P1-021 map is the exhaustive grouping companion. |
...
| `control-center` | 28 | `control-center` | `control_center_service` | local status or future auth per route | `local_dev_workspace_only:19`, `validation_only:9` | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
...
Columns: method, path, operation ID, side-effect class, validation-only,
future-auth posture, blocked-from-production.

### `adapter-boundary`

| Method | Path | Operation ID | Side effect | Validation-only | Auth posture | Blocked from production |
|---|---|---|---|---|---|---|
...
Future edits must keep path, method, operation ID, tag/group, request/response
schema, side-effect class, auth posture, capability declaration, OpenAPI path
count, and `/api/manifest` route inventory stable unless a separately accepted
route-contract milestone updates tests and docs at the same time.
