...
|---|---|
| OpenAPI path count | 174 paths. |
| `/api/manifest` route count | 174 routes. |
...
| Route inventory doc | Current count matches 174; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `control-center` | 73 | `control-center` | `control_center_service` | local status or future auth per route | `governed_network_read_only`:2, `local_dev_workspace_only`:56, `validation_only`:15 | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
