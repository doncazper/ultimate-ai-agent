...
|---|---|
| OpenAPI path count | 172 paths. |
| `/api/manifest` route count | 172 routes. |
...
| Route inventory doc | Current count matches 172; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `control-center` | 71 | `control-center` | `control_center_service` | local status or future auth per route | `governed_network_read_only`:2, `local_dev_workspace_only`:54, `validation_only`:15 | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
