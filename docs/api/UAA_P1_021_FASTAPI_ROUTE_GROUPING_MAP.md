...
| `control-center` | 77 | `control-center` | `control_center_service` | local status or future auth per route | `governed_network_read_only`:2, `local_dev_workspace_only`:59, `validation_only`:16 | medium | stable/generated from path; unique | partial_backend_not_product_ready |
...
| `governed-runtime` | 10 | `governed-runtime` | `runtime_pilot_service` | future auth required and exact approval before broader execution | `local_dev_workspace_only`:9, `validation_only`:1 | high | stable/generated from path; unique | partial_backend_not_product_ready |
