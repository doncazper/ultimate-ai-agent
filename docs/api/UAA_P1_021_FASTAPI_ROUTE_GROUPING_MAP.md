...
|---|---|
| OpenAPI path count | 143 paths. |
| `/api/manifest` route count | 143 routes. |
...
| Route inventory doc | Current count matches 143; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `control-center` | 44 | `control-center` | `control_center_service` | local status or future auth per route | `local_dev_workspace_only`:33, `validation_only`:11 | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
