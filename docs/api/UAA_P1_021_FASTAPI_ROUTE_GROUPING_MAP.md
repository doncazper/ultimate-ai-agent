...
|---|---|
| OpenAPI path count | 121 paths. |
| `/api/manifest` route count | 121 routes. |
...
| Control Center route-status manifest | 95 backend route refs checked against `/api/manifest`; 0 missing and 0 path/method/operation/side-effect mismatches. |
| Route inventory doc | Current count matches 121; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `docs/api/route_inventory.md` vs `/api/manifest` | Count and high-level groups agree. The doc intentionally summarizes broad groups rather than listing all 121 rows. | This UAA-P1-021 map is the exhaustive grouping companion. |
...
| `control-center` | 22 | `control-center` | `control_center_service` | local status or future auth per route | `local_dev_workspace_only:13`, `validation_only:9` | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
