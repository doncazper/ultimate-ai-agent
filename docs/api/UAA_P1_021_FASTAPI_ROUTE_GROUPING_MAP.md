...
Current OpenAPI path count: 167
...
|---|---|
| OpenAPI path count | 167 paths. |
| `/api/manifest` route count | 167 routes. |
...
| Route inventory doc | Current count matches 167; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `control-center` | 66 | `control-center` | `control_center_service` | local status or future auth per route | `governed_network_read_only`:1, `local_dev_workspace_only`:47, `validation_only`:18 | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
