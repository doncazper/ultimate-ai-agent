...
|---|---|
| OpenAPI path count | 171 paths. |
| `/api/manifest` route count | 171 routes. |
...
| Route inventory doc | Current count matches 171; inventory is summarized by group and remains subordinate to `/api/manifest`. |
...
| `control-center` | 70 | `control-center` | `control_center_service` | local status or future auth per route | `governed_network_read_only`:2, `local_dev_workspace_only`:53, `validation_only`:15 | medium | stable/generated from path; unique | `partial_backend_not_product_ready` |
