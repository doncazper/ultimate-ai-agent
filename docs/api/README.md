...
Current OpenAPI path count: `211`, generated from the FastAPI application and
exposed through `/api/manifest`. `/api/manifest` currently reports `212` route
...
streaming progress, profile isolation, tool registry, session-search, and
context-reference routes, Governed Product Pilot authority profile, and runtime
parity loop add protected read-only `/api/runtime/*` inspection routes.
