...
| `ultimate_ai_agent.api.routes.runtime_pilot_service` | `/api/runtime/*` governed runtime pilot contract and metadata routes | May expose capability metadata, safe-ref invocation metadata, policy decisions, approval-ref bindings, metadata-only configured loopback local-model receipts, one exact allowlisted argv-only read-only command status receipt, exact Action Inbox approved focused pytest, repo verifier, and frontend check command receipts with redacted output summaries, blocked execution receipts, and safe-disable posture only; no repo verifier/frontend-check execution outside named exact lanes, remote provider/model call, arbitrary command text, arbitrary shell/subprocess execution, provider SDK call, browser automation, connector write, plugin import, remote execution, public release, or production runtime authority. | High because these routes name execution authority and must remain exact-scoped, disabled by default, and receipt-backed until each later promotion. | Governed runtime contract/storage tests, API manifest, OpenAPI contract, Foundation Gate. |
...
the accepted service-module boundary without changing the current 196-path API.
