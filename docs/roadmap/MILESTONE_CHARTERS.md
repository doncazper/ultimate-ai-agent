...
- M22 is implemented/released by v0.26.0 as contract-only and hardened by v0.26.1; M23 is implemented/released by v0.27.0 as manual fixed-prompt local call only; M24-M40 are planned/provisional in `docs/roadmap/M21_M40_CAPABILITY_CHARTERS.md`.
...
by v0.27.0 as manual fixed-prompt local call only.
