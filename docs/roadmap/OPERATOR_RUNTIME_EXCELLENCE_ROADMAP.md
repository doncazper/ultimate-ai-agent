...
| Ship one receipt-bearing Founder Loop V1 before broader expansion | `FCC-V1-000` Done: Control Center Release Surface Manifest; `FCC-V1-001` Done: API Perimeter For Real Mutations; `FCC-V1-002` Done: Action Inbox Backend State Machine; `FCC-V1-003` Planned: Founder Loop V1 Vertical Slice; `FCC-V1-004` Planned: Chat Durable Receipt And Handoff; `FCC-V1-005` Planned: Memory Review Decisions; `FCC-V1-006` Planned: Evidence Timeline Productization; `FCC-V1-007` Planned: Promotion And Proof Lane | P0/P1 | The conveyor in `docs/control_center/FOUNDER_LOOP_V1_MILESTONES.md` keeps running until Today item -> Action envelope -> exact approval -> durable receipt -> Evidence Timeline update is real, Chat has durable receipts and reviewable handoffs, Action Inbox approve/edit/reject/defer uses backend receipts, Memory Review accept/correct/reject uses backend receipts, and every visible route is promoted only through release-surface manifest proof lanes |
...
complete for release-surface truth; `FCC-V1-001` is complete for API perimeter
  ...
  backend decision state; and `FCC-V1-003` is the next productization conveyor
  milestone before
