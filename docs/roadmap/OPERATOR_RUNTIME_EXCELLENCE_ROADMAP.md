...
| Harden the browser-facing API perimeter before new authority | `UAA-P1-080` Done: route classification and public/protected inventory; `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` through `UAA-P1-086` planned: loopback CORS, local auth gate, idempotency audit, targeted rate limits, and OpenAPI/API manifest enforcement | P1 | Control Center remains local-first while every route now has explicit public/protected classification and centralized browser-hardening headers. Sensitive or mutating paths still need CORS, auth, approval, idempotency, redaction, rate-limit, and test posture before authority-heavy Plans, Chat, Code, loop-binding, or beta-readiness claims |
