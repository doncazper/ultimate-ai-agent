...
| Ship one receipt-bearing Founder Loop V1 before broader expansion | `FCC-V1-000` Done: Control Center Release Surface Manifest; `FCC-V1-001` Done: API Perimeter For Real Mutations; `FCC-V1-002` Done: Action Inbox Backend State Machine; `FCC-V1-003` Done: Founder Loop V1 Vertical Slice; `FCC-V1-004` Done: Chat Durable Receipt And Handoff; `FCC-V1-005` Done: Memory Review Decisions; `FCC-V1-006` Planned: Evidence Timeline Productization; `FCC-V1-007` Planned: Promotion And Proof Lane | P0/P1 | The conveyor in `docs/control_center/FOUNDER_LOOP_V1_MILESTONES.md` now has the first Today item -> Action envelope -> exact approval/edit/reject/defer receipt -> Evidence Timeline update path, durable Chat receipt and reviewable Actions/Plans handoff refs, and backend-owned Memory Review accept/correct/reject receipts; it keeps running until Evidence Timeline is productized and every visible route is promoted only through release-surface manifest proof lanes |
...
complete for release-surface truth; `FCC-V1-001` is complete for API perimeter
  ...
  decision receipts; and `FCC-V1-006` is the next productization conveyor
  milestone before
