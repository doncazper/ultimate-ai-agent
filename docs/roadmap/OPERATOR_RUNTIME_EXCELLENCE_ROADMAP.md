...
- `UAA-P1-087` private operator trial and UI functional tuning: after
  ...
  read-only Control Center surface; full `UAA-P1-087.2` uses that boot flow
  for local/in-person founder testing of Today, Actions, Memory,
  Evidence, Chat handoff, blocked-state language, and CRM-lite flow; and
  `UAA-P1-087.3` plans/source-scaffolds the native SwiftUI boot cockpit only
  after the launcher contract is proven. This sequence records findings, manual
