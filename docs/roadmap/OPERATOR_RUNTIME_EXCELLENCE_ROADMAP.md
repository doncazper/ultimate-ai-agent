...
| Harden the browser-facing API perimeter before new authority | `UAA-P1-080` Done: route classification and public/protected inventory; `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` Done: explicit loopback CORS allowlist; `UAA-P1-083` Done: configured local bearer gate for protected route classifications; `UAA-P1-084` Done: mutating-route idempotency header gate; `UAA-P1-085` Done: targeted local rate limits; `UAA-P1-086` planned: OpenAPI/API manifest/route inventory enforcement tests | P1 | Control Center remains local-first while every route now has explicit public/protected classification, centralized browser-hardening headers, exact loopback CORS allowlisting, a local protected-route bearer gate, a mutating-route idempotency header gate, and targeted local fixed-window rate limits for expensive/sensitive routes. Sensitive or mutating paths still need approval, redaction, and enforcement-test posture before authority-heavy Plans, Chat, Code, loop-binding, or beta-readiness claims |
| Run the first private operator trial before broader roadmap expansion | `UAA-P1-087` Planned: Private Operator Trial And UI Functional Tuning, sequenced as `UAA-P1-087.1` Local Launcher Dual-Surface Boot Readiness, `UAA-P1-087.2` In-Person Private Operator UI Functional Tuning, and `UAA-P1-087.3` Native SwiftUI Boot Cockpit Planning And Source-Only Scaffold | P1 | After API perimeter hardening, first prove the `.command`/launcher dual-surface boot path, then run in-person/local founder testing, then plan/source-scaffold native SwiftUI over the proven launcher contract before moving to P2/provider/packaging/commercialization work |
...
- API Boundary Hardening Lane, required before authority-heavy claims from
  ...
  authority. The remaining UAA-P1-086 lane is planned/queued and does not add
  broader runtime authority until separately implemented.
- `UAA-P1-087` Planned private operator trial and UI functional tuning: after
  UAA-P1-080 through UAA-P1-086, run the ordered private trial sequence:
  `UAA-P1-087.1` proves local launcher/`.command` dual-surface boot readiness
  for Control Center plus the secondary OpenWebUI shell; `UAA-P1-087.2` uses
  that boot flow for local/in-person founder testing of Today, Actions, Memory,
...
- `UAA-P1-086` Planned: OpenAPI/API manifest/route inventory tests enforcing
  ...
  posture.
- `UAA-P1-087` Planned: private operator trial and UI functional tuning after
  API boundary hardening, now ordered as `UAA-P1-087.1` launcher/`.command`
  dual-surface boot readiness, `UAA-P1-087.2` local hands-on UI tuning and
