...
| Promote the Web Runtime Authority Promotion Ladder through exact web/provider lanes | `WEB-RUNTIME-AUTH-001` Implementation: `WEB-HYBRID-001` through `WEB-HYBRID-007` provide exact SearXNG search, self-hosted and free-plan Firecrawl markdown extraction, one eligible self-host-first fallback, pinned loopback packaging, and backend-owned API/CLI/UI posture; `WEB-HYBRID-008` dogfood/promotion review remains active | P1 | Older web docs are historical/contract evidence, not blanket authority. Every exact attempt routes through `WebAccessGateway` and revalidates PolicyEngine, LocalApprovalAuthority, exact AuthorityLease, safe-disable, idempotency, audit, redaction, and cloud budget/reservation posture. The read model performs no probe and grants no authority. Unrestricted web fetching, browser actions, paid use, Keyless, cloud-first or multi-step fallback, raw payload persistence, and zero-data-retention claims remain blocked. |
