...
| Promote the Web Runtime Authority Promotion Ladder before web/provider execution | `WEB-RUNTIME-AUTH-001` Shaping: active roadmap/board lane for `ultimate_ai_agent.core.web_access`, governed read-only fetch migration, provider shells and diagnostics, read-only provider adapters, browser observe, browser action dry-run, low-risk clicks, connector-specific writes, and callable runtime authority sequencing | P1 | Older M72/M74/M75/M94/M95 and M121-M140 docs are historical/contract evidence, not blanket callable runtime authority. Web runtime authority remains one WIP lane. No live web fetching, Browserbase/Playwright automation, provider SDK calls, POST/click/form/download/upload, generic public-web mutation, or callable runtime authority ships until each step has `WebAccessGateway` routing, durable audit, side-effect ledger blockers, exact LocalApprovalAuthority scope, operator blocked/degraded/partial labels, CLI/UI inspection, verifier coverage, and rollback/revocation posture. Any paid/frontier provider use requires CostGovernor cost estimate refs, budget decision refs, cost receipt refs, provider/model safe refs, unknown paid-cost approval, and budget-exceeded blocking before routing. |
