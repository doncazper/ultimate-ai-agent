...
| Harden the browser-facing API perimeter before new authority | `UAA-P1-080` Done: route classification and public/protected inventory; `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` Done: explicit loopback CORS allowlist; `UAA-P1-083` through `UAA-P1-086` planned: local auth gate, idempotency audit, targeted rate limits, and OpenAPI/API manifest enforcement | P1 | Control Center remains local-first while every route now has explicit public/protected classification, centralized browser-hardening headers, and exact loopback CORS allowlisting. Sensitive or mutating paths still need auth, approval, idempotency, redaction, rate-limit, and test posture before authority-heavy Plans, Chat, Code, loop-binding, or beta-readiness claims |
...
- API Boundary Hardening Lane, required before authority-heavy claims from
  ...
  browser hardening only. The remaining lane is planned/queued and does not add
  auth, rate limits, dependencies, or runtime authority until separately
  implemented.
