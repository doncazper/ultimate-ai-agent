...
| Promote the Web Runtime Authority Promotion Ladder through exact web/provider lanes | `WEB-RUNTIME-AUTH-001` Implemented exact slice: `WEB-HYBRID-001` through `WEB-HYBRID-008` provide exact SearXNG search, self-hosted and free-plan Firecrawl markdown extraction, one eligible self-host-first fallback, pinned loopback packaging, backend-owned API/CLI/UI posture, and bounded private dogfood/hardening evidence | P1 | Older web docs are historical/contract evidence, not blanket authority. Every request-scoped exact attempt routes through `WebAccessGateway` and revalidates PolicyEngine, LocalApprovalAuthority, exact AuthorityLease, safe-disable, idempotency, audit, redaction, and cloud budget/reservation posture. Cloud use is serialized to one UAA request for usage attribution. The read model performs no probe and grants no authority. Unrestricted web fetching, browser actions, paid use, Keyless, cloud-first or multi-step fallback, raw payload persistence, provider retention guarantees, and zero-data-retention claims remain blocked. |
