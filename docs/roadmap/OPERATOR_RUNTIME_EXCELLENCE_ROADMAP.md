...
| Promote a Today-spine, memory-first private beta path | `UAA-P1-067` Done: Today-Spine Founder Command Center beta-readiness planning/currentness path; `UAA-P1-068` Done: Today Product Spine Contract; `UAA-P1-069` Done: Evidence History Grammar; `UAA-P1-070` Done: Memory Source And Provenance Model; `UAA-P1-071` Done: Memory Review Decision Capture; `UAA-P1-072` Done: Business Memory And Memory Quality Controls; `UAA-P1-073` Done: Plans To Reviewable Action Envelopes; `UAA-P1-074` Done: Chat Local Operator Surface; `UAA-P1-075` Done: Governed Code Workbench V1; `UAA-P1-076` Done: Cross-Surface Memory Intake; `UAA-P1-077` Done: Memory-To-Loop Binding; `UAA-P1-078` Done: Private Beta-Readiness Gate; `UAA-P1-079` Done: User Intent Understanding V1; `UAA-P1-080` Done: API Route Classification And Public/Protected Inventory; then `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` Done: explicit loopback CORS allowlist; `UAA-P1-083` Done: local bearer gate for protected routes; `UAA-P1-084` Done: mutating-route idempotency header gate; `UAA-P1-085` Done: targeted local rate limits for expensive and sensitive routes; `UAA-P1-086` Done: API boundary enforcement tests; `UAA-P1-087.1` Done: local launcher dual-surface boot readiness; `UAA-P1-087.2a` Done: private trial packet and UI tuning surface; `UAA-P1-087.2b` Done: private trial findings capture and acceptance ledger; `UAA-P1-087.2c` Done: unanswered manual review scaffold; `FCC-V1-000` Done: Control Center release surface manifest; `FCC-V1-001` Done: API perimeter for real mutations contract/verifier coverage; `FCC-V1-002` Done: Action Inbox Backend State Machine; `FCC-V1-003` Done: Founder Loop V1 Vertical Slice; `FCC-V1-004` Done: Chat Durable Receipt And Handoff; `FCC-V1-005` Done: Memory Review Decisions; `FCC-V1-006` Done: Evidence Timeline Productization; `FCC-V1-007` Done: Promotion And Proof Lane for exact route-surface proof of `/actions`, `/chat`, `/memory`, and `/evidence` while full `UAA-P1-087.2`/`UAA-P1-087.3` remain deferred | P0 | Today becomes the product spine; every module feeds Today, Actions, Evidence, and Memory. Memory becomes the product differentiator only after the loop has reviewable evidence, action envelopes, Chat operator truth refs, governed Code proposal refs, safe source refs from ChatGPT/manual review/local coding/calendar/email metadata, memory-to-loop refs, backend-owned Memory Review decision receipts, productized Evidence Timeline events, private beta-readiness evidence states, reviewable user intent proposals, route classification truth, release-surface truth, backend-owned Action Inbox decision receipts, the first Today-to-Action receipt loop, durable Chat receipt/handoff refs, no hidden prompt injection, no raw private content, no public beta or connector authority claim, and a local/private boot plus UI trial scaffold that remains honest until the product is ready for later manual review |
...
| Harden the browser-facing API perimeter before new authority | `UAA-P1-080` Done: route classification and public/protected inventory; `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` Done: explicit loopback CORS allowlist; `UAA-P1-083` Done: configured local bearer gate for protected route classifications; `UAA-P1-084` Done: mutating-route idempotency header gate; `UAA-P1-085` Done: targeted local rate limits; `UAA-P1-086` Done: OpenAPI/API manifest/route inventory enforcement tests | P1 | Control Center remains local-first while every route now has explicit public/protected classification, centralized browser-hardening headers, exact loopback CORS allowlisting, a local protected-route bearer gate, a mutating-route idempotency header gate, targeted local fixed-window rate limits for expensive/sensitive routes, and enforcement tests. Sensitive or mutating paths still need exact approval and redaction posture before authority-heavy Plans, Chat, Code, loop-binding, or beta-readiness claims |
| Promote the Web Runtime Authority Promotion Ladder before web/provider execution | `WEB-RUNTIME-AUTH-001` Shaping: active roadmap/board lane for `ultimate_ai_agent.core.web_access`, governed read-only fetch migration, provider shells and diagnostics, read-only provider adapters, browser observe, browser action dry-run, low-risk clicks, connector-specific writes, and callable runtime authority sequencing; `docs/network/SEARXNG_FIRECRAWL_HYBRID_IMPLEMENTATION_PLAN.md` is the planning-only child sequence for SearXNG plus hybrid Firecrawl | P1 | Older M72/M74/M75/M94/M95 and M121-M140 docs are historical/contract evidence, not blanket callable runtime authority. Web runtime authority remains one WIP lane. The SearXNG/Firecrawl child plan adds no Docker runtime, live web fetching, credential, provider call, or second WIP lane by itself. No live web fetching, Browserbase/Playwright automation, provider SDK calls, POST/click/form/download/upload, generic public-web mutation, or callable runtime authority ships until each step has `WebAccessGateway` routing, durable audit, side-effect ledger blockers, exact LocalApprovalAuthority scope, operator blocked/degraded/partial labels, CLI/UI inspection, verifier coverage, and rollback/revocation posture. Any paid/frontier provider use requires CostGovernor cost estimate refs, budget decision refs, cost receipt refs, provider/model safe refs, unknown paid-cost approval, and budget-exceeded blocking before routing. |
| Plan background/autonomous provider-call promotion before any runtime work | `BACKGROUND-AUTONOMOUS-PROVIDER-CALLS` Planning-only: define gates for scoped autonomy windows, exact provider/model refs, exact credential refs, max spend per window, per-request and per-session budgets, CostGovernor hard blocks, queue inspection, kill switch, revocation, replay/audit, red-team checks, UI/CLI parity, no hidden prompt injection, no raw payload persistence, incomplete-cost blocking, explicit human approval boundaries, and safe-disable/rollback posture | P1 | Background/autonomous provider calls remain blocked. This planning lane adds no background execution, scheduler, autonomous model calls, provider calls, runtime activation, billing authority, broad provider router, new API runtime route, public beta, public release, or production authority. Any later runtime PR must be separately scoped and verifier-backed. |
| Define provider billing authority before spend-bearing runtime work | `PROVIDER-BILLING-AUTHORITY-BOUNDARY` Planning-only: define `no_billing_authority`, `per_request_max_usd`, `per_session_max_usd`, `spend_window_exhausted`, `unknown_cost_blocked`, `incomplete_cost_blocked`, and `billing_review_required` states; require exact approval, CostGovernor hard limits, actual usage/cost receipts, incomplete-cost blocking, revocation, UI/CLI inspection, audit/replay posture, safe-disable/rollback posture, and no broad spend toggle | P1 | Provider billing authority remains blocked. This planning lane adds no billing integration, payment methods, subscription management, broad spend toggle, provider calls, autonomous/background calls, runtime activation, public beta, public release, or production authority. Any later billing authority PR must be separately scoped and verifier-backed. |
| Keep the private operator trial honest before broader expansion | `UAA-P1-087.1` Done: Local Launcher Dual-Surface Boot Readiness; `UAA-P1-087.2a` Done: Private Trial Packet And UI Tuning Surface; `UAA-P1-087.2b` Done: Private Trial Findings Capture And Acceptance Ledger; `UAA-P1-087.2c` Done: Private Trial Manual Review Intake Scaffold; full `UAA-P1-087.2` Deferred: In-Person Private Operator UI Functional Tuning; `UAA-P1-087.3` Deferred: Native SwiftUI Boot Cockpit Planning And Source-Only Scaffold | P1 | After API perimeter hardening, the `.command`/launcher dual-surface boot path is proven and safe-ref-only trial packet, acceptance ledger, and unanswered manual-review scaffold exist; next build the missing Founder Loop implementation in FCC-V1 before later in-person/local founder testing records accepted/revised findings |
| Score core agent modules from repo evidence before the next capability push | `UAA-P1-088` Done: Agent Module Maturity Review V2; `UAA-P1-089` Done: Top-Level Decision Router Contract; `UAA-P1-090` Done: Task Decomposition Proposal Engine; `FCC-LOOP-002` Done: Founder Loop Ergonomics Pass; `FCC-MEM-022` Done: Ranked Retrieval / Recall Tuning | P0/P1 | The review/scoring/read-model lane covers every requested module, calls out `decision_router` and `task_decomposition_module` as weakest, preserves the V1 score model, requires subagent review for queued follow-ups, and the router plus task proposal contracts now model deterministic no-effect route/decomposition proposals. FCC-LOOP-002 adds daily-loop command grouping and operator summaries over existing backend read models. FCC-MEM-022 adds lexical/tag/ref-only ranked recall diagnostics, safe-query hash refs, feedback receipts, epistemic roles, observation candidates, probe, contradiction previews, and an explicit HRR readiness gate. These lanes add no backend authority beyond local idempotent feedback posture updates, runtime model calls, provider calls, shell/subprocess execution, network/browser authority, connector writes, external memory provider runtime, cloud sync, HRR/algebraic retrieval, vector DB, embeddings, context injection, tool execution, action execution, workflow execution, autonomous routing/planning authority, public beta, or production authority |
| Ship one receipt-bearing Founder Loop V1 before broader expansion | `FCC-V1-000` Done: Control Center Release Surface Manifest; `FCC-V1-001` Done: API Perimeter For Real Mutations; `FCC-V1-002` Done: Action Inbox Backend State Machine; `FCC-V1-003` Done: Founder Loop V1 Vertical Slice; `FCC-V1-004` Done: Chat Durable Receipt And Handoff; `FCC-V1-005` Done: Memory Review Decisions; `FCC-V1-006` Done: Evidence Timeline Productization; `FCC-V1-007` Done: Promotion And Proof Lane | P0/P1 | The conveyor in `docs/control_center/FOUNDER_LOOP_V1_MILESTONES.md` now has the first Today item -> Action envelope -> exact approval/edit/reject/defer receipt -> Evidence Timeline update path, durable Chat receipt and reviewable Actions/Plans handoff refs, backend-owned Memory Review accept/correct/reject receipts, and productized Evidence Timeline event groups; it now promotes only `/actions`, `/chat`, `/memory`, and `/evidence` through the focused proof lane while `/today` stays partial and `/inbox`, `/settings`, and model lifecycle stay blocked or partial |
...
- Chat Local Operator: first-party Control Center chat operator surface with
...
- `UAA-P1-033` Done: add accessible loading/error/empty/blocked/denied
  operator states for Chat Local Operator, Plans, Models, Approvals, Files, Runtime,
...
- `UAA-P1-074` Done: First-party Control Center chat local operator surface: Chat
  sends a local turn through the governed local gateway, shows
  model/runtime/auth/tool-denial truth, produces safe evidence refs, and hands
...
- `UAA-P1-077` Done: Memory-to-loop binding: Today, Action Inbox, Evidence
  Timeline, Memory Review, and Weekly CEO Review show memory candidates,
  accepted recall display-only refs, corrections, rejected items, follow-up
  commitments, memory-derived Action proposals, stale refs, and
  missing-evidence blockers in human-readable form.
- `UAA-P1-078` Done: Private beta-readiness gate: defines local/private
  beta-test acceptance evidence for Morning Briefing, Action Inbox, Memory
  Review, Evidence Timeline, safe local Chat/Plans handoff, governed Code
  proposal refs, and CRM-lite follow-ups with pass/fail/skipped/blocked/
  partial/mock-only/accepted-failure states. This gate is not public beta,
  public distribution, production readiness, or broad autonomy.
- `UAA-P1-079` Done: User Intent Understanding V1 adds reviewable intent
  proposals with confidence, source refs, evidence refs, ambiguity posture, and
...
- API Boundary Hardening Lane, required before authority-heavy claims from
  ...
  `UAA-P1-081` Done: centralized FastAPI security headers;
  `UAA-P1-082` Done: explicit loopback CORS allowlist;
  ...
  authority. UAA-P1-084 adds a runtime idempotency header gate only; it is not
  durable dedupe, exactly-once execution, replay execution, mutation authority,
  or production authority. UAA-P1-085 adds targeted local rate limits only; it
  does not add auth, distributed quota, dependencies, billing, or production
  authority. UAA-P1-086 adds enforcement tests and does not add broader
  runtime authority.
- `UAA-P1-087` private operator trial and UI functional tuning: after
  UAA-P1-080 through UAA-P1-086, `UAA-P1-087.1` proves local launcher/
  `.command` dual-surface boot readiness for Control Center plus the secondary
  OpenWebUI shell; `UAA-P1-087.2a` adds a safe-ref-only trial packet and
  read-only Control Center surface; `UAA-P1-087.2b` adds the acceptance ledger
  and pending manual-smoke review refs; `UAA-P1-087.2c` adds unanswered manual
  review slots, missing implementation refs, and deferred decision refs. Full
  `UAA-P1-087.2` is deferred until more Founder Loop implementation exists,
  then uses that boot flow for local/in-person founder testing of Today,
  Actions, Memory, Evidence, Chat handoff, blocked-state language, and
  CRM-lite flow; and `UAA-P1-087.3` plans/source-scaffolds the native SwiftUI
  boot cockpit only after the launcher contract and accepted/revised findings
  are proven. This sequence prepares findings, manual
  smoke checklists, UI/copy tuning, and beta-readiness friction without adding
...
- `UAA-P1-077` Done: bind memory state into Today, Action Inbox, Evidence
  Timeline, Memory Review, and Weekly CEO Review.
- `UAA-P1-078` Done: private beta-readiness gate for the local Founder Command
...
- `UAA-P1-081` Done: centralized FastAPI response security headers for the
  browser-facing Control Center boundary, with HTTPS-only HSTS and no CORS,
  auth, rate-limit, route authority, runtime authority, or production claim.
- `UAA-P1-082` Done: explicit loopback CORS allowlist for local Control Center
  dev/preview origins only, with wildcard CORS and CORS credentials denied;
  CORS is browser hardening, not auth.
- `UAA-P1-083` Done: simple configured local bearer gate for protected route
  classifications; no enterprise auth, OAuth, roles, password flow, or
  production authority.
- `UAA-P1-084` Done: mutating-route idempotency enforcement audit requiring
  `X-UAA-Idempotency-Key` or `X-UAA-Idempotency-Ref` before mutating handlers,
  without durable dedupe, exactly-once execution, or production authority.
- `UAA-P1-085` Done: targeted local fixed-window rate limits for model/chat,
  task decomposition, action preview/proposal, and expensive
  validation/local-model paths.
- `UAA-P1-086` Done: OpenAPI/API manifest/route inventory tests enforcing
  classification, auth, approval, idempotency, headers, CORS, and rate-limit
  posture.
- `UAA-P1-087.1` Done: launcher/`.command` dual-surface boot readiness after
  API boundary hardening.
- `UAA-P1-087.2a` Done: safe-ref-only private trial packet and read-only
  Control Center tuning surface.
- `UAA-P1-087.2b` Done: safe-ref-only private trial acceptance ledger,
  manual smoke step refs, pending surface review refs, acceptance question
  refs, and tuning decision refs.
- `UAA-P1-087.2c` Done: safe-ref-only unanswered manual review scaffold,
  pending answer refs, missing implementation refs, and deferred decision refs.
- `UAA-P1-087.2` Deferred: later local hands-on UI tuning and manual smoke
