...
| Harden the browser-facing API perimeter before new authority | `UAA-P1-080` Done: route classification and public/protected inventory; `UAA-P1-081` Done: centralized FastAPI security headers; `UAA-P1-082` Done: explicit loopback CORS allowlist; `UAA-P1-083` Done: configured local bearer gate for protected route classifications; `UAA-P1-084` through `UAA-P1-086` planned: idempotency audit, targeted rate limits, and OpenAPI/API manifest enforcement | P1 | Control Center remains local-first while every route now has explicit public/protected classification, centralized browser-hardening headers, exact loopback CORS allowlisting, and a local protected-route bearer gate. Sensitive or mutating paths still need approval, idempotency, redaction, rate-limit, and test posture before authority-heavy Plans, Chat, Code, loop-binding, or beta-readiness claims |
...
- API Boundary Hardening Lane, required before authority-heavy claims from
  ...
  authority. The remaining lane is planned/queued and does not add idempotency,
  rate limits, dependencies, or runtime authority until separately implemented.
