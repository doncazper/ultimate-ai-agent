...
M32 is implemented/released as one safe local filesystem metadata tool under server-owned safe roots. It does not add file content reads, directory listing, traversal, mutation, backend execution routes, arbitrary tools, or production authority. M33-M40 remain planned/provisional; future native-client, device, browser, observability, and evaluation work must not bypass approvals, consent, receipts, redaction, or Python Agent Core authority.
