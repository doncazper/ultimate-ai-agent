...
M31 is implemented/released as the first governed tool runtime adapter and enables only the deterministic no-op tool. M32-M40 remain planned/provisional; future tool-runtime, native-client, device, browser, observability, and evaluation work must not bypass approvals, consent, receipts, redaction, or Python Agent Core authority.
