...
| `threat-ref:matrix:unknown-delivery` | crash after remote start or local receipt-persistence failure causes duplicate send or false cancellation/success | durable pre-start/start boundary; remote terminal evidence alone marks delivery and cannot be masked by a later local outbox failure; local persistence failure is reconciliation-required; unknown remains uncertain/recovery-required; no automatic non-idempotent replay | crash-before/after-start, local acknowledgement-persistence failure, and remote-echo reconciliation tests |
