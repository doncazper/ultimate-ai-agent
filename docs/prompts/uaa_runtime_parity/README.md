# UAA Runtime Parity Prompt Pack

Status: stored execution prompts, not runtime authority

Purpose: push UAA toward external-comparison-runtime-level real operation-loop maturity while
...
This pack treats external comparison runtime as a read-only architectural reference. It may
adapt patterns, failure modes, and product contracts, but it must not import
external runtime packages or copy implementation code wholesale. Every improvement
...
```bash
bash scripts/dev/run_uaa_runtime_parity_prompt_pack.sh
```

Dry-run and emit the combined prompt without invoking Codex:

```bash
bash scripts/dev/run_uaa_runtime_parity_prompt_pack.sh --dry-run
```

Emit a reviewable combined prompt to a chosen path:

```bash
bash scripts/dev/run_uaa_runtime_parity_prompt_pack.sh --dry-run --output /tmp/uaa-runtime-parity.md
```
...
external comparison runtime gaps:
...
## external comparison runtime Reference Patterns
...
```bash
.venv/bin/python scripts/verify_uaa_runtime_parity_prompt_pack.py
```

Run the focused unit test:

```bash
PYTHONPATH=src .venv/bin/python -m pytest tests/test_uaa_runtime_parity_prompt_pack.py -q
```
