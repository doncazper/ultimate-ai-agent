...
4. Capture an exact five-minute `LocalApprovalAuthority` grant.
5. Issue and evaluate one exact Contacts/write `AuthorityLease` constrained to
   the contract, route, action, preview, approval, revision, payload, and
   idempotency refs with an operation budget of one.
6. Validate the complete prospective state, append a content-free audit event,
   and atomically replace the encrypted state.
