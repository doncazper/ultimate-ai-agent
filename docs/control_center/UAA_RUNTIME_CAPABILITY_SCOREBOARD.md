...
| Research, web, and external information handling | 5 | partial | 7 | partial | external comparison runtime | UAA has WebAccessGateway guardrails, one AuthorityLease-gated allowlisted web evidence preview lane, and `contract-ref:external-information-handling-posture:v1` to distinguish trusted local evidence, untrusted external content, planned browser observe, blocked browser actions, and blocked provider/search expansion. external comparison runtime has provider/research and browser/evidence posture in docs/contracts. | Keep UAA web evidence read-only and gateway-owned; do not enable browser action or unrestricted web fetching without a separate AuthorityLease-gated capability. |
