...
It is not runtime authority, does not change Control Center behavior, and does
...
| Planning and orchestration | 8 | implemented | 8 | implemented | Tie | UAA has `contract-ref:durable-orchestration-posture:v1`, append-first run records, canonical lifecycle states, run observability, approval waits, retry/recovery diagnostics, cancellation/dead-letter posture, staged checkpoints, and one exact AuthorityLease-gated approved-runtime-command step. external comparison runtime documents durable mission sessions, checkpoints, retries, approval waits, dead letters, and fan-out. | Keep generic autonomous recovery, cancel/resume, retry workers, schedulers, and production orchestration blocked until exact AuthorityLease-gated execution lanes exist. |
...
| Communication and interaction quality | 8 | implemented | 8 | implemented | Tie | UAA now exposes `contract-ref:founder-loop-product-cockpit-posture:v1` under `GET /control-center/agent-loop/thread#high_maturity_spine_readiness`, tying Start Here, Today, Plans, Actions, Proof, Evidence, Memory, Trust, Coding, Work Board, Runtime, Providers, Web Evidence, and blocked-state refs into one readable operator loop. external comparison runtime README shows a unified Work/Projects/Library/Ops/Settings cockpit. | Keep replacing fallback-shaped panels with backend-owned route/CLI/UI/evidence/test refs; do not add mutation controls without exact AuthorityLease-gated lanes. |
| Action and tool calling | 8 | implemented | 8 | implemented | Tie | UAA has `contract-ref:action-tool-lane-posture:v1`, the Python-owned action/tool/code catalog, preview-only tools, one exact local mutation lane, exact approval-bound RuntimeGateway utility lanes, proposal-only code workflows, receipts/evidence/proof refs, and blocked broad execution flags. external comparison runtime shows capability/tool catalogs, policy-gated invocation, approvals, and code-mode receipts. | Keep generic tool execution, unrestricted shell, browser automation, connector writes, provider/model calls, plugin runtime import, and production authority blocked unless exact AuthorityLease-gated lanes are accepted. |
...
| Research, web, and external information handling | 8 | implemented | 7 | partial | UAA | UAA has WebAccessGateway guardrails, one AuthorityLease-gated allowlisted web evidence preview lane, and `contract-ref:external-information-handling-posture:v1` to distinguish trusted local evidence, untrusted external content, planned browser observe, blocked browser actions, and blocked provider/search expansion. external comparison runtime has provider/research and browser/evidence posture in docs/contracts. | Keep UAA web evidence read-only and gateway-owned; do not enable browser action, provider search/scrape runtime, or unrestricted web fetching without a separate AuthorityLease-gated capability. |
| Model/provider management | 8 | implemented | 8 | implemented | Tie | UAA has `contract-ref:model-provider-management-posture:v1`, the read-only provider control plane, delegated runtime model catalog, model slot posture, role/provider evidence, provider research posture, cost hooks, router traces, local lifecycle posture, exact tiny-provider/credential-validation lane metadata, and blocked provider/model authority. external comparison runtime has provider catalog, local llama.cpp posture, model discovery, provider summaries, and model-router trace contracts. | Keep provider SDK calls, remote model calls, model-output authority, runtime selection mutation, lifecycle start/stop, raw provider payload persistence, and production authority blocked unless exact AuthorityLease-gated lanes are accepted. |
...
| UX as an AI cockpit | 8 | implemented | 9 | implemented | external comparison runtime | UAA renders the Founder Loop Product Cockpit in the Agent Loop Thread panel from Python/API truth with route refs, CLI refs, UI refs, evidence refs, test refs, blocked-authority refs, and no raw JSON as the primary operator workflow. external comparison runtime presents one Work cockpit with Library/Ops/Settings and visual proof. | Improve visual polish and live usefulness while preserving the non-executing read-model boundary. |
...
| Productized agent loop | 8 | implemented | 9 | implemented | external comparison runtime | UAA has a governed Founder Loop across Start, Today, Plans, Actions, Proof, Evidence, Memory, Trust, Coding, Work Board, Runtime, Providers, Web Evidence, and the High-Maturity Spine through `contract-ref:founder-loop-product-cockpit-posture:v1` plus CLI parity at `scripts/dev/uaa_founder_loop.py inspect-product-cockpit-posture`. external comparison runtime presents a broader input-to-plan-to-action-to-evidence-to-memory loop. | More live useful lanes still need exact authority before mutation; unsupported adapters remain blocked. |
