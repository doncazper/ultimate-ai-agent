...
| Governed Runtime Pilot | 0 for Phase 01 baseline freeze; planned scoped promotion for v0.105.0 | Promote through separate PRs only after RuntimeGateway profiles, policy decisions, redacted receipts, exact Action Inbox approval validation, CLI/API/Control Center parity, safe-disable refs, rollback posture, and verifiers are implemented; execution-capable calls or commands still require an `operator-approved` Action Inbox approval envelope | Phase-specific RuntimeGateway contract tests, local model loopback tests, allowlisted argv command tests, Action Inbox execution-bridge tests, receipt/evidence redaction tests, API manifest/OpenAPI checks, product-language checks, Foundation Gate | `sealed` default profile, per-profile disable config, per-lane safe-disable refs, approval-scope validation, bounded output summaries, receipt-backed audit refs, no raw durable payloads | Baseline tag `uaa-governed-runtime-baseline-2026-07-04`; Phase 01 contract recorded in `VERSION.md`, product release truth, and current board; runtime evidence remains pending later phases | No unrestricted shell, unrestricted provider/model authority, browser automation, connector writes, plugin runtime import, remote execution, unrestricted web access, raw prompt/response/provider payload/local path/command output persistence, public beta, public release, production authority, or broad autonomy |
