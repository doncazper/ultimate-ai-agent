...
| Governed Runtime Pilot | Phase 05 scoped pilot: configured loopback local-model receipts, one exact allowlisted argv-only read-only command status receipt, and exact Action Inbox approved focused pytest command receipts; broader runtime authority blocked | Promote further through separate PRs only after broader Control Center parity, CLI proof, safe-disable refs, rollback posture, content/clean-tree binding, and verifiers are implemented for each additional command/provider lane; execution-capable non-status commands still require an `operator-approved` Action Inbox approval envelope | Phase-specific RuntimeGateway contract tests, local model loopback tests, allowlisted argv command tests, Action Inbox execution-bridge tests, receipt/evidence redaction tests, API manifest/OpenAPI checks, product-language checks, Foundation Gate | `sealed` default profile, per-profile disable config, per-lane safe-disable refs, approval-scope validation, bounded output summaries, receipt-backed audit refs, no raw durable payloads | Baseline tag `uaa-governed-runtime-baseline-2026-07-04`; Phase 03 local loopback model receipt evidence, Phase 04 exact read-only command receipt evidence, and Phase 05 focused pytest bridge evidence are recorded in `VERSION.md`, product release truth, runtime contract/API tests, Control Center tests, and `scripts/dev/uaa_runtime.py inspect-action-inbox-bridge` | No unrestricted shell, arbitrary command text, repo verifier/frontend-check execution, unrestricted provider/model authority, browser automation, connector writes, plugin runtime import, remote execution, unrestricted web access, raw prompt/response/provider payload/local path/command output persistence, public beta, public release, production authority, or broad autonomy |
