# Founder Command Center Authority Capability Conveyor
...
`FCC-AUTH-RAMP-001` is the repo-owned authority capability conveyor for moving
one explicit Founder Command Center domain/capability from visible proposal
state to possible AuthorityLease-gated implementation. It does not grant authority by itself.
...
-> one approved authority capability candidate
-> AuthorityLease-gated implementation later
```
...
The first implementation capability is fixed and may not be substituted:

```text
read_only_real_world_web_fetch through WebAccessGateway
```

This capability is narrower than the follow-on authority candidate set. It may only
...
If this capability cannot satisfy the lease/domain/capability gate, the program
must record a blocked/no-go posture and harden docs, tests, and verifiers
instead of selecting another capability.

## Foundation Capabilities

The near-term foundation stays read-only or proposal-only:

| Capability | Current posture | Authority boundary |
|---|---|---|
...
`memory_write` is implemented only as the narrow reviewed recall-write capability
for Memory Review accept/correct decisions. The capability is local-first,
Founder Loop valuable, and bound to active memory/write AuthorityLease scope,
exact approval, `LocalApprovalAuthority` validation, idempotency, receipts,
safe-disable posture, CLI parity, and verifier refs.
...
context-pack preview/materialization capability. Phase 6.1 may create an internal
...
`contract_ready`, `authority_capability_candidate`, `implemented`, or
`blocked_by_policy`.

## Authority Capability Gate

A candidate may become `authority_capability_candidate` only when all of the following
...
At most one candidate may be selected for the first authority capability at a
time. If no candidate satisfies the gate, the first authority capability
decision must be `no_go` with a blocker and smallest next safe action.

## Current Decision

The first follow-on authority capability is selected: `memory_write`, only for
reviewed recall-write accept/correct decisions under active memory/write
AuthorityLease scope. The existing Action Inbox `local_task_create` capability
remains rank 5, and Memory now has a separate rank 5
`reviewed_memory_recall_write` capability inside an otherwise limited module. Future
...
AuthorityLease-gated capability is ready.
The fixed first implementation capability, `read_only_real_world_web_fetch` through
...
focused verifier refs for the narrow accept/correct capability only.

The next safe context step is not prompt injection. It is a narrow
context-pack materialization or preview capability that would produce
operator-reviewable refs and receipts only, if and when a later PR proves that
capability under AuthorityLease evaluation.
