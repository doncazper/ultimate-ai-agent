...
redacted preview, durable safe-ref audit, and blocked labels for every
...
|---|---|---|
| Read-only real-world web fetch through `WebAccessGateway` | Partial: M72 contracts, policy, fake-transport tests, gateway routing, and static guards exist; real-world transport graduation is reserved for Prompt 02. | HTTPS GET only, explicit public allowlist, bounded redacted preview, safe refs, durable audit posture. No raw body/header persistence, browser automation, provider SDK call, connector write, credentials/cookies, downloads/uploads, POST/PUT/PATCH/DELETE, memory write, context injection, action execution, generic browsing, or production authority. |
...
The fixed first implementation lane is `read_only_real_world_web_fetch` through
`WebAccessGateway`; it is not a follow-on authority candidate and cannot be
