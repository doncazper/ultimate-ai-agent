...
Status: Active for v0.18.0 / M14 local backend connection stabilization. The frontend route set is unchanged.
...
OpenAPI remains a backend contract. v0.18.0 changes only `info.version` to `0.18.0`; backend path count remains `74` with unique operation IDs.
