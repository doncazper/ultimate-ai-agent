...
| Mature action execution | Implemented | Named RuntimeGateway micro-lanes: one read-only local status command and exact Action Inbox approved focused pytest, repo-verifier, and frontend-check command execution. | No generic tool execution, arbitrary shell/subprocess, networked commands, installs, background execution, or approval refs as authority. |
...
|---|---|---|---|
| Mature action execution | Strong exact-lane local pilot | RuntimeGateway allowlist, Action Inbox approval envelope, receipt refs, CLI/API/Core parity, and tests for read-only command plus exact approved focused pytest, repo-verifier, and frontend-check execution. | More action lanes require separate scope, approval binding, rollback/safe-disable, redaction, and verifier coverage. |
