...
|---|---|---|---|
| Mature action execution | Strong exact-lane local pilot | RuntimeGateway allowlist, Action Inbox approval envelope, receipt refs, CLI/API/Core parity, and tests for read-only command plus exact approved focused pytest, repo-verifier, frontend-check, and repo-doctor execution. | More action lanes require separate scope, approval binding, rollback/safe-disable, redaction, and verifier coverage. |
