...
| Settings | `/settings` now has backend-owned read-only status from `GET /control-center/settings/status` for maturity gate status, feature-flag posture, kill-switch posture, route safety refs, blocked authority classes, and review-only settings proposal refs. Local backend base policy remains code/config only, and FCC-P1-011 remains the Settings kill-switch/feature-flag spec foundation. | `GET /control-center/settings/status`, plus related status from `/control-center/status`, `/runtime/readiness`, and `/api/manifest`. | Loopback auth-token setup/status route, reviewed local llama.cpp settings route, local model identity/alias status route, local model lifecycle safe-disable route, and any mutation route. | Settings must remain disabled-by-default, redacted, local-only, revocable, and policy-gated. Feature-flag, kill-switch, permission-mode, model identity, and lifecycle names are posture vocabulary only until separate scoped authority exists. | `GET /control-center/settings/status` is `validation_only`; `/settings` owns presentation state only. | No approval is required for read-only Settings status. Exact approval is required for any setting that enables runtime authority, persistence, local lifecycle behavior, model identity mutation, or other mutation. | FCC-P1-011 spec refs in `docs/control_center/SETTINGS_KILL_SWITCH_FEATURE_FLAGS_SPEC.md`, operational maturity manifest/verifier refs, route-status refs, blocked authority refs, and safe settings proposal refs only; future local model manager receipts must use redacted refs only. | Operational maturity scorecard: Settings is rank 2 proposal review for backend-owned status and review-only settings proposal refs only. | Mutating feature flags, kill-switch execution, reviewed runtime settings mutation, local model identity mutation, and lifecycle safe-disable execution remain future scoped milestones. |
