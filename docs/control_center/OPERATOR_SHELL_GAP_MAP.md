...
| Models | `/models` now exposes accessible loading/error/empty/blocked/denied state copy only, while `GET /control-center/local-models/status` exposes backend-owned read-only inventory/gateway posture and review-only lifecycle proposal refs. Runtime panels still carry the implemented readiness/capability summaries. UAA-P1-064 adds read-only Python Agent Core inventory and CLI inspection only; UAA-P1-066 is now represented as Control Center read-only status with no activation control. | `GET /control-center/local-models/status`, `GET /v1/models`, `POST /models/route/preview`, `POST /model-runtime/manifests/validate`, `POST /model-runtime/requests/validate`, `POST /model-runtime/responses/validate`, `GET /runtime/readiness`, `GET /runtime/capability-matrix`; CLI-only `uaa local-model status/list/inspect` for UAA-P1-064 inventory. | Current loaded-model receipt route, memory-fit planner route, GGUF selection/approval/readiness route, llama.cpp lifecycle status route, dry-run switch planner, approval-bound switch execution route, one-big-model guard status, identity-update receipt route, tuning recommendation route, rollback status route. | M160-M167 local model lane only; model/provider output is not production authority. UAA-P1-064 adds read-only inventory and CLI inspection only; UAA-P1-066 renders read-only backend-owned inventory/status state and review-only proposal refs. Lifecycle, switch receipts, identity updates, safe-disable, rollback, and later backend contracts remain future Python Agent Core authority. Control Center and OpenWebUI render state and request governed actions only. | `/v1/*` is `local_dev_workspace_only`; model-runtime, model-route, and `GET /control-center/local-models/status` routes are `validation_only`; `/models` owns presentation state only. | Approved GGUF/model refs and reviewed local runtime settings are required before live local use. Exact approval is required for any start, stop, switch, identity update, or lifecycle mutation. | M167 evidence matrix, local E2E smoke harness, P0-015 checklist evidence refs, P0-016 tuning hardening test refs, P0-017 operational runbook refs, UAA-P1-062 scope doc, UAA-P1-064 read-only inventory scope doc, UAA-P1-066 read-only Control Center status scope doc, backend-owned local model status refs, UAA-P1-064 core/CLI test refs, route preview decisions, latency results, and future backend receipts for status, fit plans, lifecycle actions, identity updates, redacted logs/status, safe-disable, and rollback. | Operational maturity scorecard: Local Models is rank 2 proposal review for read-only status and lifecycle proposal refs only. | Reviewed local operational recovery evidence, reviewed hardware evidence, later lifecycle/status receipts, one-big-model enforcement, switch rollback proof, and any start/stop/switch/download authority remain future work. |
...
| Settings | `/settings` now has backend-owned read-only status from `GET /control-center/settings/status` for maturity gate status, feature-flag posture, kill-switch posture, route safety refs, blocked authority classes, review-only settings proposal refs, and Product Loop 011 authority posture labels. The posture matrix covers web, providers, connectors, memory context use, model runtime, local model lifecycle, and platform capabilities with blocked/degraded/partial/metadata-only states. Local backend base policy remains code/config only, and FCC-P1-011 remains the Settings kill-switch/feature-flag spec foundation. | `GET /control-center/settings/status`, plus related status from `/control-center/status`, `/control-center/runtime-readiness/summary`, `/control-center/local-models/status`, `/control-center/sources/readiness`, `/api/manifest`, and `scripts/inspect_settings_authority_posture.py`. | Loopback auth-token setup/status route, reviewed local llama.cpp settings route, local model identity/alias status route, local model lifecycle safe-disable route, provider configuration route, kill-switch execution route, and any mutation route. | Settings must remain disabled-by-default, redacted, local-only, revocable, and policy-gated. Feature-flag, kill-switch, permission-mode, model identity, lifecycle, provider, connector, web, memory-context, and platform names are posture vocabulary only until separate scoped authority exists. No toggles that grant authority, provider configuration, installer behavior, runtime activation, feature-flag writes, kill-switch execution, revocation execution, connector runtime, connector writes, live web, model calls, provider SDK calls, shell/browser execution, public beta, production readiness claims, or production authority is added. | `GET /control-center/settings/status` is `validation_only`; `/settings` owns presentation state only and renders backend-owned posture rows. | No approval is required for read-only Settings status. Exact approval is required for any setting that enables runtime authority, persistence, local lifecycle behavior, model identity mutation, provider configuration, kill-switch execution, revocation execution, or other mutation. | FCC-P1-011 spec refs in `docs/control_center/SETTINGS_KILL_SWITCH_FEATURE_FLAGS_SPEC.md`, Product Loop 011 docs/verifier/CLI refs, operational maturity manifest/verifier refs, route-status refs, blocked authority refs, platform capability snapshot refs, and safe settings proposal refs only; future local model manager receipts must use redacted refs only. | Operational maturity scorecard: Settings is rank 2 proposal review for backend-owned status, blocked/degraded/partial posture labels, and review-only settings proposal refs only. | Mutating feature flags, kill-switch execution, revocation execution, provider configuration, reviewed runtime settings mutation, local model identity mutation, and lifecycle safe-disable execution remain future scoped milestones. |
