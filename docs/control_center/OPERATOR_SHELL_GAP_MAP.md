...
API boundary: current FastAPI manifest has 125 OpenAPI paths
...
9. Continue the FCC-V1 Founder Loop V1 conveyor before promoting the first
   ...
   FCC-V1-001 API perimeter contract/verifier coverage is complete,
   Action Inbox approve/edit/reject/defer backend decisions are complete for
   decision state and receipt refs, and FCC-V1-003 completes the first Today
   item to Action envelope to exact approval/edit/reject/defer receipt to
   Evidence update path; Chat durable receipt and handoff, Memory Review
