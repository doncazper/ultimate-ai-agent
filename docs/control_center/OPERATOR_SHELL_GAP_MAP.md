...
Baseline: v0.102.1 / 0.102.1
Source plan: `docs/roadmap/OPERATOR_RUNTIME_EXCELLENCE_ROADMAP.md` M172
API boundary: current FastAPI manifest has 108 OpenAPI paths
...
|---|---|---|---|---|---|---|---|---|---|
| Setup Assistant | `/setup` shows the macOS-first setup preview using the read-only backend summary when available and the existing mock fallback otherwise. | `GET /control-center/setup-assistant/summary`. | Setup action approval envelope route, rollback status route, native SwiftUI shell. | Existing macOS Setup Assistant contract only; no installer, shell, model download, LaunchAgent, background service, provider/model, credential, receipt persistence, or rollback execution authority. | `validation_only`; `/setup` remains an inspection UI. | Future setup mutations require exact local approval; this summary route grants none. | Dry-run setup plan with bounded previews, receipt refs, audit ref, latency ref, and rollback refs only. | Partial. | Approval envelope, rollback rehearsal, bounded real setup-log source, and native macOS visual QA remain future work. |
