...
|---|---|---|---|---|---|---|---|---|---|
| Today | `/today` renders the storage-backed first-loop summary plus UAA-P1-068 Today Product Spine Contract fields, UAA-P1-069 Evidence History Grammar fields, UAA-P1-073 Plans Action envelope posture, UAA-P1-074 Chat local operator truth, UAA-P1-075 Governed Code Workbench metadata, and UAA-P1-076 Cross-Surface Memory Intake proposal refs: required loop surfaces, required Today signals, module feed rows, necessary-not-sufficient completion posture, plan/action state, stale-source posture, next safe actions, history states/questions, surface bindings, per-timeline history answers, envelope contract refs, review actions, expected receipt refs, rollback/safe-disable refs, Chat turn truth refs, proposal handoff refs, governed Code proposal refs, validation refs, expected apply/rollback receipt refs, memory intake source/provenance/evidence refs, and blocked authority refs. | `GET /control-center/today/summary`, `GET /control-center/storage/status`. | Today action mutation contract, email/calendar read-only contracts, notification contract, memory-to-loop binding. | Founder Loop storage summaries only; loop visibility is necessary but not sufficient for completion; evidence history, Action envelopes, Chat local operator truth, governed Code workbench metadata, and cross-surface memory intake proposal refs are read-only posture; no action execution, apply execution, approval grant capture, connector writes, provider calls, email/calendar reads, notification delivery, rollback execution, approval grant, memory write, context injection, or production authority. | `local_dev_workspace_only`. | None for read-only summary; any mutation requires exact LocalApprovalAuthority scope, idempotency, rollback, audit, and receipt refs. | SQLite state refs, JSONL log refs, safe evidence refs, bounded summaries, blocked states, backup manifest refs, UAA-P1-068, UAA-P1-069, UAA-P1-073, UAA-P1-074, UAA-P1-075, and UAA-P1-076 contract/schema/verifier/test refs, checked-in visual baselines, and local loopback packaging proof refs. | Partial: UAA-P1-068, UAA-P1-069, UAA-P1-073, UAA-P1-074, UAA-P1-075, and UAA-P1-076 contracts are implemented; later memory-to-loop binding remains pending. | Scoped mutation contracts, integration contracts, notification contracts, memory write policy binding, memory-to-loop binding, and full API/Foundation Gate extraction remain future work. |
...
| Memory Review | `/memory` renders the storage-backed Founder Loop memory review queue with candidate provenance/source/evidence refs, review state, UAA-P1-071 review-only decision metadata, UAA-P1-072 business-memory candidate/quality metadata, UAA-P1-076 cross-surface intake proposals, correction/rejection posture, retention/delete/export posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels. FCC-P1-010 adds a contract-only relationship/follow-up candidate schema for future review workflows. | `GET /control-center/today/summary`. | Memory write policy binding, retention/delete/export execution contract, external CRM write/account-sync contract, context-injection contract, memory-to-loop binding. | Review-only memory candidates, review-only decision metadata, review-only business quality metadata, and review-only intake proposals; no automatic memory writes, context injection, model/provider authority, connector writes, raw transcript/prompt/source display, background sync, memory delete/export execution, external CRM writes, account sync, or production authority. | `local_dev_workspace_only`. | None for read-only review queue inspection. Accept, correct, reject, retain, delete, export, write, CRM sync, context-injection, quality-control actions, and loop-binding actions require later exact scoped memory contracts. | Memory candidate refs, safe summaries, provenance refs, source refs, evidence refs, review state, UAA-P1-071 decision states/actor/audit/receipt/blocked refs, UAA-P1-072 candidate kinds/quality states/surface refs, UAA-P1-076 intake proposal/source/provenance/evidence refs, correction/rejection posture, retention/delete/export posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels only. | Partial: UAA-P1-071 decision metadata, UAA-P1-072 business quality metadata, and UAA-P1-076 intake proposal contracts are implemented; memory mutation and loop-binding policies remain missing. | Write policy binding, retention/delete/export semantics, external CRM write/account sync, context-injection policy, memory-to-loop binding, CLI inspection path, and durable receipt binding remain future work. |
