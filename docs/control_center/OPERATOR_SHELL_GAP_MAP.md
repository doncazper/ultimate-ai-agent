...
API boundary: current FastAPI manifest has 117 OpenAPI paths
...
| Action Inbox | `/actions` renders review-ready action proposal summaries from local Founder Loop storage with UAA-P1-073 Action envelope contract posture, backend-owned decision controls, exact scope refs, side-effect/risk/approval posture, idempotency, expiry, expected receipt refs, rollback/safe-disable posture, blocked authority refs, receipt/audit refs when available, and next safe action labels. | `GET /control-center/actions/inbox`; `POST /control-center/actions/{action_id}/approve`; `POST /control-center/actions/{action_id}/edit`; `POST /control-center/actions/{action_id}/reject`; `POST /control-center/actions/{action_id}/defer`; `GET /control-center/actions/{action_id}/receipt`. | Today-to-action envelope creation contract, action execution contract, Evidence Timeline write binding. | Backend decision state only; approve/edit/reject/defer produce local receipt refs but do not send, run, install, dispatch, write memory, perform connector writes, execute shell/subprocess work, call model/provider runtime, or grant production authority. | `local_dev_workspace_only`. | Read-only inbox and receipt inspection need no approval. Approve validates exact LocalApprovalAuthority scope; edit, reject, and defer require idempotency, audit, and receipt refs. Approval refs remain identifiers until exact scope is validated. | Action proposal refs, UAA-P1-073 Action envelope contract refs, side-effect class, risk class, blocked-state labels, backend decision receipt refs, receipt/audit/idempotency/rollback refs, expected receipt refs, and evidence refs only. | Partial: reviewable Action envelope metadata and backend-owned decision state are implemented; action execution and first full loop remain pending. | Today-to-action creation, action execution, Evidence Timeline mutation binding, CLI inspection path, and first vertical-loop proof remain future work. |
...
9. Continue the FCC-V1 Founder Loop V1 conveyor before promoting the first
   ...
   decision state and receipt refs; Today item to Action envelope to exact
   approval to durable receipt to Evidence update,
