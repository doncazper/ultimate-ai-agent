...
| Memory Review | `/memory` renders the storage-backed Founder Loop memory review queue with candidate provenance/source/evidence refs, review state, correction/rejection posture, retention/delete posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels. | `GET /control-center/today/summary`. | Memory review decision capture contract, memory write policy binding, retention/delete contract, context-injection contract. | Review-only memory candidates; no automatic memory writes, context injection, model/provider authority, connector writes, raw transcript/prompt/source display, background sync, memory delete execution, or production authority. | `local_dev_workspace_only`. | None for read-only review queue inspection. Accept, correct, reject, retain, delete, write, and context-injection decisions require later exact scoped memory contracts. | Memory candidate refs, safe summaries, provenance refs, source refs, evidence refs, review state, correction/rejection posture, retention/delete posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels only. | Partial. | Decision capture, write policy binding, retention/delete semantics, context-injection policy, CLI inspection path, and durable receipt binding remain future work. |
