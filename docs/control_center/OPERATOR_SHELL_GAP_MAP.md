...
| Memory Review | `/memory` renders the storage-backed Founder Loop memory review queue with candidate provenance/source/evidence refs, review state, UAA-P1-071 review-only decision metadata, correction/rejection posture, retention/delete posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels. FCC-P1-010 adds a contract-only relationship/follow-up candidate schema for future review workflows. | `GET /control-center/today/summary`. | Business memory quality controls, memory write policy binding, retention/delete/export execution contract, context-injection contract. | Review-only memory candidates and review-only decision metadata; no automatic memory writes, context injection, model/provider authority, connector writes, raw transcript/prompt/source display, background sync, memory delete/export execution, or production authority. | `local_dev_workspace_only`. | None for read-only review queue inspection. Accept, correct, reject, retain, delete, export, write, context-injection, and quality-control actions require later exact scoped memory contracts. | Memory candidate refs, safe summaries, provenance refs, source refs, evidence refs, review state, UAA-P1-071 decision states/actor/audit/receipt/blocked refs, correction/rejection posture, retention/delete/export posture, confidence posture, stale-state posture, authority boundary, blocked states, and next safe action labels only. | Partial: UAA-P1-071 decision metadata contract is implemented; business memory quality controls and mutation policies remain missing. | Business memory quality controls, write policy binding, retention/delete/export semantics, context-injection policy, CLI inspection path, and durable receipt binding remain future work. |
