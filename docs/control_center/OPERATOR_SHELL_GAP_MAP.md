...
| Inbox | `/inbox` renders a blocked/planned Founder Command Center communication triage posture plus the dedicated backend-owned Source Readiness read model for inbox, calendar, tasks, CRM-lite/manual notes, repo, and local-file lanes using explicit `ready`, `blocked`, `metadata_only`, and `not_configured` labels. The Inbox page still does not bind to live email, calendar, account, draft UI, polling, or connector runtime behavior. | `GET /control-center/sources/readiness`, with the same readiness embedded in `GET /control-center/today/summary` and `GET /control-center/morning-briefing/summary`. | Draft-only proposal UI binding, connector account/read-metadata contracts, and any live source ingestion contract. | Read-only source readiness posture only; no connector runtime, connector reads/writes, account auth, credential handling, message send/archive/delete/label/move, raw message/calendar metadata display, memory writes, context injection, model/provider calls, background fetch, notification delivery, or production authority. FCC-P1-007, FCC-P1-008, and FCC-P1-009 are contract-only Python-core metadata/proposal envelopes. | Dedicated source readiness is `local_dev_workspace_only` read-model data; `/inbox` remains a shell over backend-owned status. Future connector metadata routes must declare read-only/validation-only side-effect classes before UI controls appear. | No approval for read-only source-readiness inspection. Future triage, draft proposal, or connector actions require exact scope, LocalApprovalAuthority boundary, idempotency, rollback/safe-disable posture, audit, and receipt refs. | Source-readiness safe refs, missing contract refs, blocked-state refs, blocked authority refs, source status labels, route refs, product spec and gap-map refs, plus contract-only calendar/email/draft proposal test evidence; no connector receipts, account proofs, raw source evidence, sent draft, or completion evidence. | Rank 1 dedicated read-only readiness route and UI binding. No rank 2 proposal-review promotion is claimed until draft-only proposal binding and verifier-backed contracts justify it. | Draft UI binding, connector/account metadata contracts, and frontend tests proving no send/write authority remain future scoped. |
