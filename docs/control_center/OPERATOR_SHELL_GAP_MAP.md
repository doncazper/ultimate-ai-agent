...
| Memory Review | `/memory` renders the storage-backed Founder Loop memory review queue with candidate provenance/source/evidence refs, UAA-P1-071 review-only decision metadata, UAA-P1-072 business-memory candidate/quality metadata, UAA-P1-076 cross-surface intake proposals, UAA-P1-077 memory-to-loop refs, FCC-V1-005 backend-owned accept/correct/reject decision receipts, the Phase 2 L1 hot local memory index derived preview, the Phase 3 L2 factual/graph/temporal read-only ref projection, the Phase 4 L3 identity/session/preference/commitment representation proposal inspection, the Phase 5 context-pack proposal inspection, and the Phase 6.1 internal Action proposal hook. FCC-P1-010 adds a contract-only relationship/follow-up candidate schema for future review workflows. | `GET /control-center/today/summary`, `GET /control-center/memory/review`, `GET /control-center/memory/l1-index`, `GET /control-center/memory/l2-index`, `GET /control-center/memory/l3-index`, `GET /control-center/memory/context-packs`, `POST /control-center/memory/context-packs/{context_pack_ref}/action-proposal`, `GET /control-center/memory/review/{candidate_ref}/receipt`, `GET /control-center/evidence/timeline`, `POST /control-center/memory/review/{candidate_ref}/accept`, `POST /control-center/memory/review/{candidate_ref}/correct`, `POST /control-center/memory/review/{candidate_ref}/reject`. | Memory write policy binding, retention/delete/export execution contract, external CRM write/account-sync contract, connector write contract, context-injection contract, and Phase 6 narrow execution hook contract. | Backend-owned Memory Review decisions record receipt refs; accept/correct create reviewed recall-only `LocalMemoryStore` records with safe refs, correct stores `corrected_summary_ref` only, and reject preserves stale candidates as rejected review state without creating recall records. The L1 index derives read-only recall previews from reviewed records with source/evidence/receipt explanations; the L2 index derives factual/graph/temporal inspection refs from L1 previews only; the L3 index derives review-required representation proposal refs from L2 safe refs only; the context-pack lane derives proposal-only envelopes from reviewed L1/L2/L3 safe refs only; Phase 6.1 records exact-approved internal Action proposal receipts from context-pack refs only. No automatic memory writes, source/memory truth authority, hidden context injection, prompt context writing, model/provider authority, embeddings/vector DB, semantic search, background indexing, context-pack injection, connector writes, raw transcript/prompt/source display, background sync, memory delete/export execution, external CRM writes, account sync, action execution, Phase 6 execution hooks, public beta, or production authority. | `local_dev_workspace_only`. | None for read-only review queue, receipt, L1 preview inspection, L2 ref-projection inspection, L3 representation proposal inspection, Phase 5 context-pack proposal inspection, or Phase 6.1 internal Action proposal receipt inspection. Accept, correct, reject, and context-pack Action proposal creation require idempotency and backend receipt refs; retain, automatic write, delete, export, CRM sync, context-injection, quality-control actions, approval capture, action execution, and loop-binding mutation require later exact scoped memory contracts. | Memory candidate refs, safe summaries, provenance refs, source refs, evidence refs, review state, UAA-P1-071 decision refs, UAA-P1-072 business quality refs, UAA-P1-076 intake refs, UAA-P1-077 loop/action/weekly-review refs, FCC-V1-005 decision receipt refs, L1 match reasons, L1 supporting ref groups, L2 derivation reasons, L2 fact/relation/time refs, L3 derivation reason refs, L3 supporting memory/L1/L2 refs, context-pack inclusion/exclusion reason refs, context-pack evidence answer refs, Phase 6.1 internal Action proposal refs, idempotency refs, replay/conflict posture, Evidence Timeline refs, authority boundary, blocked states, and next safe action labels only. | Proofed route surface: UAA-P1-071, UAA-P1-072, UAA-P1-076, UAA-P1-077, FCC-V1-005 decision receipts, Phase 2 L1 hot local memory index read-only preview, Phase 3 L2 factual/graph/temporal deterministic read-only projection, Phase 4 L3 representation proposal inspection, Phase 5 context-pack proposal inspection, Phase 6.1 internal Action proposal receipts, and FCC-V1-007 proof evidence are implemented for the exact `/memory` behavior; memory write/context policies remain missing. | Write policy binding, retention/delete/export semantics, external CRM write/account sync, connector writes, context-injection policy, broad Phase 6 execution hooks, and broader CLI inspection polish remain future work. |
