...
| No planned unsupported authority overclaim | Trust and AuthorityState copy may describe issue sync, connector write/send, browser low-risk action, scoped background work, and production deploy as planned known AuthorityLease capabilities only when the decision catalog reports unsupported adapter refs and deny outcomes. | Any copy implying planned unsupported capabilities are approval-ready, executable, callable, issue-ready, live, adapter-backed, or merely waiting for a user toggle is blocked. |
