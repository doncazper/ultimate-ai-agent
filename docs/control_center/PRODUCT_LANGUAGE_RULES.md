...
inspection source of truth for active mode, domain grants, existing legacy
compatibility mappings, policy decisions, receipts, audit refs,
...
- Unsupported adapters are planned unsupported, denied, blocked, or draft-only; never claim live
...
| No planned unsupported authority overclaim | Trust and AuthorityState copy may describe issue sync, connector write/send, browser low-risk action, scoped background work, and production deploy as planned known AuthorityLease capabilities only when the decision catalog reports unsupported adapter refs, deny outcomes, reason refs, and no execution claim. Control Center capability cards must surface mapped AuthorityState decision refs for every Trust capability row. | Any copy implying planned unsupported capabilities are approval-ready, executable, callable, issue-ready, live, adapter-backed, or merely waiting for a user toggle is blocked. |
...
| No runtime lane authority drift | Worktree-per-agent and staged orchestration copy must show the backing AuthorityState mapping/decision refs, active-lease outcome, route/CLI inspection path, and blocked follow-on mutation/execution authority. Worktree lanes may be described as read/proposal/prepare posture only; staged utility command execution may be described only as exact RuntimeGateway-approved `workspace/execute` authority. | Any copy implying worktree create/delete, branch/file mutation, commit, push, arbitrary shell, browser automation, connector write, provider/model call, production authority, or autonomous staged execution from read-model visibility is blocked. |
...
| No provider invocation promotion authority drift | Exact-approved provider capability copy must distinguish Trust matrix `approval_required` posture for known `provider_model_calls/execute` capabilities from broad/default provider runtime. It may say named local loopback, validation, or tiny-provider adapter lanes require active AuthorityLease scope, exact approval, policy validation, CostGovernor decisions, max-approved USD, redacted receipt refs, actual usage/cost refs, receipt completeness, incomplete-cost review, further-use-blocked posture, safe-disable posture, CLI parity, two named single-provider adapter scope refs, and UI blocked/approved/cost-blocked states before any scoped attempt. | Any copy implying provider SDK calls by default, enabled runtime invocation, credential validation, network calls outside exact-scoped adapters, fallback execution from adapter visibility, autonomous model calls, background execution, model output authority, raw prompt/response/provider payload persistence, broad provider-enabled toggles, billing authority, actual-cost bypass, incomplete-cost bypass, or callable provider runtime from the capability is blocked. |
...
| No web-hybrid authority drift | Web-hybrid copy may say three exact read-only lanes and one self-host-first, single-eligible-fallback router are implemented behind WebAccessGateway. It must show current runtime observation as required/unknown on read-only surfaces, external content as untrusted, and exact request-scoped policy, approval, AuthorityLease, safe-disable, idempotency, audit, redaction, plus cloud budget/reservation requirements. | Any copy implying global authorization, current provider health or credits from the read model, paid use, Keyless, cloud-first or multi-step fallback, zero-data-retention provider guarantees, browser actions, raw content/payload persistence, memory/context injection, public release, or production authority is blocked. |
