...
- `local-runtime` may describe configured loopback/local runtime candidates
  ...
  receipts. Phase 05 permits focused pytest and repo-verifier command execution
  only after an exact validated `operator-approved` Action Inbox approval
  envelope; frontend-check and arbitrary commands remain blocked. Phase 06 may
