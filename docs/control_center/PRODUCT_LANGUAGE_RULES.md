...
| No File Review authority drift | File Review copy may say `Record review-only approval capture` only when it names `POST /files/review/approvals/capture`, active `files/write` AuthorityLease scope, safe refs only, idempotency, authority decision refs, receipt refs, and rollback/safe-disable posture. | Any File Review copy implying raw file access, full content read, patch apply, rollback execution, context injection, memory write, export, shell execution, browser/app control, public beta/release, production readiness, or production authority from approval capture is blocked. |
