...
| No memory-derived execution claims | Memory review, L1/L2/L3 indexes, context packs, and Phase 6 execution-hook contracts must be described as recall, inspection, proposal, or blocked proof surfaces only. Phase 6.1 copy may say active `memory/draft` AuthorityLease scope plus exact approval/idempotency records one internal Action proposal receipt from reviewed context-pack refs. | Any copy implying memory/context packs execute actions, inject runtime prompt context, write memory from the proposal lane, or that broad Phase 6 hooks are shipped/available is blocked. |
