...
| `/inbox` Source Inbox | FCC | Source readiness cards and blocked draft/input posture | `GET /control-center/sources/readiness` | partially wired | Readiness is backend-owned; email/calendar read contracts and draft-only response contracts are missing. | Add read-only email/calendar readiness contracts and CLI inspection before richer inbox cards. |
...
| `/actions` Action Inbox | FCC | Action rows, approve/edit/reject/defer receipts, local-task commit lane | `GET /control-center/actions/inbox`; decision and receipt routes under `/control-center/actions/{action_id}`; CLI: `scripts/inspect_action_inbox_decision_lanes.py`, `scripts/dev/uaa_founder_loop.py` | wired with authority limits | Backend owns state and receipts; UI decisions require existing exact-scoped backend contracts and still do not execute external actions. | Keep adding receipt visibility tests; do not add action execution without a separate scoped milestone. |
