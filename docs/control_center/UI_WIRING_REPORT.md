...
|---|---|---|---|---|---|---|
| `/today` Today | FCC | Operator-focused briefing, attention, selected detail, Day Plan, explicit unavailable News/Business pulse, Today-to-Action envelope proposal, exact metadata-action status | `GET /control-center/today/summary`; `POST /control-center/today/action-envelope`; `/control-center/today/exact-action/*`; CLI: `scripts/inspect_today_loop.py`, `scripts/dev/uaa_founder_loop.py exact-action-status` | backend read workflow implemented; exact-action UI control pending | The workspace Today composition now renders backend summaries with operator language and keeps only item selection in React presentation state. The exact predeclared metadata target still completes through MissionOrchestrator, MissionRunner, and AuthorityDispatcher only after explicit review of fixed safe source refs, exact approval, and current mission-lease checks. | Keep Add-to-Day-Plan and Ask UAA unavailable until exact contracts exist; never model approval or execution in React state. |
| `/inbox` Source Inbox | FCC | Source readiness cards, email/calendar read-only metadata contract refs, connector draft-only proposal refs | `GET /control-center/sources/readiness`; CLI: `scripts/inspect_source_readiness_metadata_contracts.py`, `scripts/inspect_connector_draft_proposals.py` | partially wired | Readiness is backend-owned and now includes email/calendar metadata contract refs plus email-response and calendar-hold draft proposal refs; live email/calendar reads, account auth, source ingestion, connector runtime, sends, and writes remain blocked. | Add test-account metadata proof and exact live metadata-read lane before richer inbox cards or any send/write controls. |
...
| `/actions` Action Inbox | FCC | Action rows, backend-linked Plan -> Action -> Decision -> Receipt review path, readable blocked authority, approve/edit/reject/defer receipts, local-task commit lane, queue scope/idempotency/staleness posture | `GET /control-center/actions/inbox`; decision and receipt routes under `/control-center/actions/{action_id}`; CLI: `scripts/inspect_plans_to_actions_bridge.py`, `scripts/inspect_action_inbox_decision_lanes.py`, `scripts/dev/uaa_founder_loop.py` | wired with authority limits | Backend owns plan links, decision lanes, state, receipts, and review refs. The review path appears only when exact envelope, scope, expected-receipt, safe-ref, and denied-authority posture agree; missing or authority-claiming links fail closed. UI decisions retain their existing exact-scoped eligibility checks and still do not execute external actions. | Keep adding receipt visibility tests; do not add action execution without a separate scoped milestone. |
...
| `/setup` Setup | Control Center support | Readable macOS Setup readiness plus collapsed technical proof | `GET /control-center/setup-assistant/summary` with backend-truth response binding | wired read-only, route-specific | Setup no longer waits for unrelated Control Center reads. Backend-owned ready/missing/blocked counts, one attention item, and one next safe action appear first; full lifecycle and rollback proof remains inspectable, and invalid/fallback-derived data fails closed. | Keep setup mutation blocked; define and rehearse an exact rollback lane before enabling any setup action. |
...
| `/files` Files | Control Center support | File refs/evidence viewer | AuthorityLease-gated preview/proposal POST routes exist; UI uses M17 mock bundle | mock-only for visible list | There is no safe GET read model for file ref inventory visible to this panel; POST previews require `files/read` or `files/prepare` and still omit raw paths/content. | Add a redacted file-ref inventory route before replacing mock data. |
| `/files/review` File Review | Control Center support | File review packets and receipt plan posture | `POST /files/review/approvals/capture` exists as an active `files/write` AuthorityLease-gated safe-ref capture route; UI uses M36 mock bundle | blocked / mock-only for visible packets | Existing backend capture route is mutating and authority-scoped; it is not a read model for visible packet state and does not grant raw file or patch authority. | Add `GET /control-center/files/review/summary` before wiring the panel to backend packet/receipt state. |
...
| `/runtime` Runtime | Control Center support | Runtime readiness, capability matrix, and authority-scoped staged orchestration posture | `GET /runtime/readiness`; `GET /runtime/capability-matrix`; `GET /control-center/runtime-readiness/summary`; `GET /api/runtime/staged-orchestration` | wired read-only | Runtime flags and staged orchestration are backend-owned inspection state only; no runtime activation or Control Center authority minting is exposed. | Keep direct summary, matrix, staged orchestration, and AuthorityState tests aligned. |
...
- Test-account metadata proof for future live calendar/email read-only lanes.
- Briefing refresh and notification contracts.
