...
| `/setup` Setup | Control Center support | macOS Setup Assistant summary | `GET /control-center/setup-assistant/summary` | wired read-only | Backend returns ready/missing/blocked diagnostics plus explicit rollback contract, execution, rehearsal, and restore-proof posture; installer side effects and credential capture remain blocked. | Define and rehearse an exact rollback lane before enabling any setup mutation. |
