...
|---|---|---|---|---|---|---|
| `/today` Today | FCC | Daily loop command deck, Today-to-Action envelope proposal, exact metadata-action status | `GET /control-center/today/summary`; `POST /control-center/today/action-envelope`; `/control-center/today/exact-action/*`; CLI: `scripts/inspect_today_loop.py`, `scripts/dev/uaa_founder_loop.py exact-action-status` | backend workflow implemented; UI control pending | The exact predeclared metadata target completes through MissionOrchestrator, MissionRunner, and AuthorityDispatcher only after explicit review of its fixed safe source refs, exact approval, and current mission-lease checks on each durable workflow mutation. Terminal UI truth is derived from completion and dispatch ledgers, not React or Action projection state. The UI must remain status-only until its control is wired to the backend contract. | Add the exact-action control through the control-registry pass; never model approval or execution in React state. |
