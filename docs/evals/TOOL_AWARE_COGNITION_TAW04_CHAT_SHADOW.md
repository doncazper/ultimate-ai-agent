...
The output binds the exact familiarity and optional TAW-03 hydration
fingerprints, legacy route, safe-disable ref, reason refs, selected operation
refs, material-effect refs, clarification posture, and a canonical decision
fingerprint. Public boundaries revalidate copied Pydantic instances so a
mutated copy cannot bypass route or fingerprint checks.
...
`ambiguous` assessment also carries at least two distinct material-effect refs.
Same-effect or otherwise non-material ambiguity does not interrupt direct chat.
Shadow recommendations never render a question or change the current response.
...
hydration is supplied, only proposal-eligible manifests bound to exact TAW-02
candidate refs can be recorded, and none is inserted into a prompt in shadow
mode.
...
schema-limited rendering-path ref for aliases, availability, description,
effect metadata, examples, input/output schemas, operation metadata,
preconditions, provenance/review metadata, risk/approval metadata, rollback,
and terminal-proof metadata. Every case is explicitly non-model-visible in
shadow mode.
...
copied-instance route substitution, shared inspection projection, and the
complete predeclared injection-field inventory.
