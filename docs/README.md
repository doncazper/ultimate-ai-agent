...
v0.31.1 normalizes the GitHub README polish commit into a clean docs-only
baseline after v0.31.0. v0.31.0 implements M27 Tool Broker v2 + Safe Tool
Intent Contracts as validation-only and preview-only contract logic. v0.31.1
adds no backend execution routes, frontend features, real tool execution, file
mutation, memory writes, network calls, model/provider calls, plugin
enablement, browser automation, context injection runtime, dependencies, M28
work, or production authority. M28-M40 remain planned/provisional.
