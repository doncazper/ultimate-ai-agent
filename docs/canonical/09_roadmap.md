...
v0.26.0 implements M22 Local Model Runtime Activation Contract as contract/planning/validation only. It adds metadata-only local runtime activation contracts, planned-disabled provider profiles, loopback/relative metadata endpoint policy validation, health probe plan validation, tests, static verifier coverage, Foundation Gate criteria, and docs. No model was called, no runtime was activated, and no endpoint was contacted. OpenAPI path count remains `74`. v0.26.0 adds no backend API route, runtime execution, local LLM call, model/provider call, endpoint probe, user prompt processing, tool execution, memory write, file write, OpenWebUI runtime behavior, dependency, or production authority. M23 remains planned/provisional.
