...
The active accepted baseline is v0.77.0. v0.77.0 implements M73 Browser Automation Contract Review as contract-only, review-only, disabled-by-default deterministic review over future browser automation contracts. It treats observe-only browser adapter work as an M74 candidate only, revalidates safety-critical fields at evaluator boundaries, and adds no browser automation, browser observe, browser navigation, browser click, form fill, screenshot, raw DOM, authenticated browser profile, cookies or credentials, download or upload, remote browser, network interception, network call, model call, tool execution, memory write, context injection, backend route, Control Center control, dependency, M74 work, or production authority. v0.76.0 implements M72 Read-Only HTTP Fetch Tool, Allowlisted as a bounded allowlisted read-only HTTP fetch tool through the governed tool runtime adapter. It requires explicit host allowlists, HTTPS-only and GET-only validation, no credentials or cookies, no request body, no request headers, no query strings, bounded response bytes, redaction before return, bounded redacted preview outputs, safe receipt metadata, evaluator revalidation, documentation-integrity checks, static verification, and Foundation Gate coverage. It adds no unrestricted network tool, authenticated network action, raw response body, raw headers, download or export, context injection, memory write, model call, browser automation, backend route, Control Center control, dependency, M73 work, or production authority. v0.75.0 implements M71 Network Tool Contract Review as contract-only, review-only, disabled-by-default, deterministic review over future network tool contracts. It marks allowlisted read-only HTTP fetch as an M72 candidate only, revalidates safety-critical fields at evaluator boundaries, and adds no network call, HTTP fetch, unrestricted network tool, authenticated network action, credentials or cookies, request body, non-GET method, download or export, raw response body, backend route, Control Center control, dependency, M72 work, or production authority. v0.74.0 implements M70 Autonomy Foundation Freeze as contract-only, review-only, freeze-only, deterministic validation over the accepted M61-M69 autonomy foundation. It requires accepted milestone refs, explicit checklist refs, evaluator revalidation, and adds no policy activation, session start, low-risk dry-run execution, autonomous actions, background worker, execution, backend route, Control Center control, dependency, or production authority. v0.73.0 implements M69 Low-Risk Autonomous Dry Run as contract-only, review-only, dry-run-only, deterministic low-risk records exact-bound to M68 Autonomy Risk Classifier decisions. It enforces a low risk ceiling; denies higher-risk M68 decisions and higher-risk dry-run steps; keeps approval refs as identifiers only; and adds no policy activation, session start, autonomous actions, background worker, execution, backend route, Control Center control, dependency, or production authority. v0.72.0 implements M68 Autonomy Risk Classifier as contract-only, review-only, deterministic validation over M66 scoped approval bundles and M67 Revocation + Kill Switch records. It derives the highest risk from declared risk, scoped approval bundle risk, and explicit risk signals; denies risk downgrade; keeps approval refs as identifiers only; and adds no policy activation, session start, autonomous actions, background worker, execution, backend route, Control Center control, dependency, M69 work, or production authority. v0.71.0 implements M67 Revocation + Kill Switch as contract-only, review-only, deterministic records over exact M66 scoped approval bundles. Revocation requested and kill-switch requested are review states only; approval refs remain identifiers only and grant no authority. M67 adds no revocation action, kill-switch activation, session stop, process kill, execution, backend route, Control Center control, dependency, M68 work, or production authority. v0.70.0 implements M66 Scoped Approval Bundles as contract-only, review-only, deterministic, exact-scope, actor-bound, resource-bound, capability-bound, allowlist-bound, non-transferable, revocable, and replay-safe approval bundle records. Approval refs remain identifiers only and grant no authority. v0.69.0 implements M65 Autonomy Audit + Replay Viewer as contract-only, review-only, replay-view-only, and deterministic inspection over exact M64 simulation results with exact simulation result refs, simulation request refs, policy decision refs, and replay step refs, and no execution or authority. v0.68.0 implements M64 Autonomous Plan Simulator as contract-only, review-only, dry-run-only, and deterministic simulation over M63 policy decisions with acyclic dependency graph validation, stable reason codes, and no execution or authority. v0.67.0 implements M63 Autonomy Policy Engine v1 as contract-only and review-only policy evaluation over M62 scoped autonomy session records with actor-bound, resource-bound, capability-bound, allowlist-bound, risk ceiling, duration ceiling, revocation, and audit/replay requirements. v0.66.0 implements M62 Scoped Autonomy Session Contracts as contract-only review records over actor-bound, resource-bound, duration-bound, allowlist-bound, revocable, auditable, and replayable scope refs. v0.65.0 implements M61 Autonomy Mode Charter + Authority Levels as contract-only planning and validation work. It defines Mode 0 through Mode 6, keeps default mode off, adds capability-toggle registry contracts, documents consent/revocation and audit/replay requirements, promotes the active M61-M100 roadmap, and adds tests, documentation-integrity checks, static verification, and Foundation Gate coverage. It adds no global autonomy switch, production authority, execution, backend route, Control Center control, background worker, autonomous session, dependency, M62 work, or production authority. v0.64.1 hardens the accepted M60 Local Developer Beta Freeze baseline after post-M60 review. v0.64.0 implements M60 Local Developer Beta Freeze as deterministic local freeze-only contracts over explicit safe checklist refs. v0.63.0 implements M59 Public GitHub Readiness as deterministic local review-only contracts over explicit safe checklist refs. It adds no GitHub push, release automation, wiki automation, artifact upload, external service, credential handling, network access, backend route, Control Center control, dependency, production authority, or M60 implementation. v0.62.0 implements M58 Dry-Run Execution Audit Harness as deterministic local dry-run-only contracts over explicitly provided safe intent refs. It adds no real execution, tool execution, subprocess execution, shell execution, process spawn, file mutation, network access, model/provider call, memory write, context injection, browser automation, plugin execution, remote execution, backend route, Control Center control, dependency, production authority, or M59 implementation. v0.61.0 implements M57 Runtime Sandbox Architecture Review as deterministic local architecture-review-only contracts over explicitly provided safe sandbox refs. v0.60.0 implements M56 Agent Eval Regression Harness as deterministic local contract-only regression reporting over explicitly provided safe observations. It adds no model/provider call, tool execution, shell execution, browser automation, network access, memory write, context injection, raw prompt capture, raw provider payload capture, external dataset fetch, backend route, Control Center control, dependency, production authority, or M57 implementation. v0.59.0 implements M55 Redacted Observability Export as deterministic local redacted-export contract generation over explicitly provided safe event refs. It adds no external SaaS or analytics SDK, network delivery, raw prompt export, raw provider payload export, raw private content export, secret export, forensic trace export, model/provider call, memory write, context injection, backend route, Control Center control, dependency, production authority, or M56 implementation. v0.58.0 implements M54 Safe Media Metadata Inspector as deterministic local metadata-only contracts for declared safe media refs. It adds no raw media export, raw media storage, full-file read, file mutation, original overwrite, OCIO transform, AI gamut expansion, model/provider call, context injection, memory write, backend route, Control Center control, dependency, production authority, or M55 implementation. v0.57.0 implements M53 Controlled Tool Expansion Review as deterministic local review-only planning for future tool capability categories. v0.56.0 implements M52 OpenWebUI Safe Conversation Surface as a deterministic local safe-summary conversation surface for future OpenWebUI shell review. v0.55.0 implements M51 OpenWebUI Bridge Adapter Pilot as a deterministic local safe-summary adapter boundary for future OpenWebUI shell integration. v0.54.0 implements M50 Mobile Approval Audit Hardening as deterministic, review-only, safe-ref-only audit reports over M49 mobile review approval records. v0.53.0 implements M49 Mobile Review Approval Capture as exact-scope, actor-bound, resource-bound, replay-safe, revocable, review-only mobile approval capture contracts with safe-ref-only persistence. v0.52.0 implements M48 First Internal TestFlight Build as an internal-only, review-only build candidate record. v0.51.0 implements M47 TestFlight Pipeline, Internal Only as internal-only, contract/checklist-only pipeline planning work. v0.50.0 implements M46 iOS Review/Receipt Read-Only Surfaces as source-only, read-only CCC iOS display contracts and Swift surface data for mock non-authoritative redacted review/receipt summaries. v0.49.0 implements M45 CCC iOS Local Read-Only Connection as local-only, loopback-only, read-only connection contracts and source-only CCC iOS status display. v0.48.1 hardens M44 CCC iOS Skeleton, No Authority by repairing verifier policy for the reviewed source-only `apps/ccc-ios/` skeleton while keeping native build, signing, store, sensor, permission, runtime, and authority artifacts blocked. v0.48.0 implements M44 CCC iOS Skeleton, No Authority as source-only, mock-only, read-only, non-authoritative SwiftUI skeleton work. It adds static source under `apps/ccc-ios/` plus contracts, tests, docs, verifier coverage, and Foundation Gate criteria while adding no Xcode project, Swift package, Info.plist, entitlements, native build workflow, signing/store workflow, backend route, mobile API route runtime, network access, mobile sensor access, OS permission integration, approval capture, approval execution, memory write, context injection, file mutation, export, execution, credential storage, background task, or production authority. v0.47.0 implements M43 Mobile API Boundary, Read-Only as contract-only planning/docs/contracts/verifier work. v0.46.0 implements M42 Mobile Companion Product Contract Refresh as planning/docs/contracts/verifier work only. v0.45.0 implements M41 Local Prototype Safety Freeze as a docs, verifier, and Foundation Gate safety freeze over the local prototype before mobile work resumes. v0.44.0 implements M40 Context Handoff Approval, No Injection as contract-only approval decisions over safe context proposals. v0.43.0 implements M39 CCC Context Proposal Surface as a frontend-only, read-only Control Center surface for M38 safe context proposals. v0.42.0 implements M38 Safe Context Proposal From Approved Review as core contracts, validators, decisions, receipt plans, tests, static verification, documentation-integrity checks, and Foundation Gate coverage. v0.41.0 implements M37 Review Approval Capture, Review-Only Persistence as safe approval and denial capture for exact redacted review packets. v0.40.1 hardens M36 CCC File Review Surface, Review-Only. v0.40.0 implements M36 CCC File Review Surface, Review-Only. v0.39.1 hardens M35 Safe File Review Workflow Contracts exact file/path binding. v0.39.0 implements M35 as contract-only, review-only logic over already-redacted preview results. v0.38.0 implements M34 Broader File Capability Review as planning, architecture review, documentation, verifier, and Foundation Gate work only. M55 is implemented/released. M56 is implemented/released. M57 is implemented/released. M58 is implemented/released. M59 is implemented/released, M60 is implemented/released by v0.64.0 as Local Developer Beta Freeze, M61 is implemented/released by v0.65.0 as Autonomy Mode Charter + Authority Levels, M62 is implemented/released by v0.66.0 as Scoped Autonomy Session Contracts, M63 is implemented/released by v0.67.0 as Autonomy Policy Engine v1, M64 is implemented/released by v0.68.0 as Autonomous Plan Simulator, M65 is implemented/released by v0.69.0 as Autonomy Audit + Replay Viewer, M66 is implemented/released by v0.70.0 as Scoped Approval Bundles, M67 is implemented/released by v0.71.0 as Revocation + Kill Switch, M68 is implemented/released by v0.72.0 as Autonomy Risk Classifier, M69 is implemented/released by v0.73.0 as Low-Risk Autonomous Dry Run, M70 is implemented/released by v0.74.0 as Autonomy Foundation Freeze, M71 is implemented/released by v0.75.0 as Network Tool Contract Review, M72 is implemented/released by v0.76.0 as Read-Only HTTP Fetch Tool, Allowlisted, M73 is implemented/released by v0.77.0 as Browser Automation Contract Review, and M74-M100 remain planned/provisional.
