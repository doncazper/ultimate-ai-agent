...
The active accepted baseline is v1.1.0. v1.1.0 implements M97 Recurring Automation Contracts as disabled-by-default, contract-only recurring automation policy, cadence, request, decision, and receipt-plan models with approval renewal, expiration, stop conditions, audit, revocation, safe refs, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. It adds no recurrence runtime, actual recurring execution, background worker, cron, daemon, scheduler, side effects, shell execution, network access, browser automation, plugin execution, memory write, context injection, backend route, Control Center control, dependency, M98 work, broad autonomy, or production authority. v1.0.0 implements M96 Plugin Execution Sandbox, No External Plugins. M98-M100 remain planned/provisional.
