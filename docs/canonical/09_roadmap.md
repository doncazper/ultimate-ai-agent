...
The active accepted baseline is v0.45.0. v0.45.0 implements M41 Local Prototype Safety Freeze as a docs, verifier, and Foundation Gate safety freeze over the local prototype before mobile work resumes. It keeps the prototype localhost-only, review-only, and mock/non-authoritative, records browser smoke review as local-only, and reasserts no raw file browsing, raw file export, full-file reads, arbitrary caller-selected roots, shell/subprocess, unrestricted network tools, provider/model calls as authority, background workers, mobile sensors, plugin enablement, production authority, unreviewed memory writes, automatic context injection, raw prompt/provider payload exposure, credentials/cookie handling, remote execution, browser automation execution, or approval refs as authority. v0.44.0 implements M40 Context Handoff Approval, No Injection as contract-only approval decisions over safe context proposals, with exact proposal binding, approval_ref and approval_test_ denial, expired/revoked/replayed approval denial, evaluator revalidation, review-only no-injection decisions, tests, static verification, documentation-integrity checks, and Foundation Gate coverage. v0.43.0 implements M39 CCC Context Proposal Surface as a frontend-only, read-only Control Center surface for M38 safe context proposals. v0.42.0 implements M38 Safe Context Proposal From Approved Review as core contracts, validators, decisions, receipt plans, tests, static verification, documentation-integrity checks, and Foundation Gate coverage. v0.41.0 implements M37 Review Approval Capture, Review-Only Persistence as safe approval and denial capture for exact redacted review packets. v0.40.1 hardens M36 CCC File Review Surface, Review-Only. v0.40.0 implements M36 CCC File Review Surface, Review-Only. v0.39.1 hardens M35 Safe File Review Workflow Contracts exact file/path binding. v0.39.0 implements M35 as contract-only, review-only logic over already-redacted preview results. v0.38.0 implements M34 Broader File Capability Review as planning, architecture review, documentation, verifier, and Foundation Gate work only. M42-M60 remain planned/provisional.
