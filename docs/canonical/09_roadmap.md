...
The active accepted baseline is v0.93.0. v0.93.0 implements M89 Emergency Stop + Process Kill Safety as contract-only, review-only, deterministic, local-only contracts over exact M88 Mutating Command Proposal decisions with safe target process refs, safe emergency scope refs, safe reason refs, safe refs only receipt plans, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. It adds no emergency stop execution, process kill, process signal, command execution, subprocess execution, shell execution, process spawn, filesystem mutation, network access, tool execution, browser automation, plugin execution, remote execution, model call, memory write, context injection, background worker, backend route, Control Center control, dependency, M90 work, or production authority. v0.92.0 implements M88 Mutating Command Proposal, No Execution as contract-only, proposal-only, review-only, deterministic, local-only contracts over exact M87 Sandboxed Command Audit Replay decisions with safe mutation scope refs, safe argument refs, safe refs only receipt plans. M90-M100 remain planned/provisional.
