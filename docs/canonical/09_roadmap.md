...
The active accepted product baseline is v1.7.2. Checkpoint M105 is implemented/released as Background Task Contract, No Execution while the product baseline remains v1.7.2. M103 remains implemented/released as Camera/Photos Metadata-Only Contract, with camera and photos off by default. Checkpoint M104 remains implemented/released as Notification Planning, No Push Execution. Already-pushed tags remain immutable historical internal milestone tags. Future M106-M149 conveyor milestones use checkpoint labels instead of product SemVer tags, and M150 is the next product release target v1.0.0-alpha. Beta begins only after the alpha UI and supporting safety/product work are reviewed, accepted, and explicitly promoted by a later roadmap patch. M105 adds no background worker, scheduler, daemon, OS background permission prompt, push trigger, device token handling, external service, raw task payload, backend route, Control Center control, dependency, broad autonomy, mobile background runtime, M106 work, or production authority. M100 remains implemented/released as Mobile Permission Model v1. M101, M102, M103, M104, and M105 are implemented/released, and M106-M150 remain planned/provisional in `docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md`.
