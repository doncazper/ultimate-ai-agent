...
The active accepted baseline is v0.80.0. v0.80.0 implements M76 OpenWebUI Runtime Bridge v1 as deterministic review-only OpenWebUI runtime bridge envelopes over safe refs only, policy validation, request validation, receipt plans, documentation-integrity checks, static verification, and Foundation Gate coverage. It adds no live OpenWebUI connection, OpenWebUI runtime call, OpenWebUI handoff execution, provider call, model call, model authority, tool execution, memory write, context injection, network call, credentials or cookies, raw prompt, raw provider payload, raw content, backend route, Control Center control, dependency, M77 work, or production authority. v0.79.0 implements M75 Browser Action Dry-Run Planner as deterministic reviewable browser action dry-run planning over safe refs only, policy validation, request validation, step validation, receipt plans, documentation-integrity checks, static verification, and Foundation Gate coverage. It adds no browser action execution, browser session start, browser navigation execution, browser click execution, form fill execution, screenshot, raw DOM, authenticated browser profile, cookies or credentials, download or upload, remote browser, network interception, network call, model call, tool execution, memory write, context injection, backend route, Control Center control, dependency, M76 work, or production authority. M77-M100 remain planned/provisional.
