...
The active accepted product baseline is v1.7.2. Checkpoint M104 is implemented/released as Notification Planning, No Push Execution while the product baseline remains v1.7.2. M103 remains implemented/released as Camera/Photos Metadata-Only Contract, with camera and photos off by default. Already-pushed tags remain immutable historical internal milestone tags. Future M105-M149 conveyor milestones use checkpoint labels instead of product SemVer tags, and M150 is the next product release target v1.0.0-alpha. Beta begins only after the alpha UI and supporting safety/product work are reviewed, accepted, and explicitly promoted by a later roadmap patch. M104 adds no push delivery, notification permission prompt, notification scheduling, background task execution, device token handling, external push provider, raw notification body, backend route, Control Center control, dependency, broad autonomy, mobile sensor runtime, M105 work, or production authority. M100 remains implemented/released as Mobile Permission Model v1. M101, M102, M103, and M104 are implemented/released, and M105-M150 remain planned/provisional in `docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md`.
