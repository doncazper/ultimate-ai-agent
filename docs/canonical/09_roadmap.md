...
The active accepted baseline is v0.42.0. v0.42.0 implements M38 Safe Context Proposal From Approved Review as core contracts, validators, decisions, receipt plans, tests, static verification, documentation-integrity checks, and Foundation Gate coverage. It adds safe, non-authoritative, proposal-only context bundles from exact-scope approved redacted file review records, and it adds no context injection, OpenWebUI handoff, model/provider calls, memory writes, raw file reads, export, execution, backend context routes, Control Center context proposal surface, dependencies, M39 work, M40 work, or production authority. v0.41.0 implements M37 Review Approval Capture, Review-Only Persistence as safe approval and denial capture for exact redacted review packets. v0.40.1 hardens M36 CCC File Review Surface, Review-Only. v0.40.0 implements M36 CCC File Review Surface, Review-Only. v0.39.1 hardens M35 Safe File Review Workflow Contracts exact file/path binding. v0.39.0 implements M35 as contract-only, review-only logic over already-redacted preview results. v0.38.0 implements M34 Broader File Capability Review as planning, architecture review, documentation, verifier, and Foundation Gate work only. M39-M60 remain planned/provisional.
