...
The active accepted product baseline is v1.7.2. Checkpoint M106 is implemented/released as Mobile Background Read-Only Status Sync while the product baseline remains v1.7.2. M103 remains implemented/released as Camera/Photos Metadata-Only Contract, with camera and photos off by default. Checkpoint M104 remains implemented/released as Notification Planning, No Push Execution. Checkpoint M105 remains implemented/released as Background Task Contract, No Execution. Already-pushed tags remain immutable historical internal milestone tags. Future M107-M149 conveyor milestones use checkpoint labels instead of product SemVer tags, and M150 is the next product release target v1.0.0-alpha. Beta begins only after the alpha UI and supporting safety/product work are reviewed, accepted, and explicitly promoted by a later roadmap patch. M106 adds no background collection, background execution, background worker, scheduler, daemon, OS background fetch, OS background permission prompt, push trigger, device token handling, external service, network sync, raw status payload, backend route, Control Center control, dependency, broad autonomy, mobile background runtime, M107 work, or production authority. M100 remains implemented/released as Mobile Permission Model v1. M101, M102, M103, M104, M105, and M106 are implemented/released, and M107-M150 remain planned/provisional in `docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md`.
