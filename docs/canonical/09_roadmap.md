...
## Accepted baseline through v0.17.2

The active accepted baseline is v0.17.2. It includes foundation modules through M10.5 plus documentation integrity synchronization, Codex plugin/external tooling governance, M11 runtime readiness/report validation, M12 Control Center backend contract/API foundation, and M13 Web Control Center read-only frontend shell with CI/static/browser-readiness hardening. v0.17.2 adds verification hardening only; it does not add production Control Center authority or execution capability.
