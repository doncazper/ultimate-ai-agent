...
The active accepted baseline is v0.38.2. It repairs M34 current-baseline labels and documentation-integrity coverage after the v0.38.1 Yellow review. Active docs now identify v0.38.2 as the current active baseline while preserving v0.38.0 as the historical M34 Broader File Capability Review implementation release and v0.38.1 as a superseded hardening release. v0.38.2 adds no runtime file capability, backend routes, frontend runtime features, raw file reads, file review workflow implementation, approval capture, context proposal, context injection, memory writes, export, execution, dependency, M35 implementation, mobile/TestFlight implementation, or production authority. M35-M60 remain planned/provisional. v0.38.0 implements M34 Broader File Capability Review as planning, architecture review, documentation, verifier, and Foundation Gate work only. v0.37.3 repaired active roadmap label alignment after v0.37.2. v0.37.2 adds local developer launcher tooling for prototype testing after v0.37.1 hardened M33. The launcher provides `uaa doctor`, `uaa start`, `uaa ui`, `uaa status`, `uaa logs`, `uaa stop`, and `uaa restart`, plus a macOS `.command` launcher generator. It starts only the existing localhost FastAPI backend and CCC Web dev server, stores ignored PID/log state under `.uaa/dev/`, and adds no production installer, backend routes, execution authority, dependency, M34 work, or production authority. v0.37.1 hardens M33 First Safe Local File Read Proposal, Redacted Preview Only after v0.37.0 implemented M33, v0.36.1 hardened M32 Filesystem Metadata Path Safety, and v0.36.0 implemented M32 Safe Local Filesystem Metadata Tool. M33 extends the governed Tool Runtime Adapter allowlist to exactly `tool:no_op.v1`, `tool:filesystem_metadata.v1`, and `tool:filesystem.redacted_preview.v1`. The redacted preview tool works only under server-owned safe roots and relative paths, reads only bounded UTF-8 preview bytes inside the redaction-before-return boundary, and returns redacted preview plus redaction summary only. v0.37.1 denies symlink safe roots before preview and rejects secret-like preview text at the output contract boundary. It denies arbitrary caller roots, absolute paths, traversal, encoded traversal, home-directory paths, Windows drive paths, doubled separators, hidden paths, private-key-like paths, secret-like paths, glob paths, symlink following, raw content output, full-file output, content hashes, directory listings, recursive traversal, metadata alias flag attempts, context injection, and filesystem mutation. v0.37.1 adds tests, static verification, documentation, and Foundation Gate coverage. It adds no backend routes, frontend features, arbitrary tool execution, raw file output, full-file reads, content hashes, directory listing, recursive traversal, symlink following, file mutation, shell/subprocess execution, memory writes, Event Ledger mutation, network calls, browser/mobile/remote/plugin actions, model/provider calls, scheduler/background worker, dependency, M34 work, or production authority. v0.36.1 hardens M32 Filesystem Metadata Path Safety. v0.35.1 hardens M31 Real Tool Runtime Adapter, Single Safe No-Op Tool after the v0.35.0 implementation release. v0.35.0 implements M31 Real Tool Runtime Adapter, Single Safe No-Op Tool as a governed local runtime adapter for `tool:no_op.v1` only. v0.34.1 hardens M30 Multi-Step Execution Framework state transitions, ready-only no-effect completion, finalize gating, dependency graph denial, replay-key and transition-id replay protection, hidden side-effect denial, authority-boundary checks, evaluator revalidation, static verification, Foundation Gate coverage, and no-side-effect invariants. v0.34.0 implements M30 Multi-Step Execution Framework as deterministic, local, side-effect-safe, state-machine-only contracts for no-effect run/step advancement, dependency-aware progression, replay protection, evaluator revalidation, transition decisions, and non-authoritative receipt plans. v0.33.1 hardens M29 Agent Task Planning Engine dependency graph validation, duplicate/missing step denial, self/indirect cycle detection, derived risk checks, hidden side-effect denial, authority-boundary checks, evaluator revalidation, static verification, Foundation Gate coverage, and no-execution invariants while preserving deterministic, local, non-executing, review-only planning contracts. v0.33.0 implements M29 Agent Task Planning Engine for task goals, task steps, task plans, dependency graphs, input boundaries, risk/authority checks, decision envelopes, and receipt plans. v0.32.1 hardens M28 Approval Authority v2 + Action Policy Expansion with evaluator-side revalidation for raw/secret action inputs and mutated approval grants while preserving policy-only and decision-only contract logic. v0.32.0 implements M28 Approval Authority v2 + Action Policy Expansion. v0.31.1 normalizes the GitHub README polish commit into a clean docs-only release after v0.31.0. v0.31.0 implements M27 Tool Broker v2 + Safe Tool Intent Contracts as validation-only and preview-only contract logic. v0.30.1 hardens M26 Grounded Recall Router + Evidence-Linked Context Pack Builder source identity checks after the v0.30.0 implementation release. v0.30.0 implements M26 as deterministic local contracts over provided safe candidates only. v0.29.5 remains the documentation organization policy polish baseline after v0.29.4. v0.29.4 repairs documentation archive references and self-maintaining documentation policy coverage after the v0.29.3 archive cleanup review found stale legacy verifier references. v0.29.3 remains a historical documentation organization release. v0.29.2 remains the local-dev API authority and raw preview safety hardening release. v0.29.0 remains the M25 implementation release, and v0.29.1 remains its unknown/arbitrary truth ref hardening patch. v0.28.0 remains the M24 implementation release, v0.28.1 repairs its public memory contract and safety/docs boundary, and v0.28.2 removes a duplicate/conflicting planned/provisional v0.28.1 roadmap row only. v0.27.0 remains the M23 implementation release, and v0.27.1 remains its safety hardening patch. v0.26.0 remains the M22 contract/planning/validation release, and v0.26.1 remains its safety hardening patch. v0.25.0 / M21 remains implemented/released as OpenWebUI Bridge + Chat Shell Integration Contract only, and v0.25.1 remains its safety hardening patch. v0.24.0 / M20 remains implemented/released as Device Capability Broker Contract only, and v0.24.1 remains its safety hardening patch. v0.23.0 / M19 is implemented/released as contract/API planning only. M35-M60 remain planned/provisional.
