...
The active accepted baseline is v0.94.0. v0.94.0 implements M90 Shell/Subprocess Hardening Freeze as contract-only, review-only, freeze-only, deterministic, local-only contracts over exact M89 Emergency Stop + Process Kill Safety decisions with safe hardening refs, safe refs only receipt plans, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. It adds no command execution, shell execution, subprocess execution, process spawn, emergency stop execution, process kill, process signal, filesystem mutation, network access, tool execution, browser automation, plugin execution, remote execution, model call, memory write, context injection, background worker, backend route, Control Center control, dependency, M91 work, or production authority. v0.93.0 implements M89 Emergency Stop + Process Kill Safety as contract-only, review-only, deterministic, local-only contracts over exact M88 Mutating Command Proposal decisions with safe target process refs, safe emergency scope refs, safe reason refs, safe refs only receipt plans. M91-M100 remain planned/provisional.
