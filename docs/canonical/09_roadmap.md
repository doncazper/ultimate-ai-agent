...
The active accepted baseline is v0.99.0. v0.99.0 implements M95 Network Tool Expansion, Authless Only as exact-scope, deterministic, contract-only authless read-only network expansion over allowlisted domain HTTPS GET only proposals. It requires exact approval binding, redirect controls, bounded output, redaction, audit, revocation, transport injection, safe refs only decisions, redacted preview only receipt plans, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. It adds no unrestricted network access, authenticated network access, credentials, cookies, credential headers, request body, POST, PUT, PATCH, DELETE, account action, private network, download, export, browser form, provider model call, shell execution, plugin execution, memory write, context injection, backend route, Control Center control, dependency, M96 work, broad autonomy, or production authority. v0.98.0 implements M94 Autonomous Browser Clicks, Low-Risk Only. M96-M100 remain planned/provisional.
