...
The active accepted baseline is v0.87.0. v0.87.0 implements M83 Shell Dry-Run Classifier as classifier-only, review-only, deterministic, local-only contracts over M82 command proposals with safe summary only receipt plans, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. It adds no dry-run execution, shell string, command execution, subprocess execution, shell execution, process spawn, filesystem mutation, network access, tool execution, browser automation, plugin execution, remote execution, model call, memory write, context injection, background worker, backend route, Control Center control, dependency, M84 work, or production authority. v0.86.0 implements M82 Command Proposal Contracts as proposal-only, review-only, deterministic, local contracts with structured argv preview metadata, safe command labels, safe purpose summaries, safe summary only receipt plans, evaluator revalidation, stable reason codes, documentation-integrity checks, static verification, tests, and Foundation Gate coverage. M84-M100 remain planned/provisional.
