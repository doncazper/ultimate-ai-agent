...
The active accepted baseline is v1.7.0. v1.7.0 implements M103 Camera/Photos Metadata-Only Contract as contract-only mobile media governance. Camera and photos remain off by default. It defines safe media refs, safe metadata refs, exact-scope metadata contracts, raw-media denial, consent, revocation, and audit requirements. It adds no camera runtime access, photo library runtime access, image capture, video capture, raw media content, precise EXIF location, face recognition, OCR, media export, native permission prompt, background media collection, backend route, Control Center control, dependency, broad autonomy, no mobile sensor runtime, or production authority. M100 remains implemented/released as Mobile Permission Model v1. M101, M102, and M103 are implemented/released, and M104-M150 remain planned/provisional in `docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md`.
