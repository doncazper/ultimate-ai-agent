...
| Supply-chain evidence | Frozen `uv.lock` installation, Python and npm audit, CycloneDX SBOM generation, and local CodeQL SARIF severity gating run on GitHub-hosted Linux. | Audit, SBOM validation, or high-severity SARIF fails the workflow. |
| Dependency compatibility | GitHub-hosted Linux installs the bounded lowest-direct and highest dependency resolutions and runs the identity, API, recovery, and property boundaries. | Either supported edge fails its focused pytest lane. |
