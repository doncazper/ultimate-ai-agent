...
## v0.5.2 Change Log — Scalable Stack + UI Strategy

v0.5.2 accepts the implementation stack and UI boundary strategy.

Added or updated:

```text
docs/canonical/38_scalable_stack_and_ui_strategy.md
docs/decisions/ADR-0037-use-python-agent-core-and-typescript-control-center.md
docs/decisions/ADR-0038-use-openwebui-as-chat-shell-not-agent-brain.md
docs/decisions/ADR-0039-use-stable-agent-api-boundary.md
...
docs/registry/capability_registry_v0_5_2.json
docs/registry/dependency_graph_v0_5_2.md
```
...
> Python Agent Core is the brain. TypeScript Control Center is the user control layer. OpenWebUI is an optional chat shell, not the agent brain.

All clients must enter through the Agent API Boundary. No UI surface may bypass the Execution Contract, Consent Ledger, Tool Broker, Event Ledger, Model Router, Cost policy, Memory Service, File Manager, or rollback rules.

## v0.5.2 Foundation Gate Addition

The Foundation Gate now requires:

```text
Agent API Boundary
OpenWebUI bypass-prevention eval
UI/client mutating calls produce Event Ledger records
TypeScript Control Center cannot directly access database or tools
API boundary schemas and contract tests
```

---

# Historical Baseline Through v0.5.1

Status: Working baseline expanded with implementation-ready Model Routing foundation, trust/control infrastructure, user-governed autonomy, observability, rollback, privacy, cost controls, source credibility, interoperability, an explicit agent constitution, and a layered brain/onion architecture designed for stable evolution.
Project: Ultimate AI Agent
...
v0.4.5 upgrades **Model Routing** from a draft-level module into a foundation-level specification.
...
docs/registry/capability_registry_v0_4_5.json
docs/registry/dependency_graph_v0_4_5.md
docs/reviews/module_readiness_audit_v0_4_5.md
```
...
Model routing is now part of the Foundation Gate. The system should route tasks by model class, not by hard-coded model names. The Orchestrator decides what work must happen; the Model Router chooses the best model/runtime; the Tool Broker controls actions; the Cost Governor controls spend; the Consent Ledger controls sensitive routing; the Event Ledger records decisions; and QA/evals measure whether routing worked.
...
# v0.4.6–v0.5.0 Foundation Detail Addendum

Status: Active planning baseline after expanding the next foundation modules.

## v0.4.6 — Execution Contract + Context Pack

v0.4.6 formalizes the agent brainstem.
...
## v0.4.7 — Event Ledger / Observability

v0.4.7 makes the black-box recorder implementation-ready.
...
## v0.4.8 — Consent Ledger + Tool Broker

v0.4.8 formalizes durable user consent and the only allowed pathway to tools.
...
## v0.4.9 — Memory Service + File Manager

v0.4.9 makes the memory/file foundation buildable.
...
v0.5.0 turns the foundation into a build sequence.
...
They may be researched and shaped, but not implemented until the Foundation Gate passes.

## Updated architecture statement

The Ultimate AI Agent should be built as a layered brain with stable foundation contracts. The Execution Contract defines what the run is. The Context Pack defines what the run may know. The Event Ledger records what happened. The Consent Ledger defines what is allowed. The Tool Broker controls actions. The Memory Service recalls and learns. The File Manager preserves project truth. The Model Router optimizes model use. The Foundation Gate proves these primitives work before advanced intelligence is built on top.


---

# v0.5.1 — Pre-Coding Readiness and Prompt Pack Update

Status: Active pre-coding baseline  
Purpose: Close the gap between foundation planning and implementation start.
...
30-hour pre-coding readiness checklist
Initial issue seed list
M0-M2 build order
...
Context Pack Builder
Model Router
Event Ledger Recorder
Consent Policy Checker
Tool Broker Policy Agent
Memory Curator
File Manager
Spec Generator
QA / Eval Agent
...
The project is ready for repository import as docs, schemas, prompts, and planning artifacts.

The project is not yet ready for feature coding beyond M0-M2 foundation primitives.

## Recommended first programming work

```text
1. Initialize repository and import docs/schemas/prompts.
2. Add schema validation script.
3. Add CI check that validates JSON schemas and prompt registry references.
4. Implement Execution Contract model and validator.
5. Implement Context Pack model and builder against stub sources.
6. Implement Event Ledger append-only event store with local/dev persistence.
7. Add contract tests and receipt generation.
```
