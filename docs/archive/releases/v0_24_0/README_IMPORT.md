...
# README Import v0.24.0

Status: Current import README for v0.24.0 / M20.
...
M20 is contract-only. It adds no backend API route, no OpenAPI path count
change, no Android app, no iOS app, no macOS app, no native build workflow, no
OS permission integration, no mobile sensor access, no background service, no
notification runtime, no device pairing runtime, no runtime execution, no
