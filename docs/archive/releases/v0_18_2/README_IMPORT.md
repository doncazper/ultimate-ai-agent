...
v0.18.2 implements Open Design System and Control Center UI Design Governance documentation only.

M15 Approval Queue + Receipt/Event Viewer UI remains future work. M15 must read the design governance docs before implementation and must preserve read-only/preview-only Control Center boundaries.

v0.18.2 adds no frontend behavior, backend API route, production Control Center authority, runtime/model/provider call, remote dispatch, mobile/native app, sensor access, plugin enablement, Chrome authenticated profile control, Computer Use automation, iOS/macOS build workflow, analytics/auth/payment/SaaS SDK, design tool integration, design SaaS dependency, production persistence, dependency, or external action.
