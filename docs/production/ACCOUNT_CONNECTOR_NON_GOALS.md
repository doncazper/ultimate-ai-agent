# M114 Account Connector Non-Goals

M114 Account Connector Contract Review is not a runtime connector milestone.

M114 explicitly adds no production authority, no production runtime, no auth
...
Control Center control, no dependency, no broad unsandboxed autonomy, no beta
release, and no side effects.

The M114 contract may record safe refs, user refs, workspace refs, connector
contract refs, connector scope refs, credential boundary ref, auth boundary ref,
data access boundary refs, audit refs, replay refs, and a no-effect receipt
