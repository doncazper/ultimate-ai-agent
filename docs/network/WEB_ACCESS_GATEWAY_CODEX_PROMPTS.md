...
11. What is the smallest safe follow-up PR?
...
```text
Plan PR 2 only: WebAccess API/Manifest Integration.

Do not implement browser behavior or add providers.

Goals:
- Add status/preview API route only if it fits existing API patterns.
...
- Ensure routes return policy/audit summaries and cannot trigger browser/non-GET behavior.

Return file list, API contract, tests, non-goals, and security concerns.
```
