...
Status: planning-only subordinate plan; no runtime authority granted
...
Current implementation posture: SearXNG is not integrated; the Firecrawl
adapter is a disabled diagnostic shell; `search` and `extract_schema` remain
policy-denied
...
cases where the managed Fire-engine path adds value. A bounded
`cloud_budget_first` policy may be supported for deliberate evaluation or
operator-selected sessions, but it must use the same credit ceilings,
concurrency reservations, safe-disable posture, and audit rules.
...
| `cloud_budget_first` | Cloud first while an exact credit reservation succeeds; local fallback after quota/provider failure | Optional bounded evaluation/session policy |
...
- optionally expose bounded `cloud_budget_first` through backend configuration
...
1. Accept `self_host_first_cloud_escalation` as the steady-state default, or
   require `cloud_budget_first` for the first evaluation window.
