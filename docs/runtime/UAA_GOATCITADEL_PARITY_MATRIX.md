...
| Evidence receipts | Mixed | UAA has redacted runtime receipts, local SHA-256 hash-integrity evidence with legacy signed identifiers, portable local evidence envelopes, append-first receipt hashes, replay validation, and full-history hash-chained AuthorityLease budget, dispatch, mission-plan, and mission-step ledgers. Bounded synchronous missions bind every step definition and dispatch request to one immutable plan and cross-ledger terminal evidence without persisting raw input, paths, content, or output. UAA does not claim cryptographic signing or external anchoring. | `apps/gateway/src/services/evidence-receipt-service.ts` signs canonical manifests with Ed25519 and verifies them offline; receipts include run lineage, approvals, artifacts, and side effects. | Add cryptographic signing only after a real Keychain-backed lifecycle, rotation, loss, recovery, and revocation model is proven. |
