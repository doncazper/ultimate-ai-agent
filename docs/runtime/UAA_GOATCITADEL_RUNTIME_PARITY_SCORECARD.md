...
| Durable runtime integration | 6 | 8 | Medium | partial | Durable run/read-model work, RuntimeGateway invocation storage, Run Observability, and GoatCitadel catch-up durable orchestration docs/tests show local durable state and replay posture. | A canonical Turn -> Durable Run -> Approval state chain is not yet exposed as one inspectable runtime contract. | Phase 03 Prompt and Phase 04 Prompt: turn/run/approval model and staged orchestration |
