...
| Decision Review | `/workspace/decisions` | `SecondarySurfaces.tsx` | `GET /control-center/actions/inbox`; CLI: `scripts/inspect_plans_to_actions_bridge.py`, `scripts/inspect_action_inbox_decision_lanes.py`; `/actions` | Wired | Backend queue and envelopes render directly. The selected item shows a compact Plan -> Action -> Decision -> Receipt path only when the existing backend bridge and decision lane are exact-bound and authority-safe; missing or contradictory links fail closed and blocked authority stays visible. Mutation eligibility, receipt/cost gates, and read-only blocked behavior are unchanged. |
...
| Decision Review — review path | Read | Existing Plans-to-Actions bridge and Action Inbox decision-lane read models from `GET /control-center/actions/inbox`; CLI inspection through `scripts/inspect_plans_to_actions_bridge.py` and `scripts/inspect_action_inbox_decision_lanes.py` | Wired | Recomputes from the selected backend item and refreshed inbox | Exact plan/action envelope, scope, expected receipts, safe refs, and denied-authority posture must agree; missing or authority-claiming bridges render unavailable rather than being inferred in React | `WiredSurfaces.test.tsx` |
