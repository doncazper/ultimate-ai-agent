...
| Communications | `/workspace/communications` | `PrimarySurfaces.tsx` | `GET /control-center/founder-loop/source-readiness`; `/inbox` | Partial | Backend source-readiness counts/posture are wired; message rows remain explicitly labeled preview data. Search and selection are local. Draft, CRM, follow-up, calendar, send, and assistant actions are disabled. |
| Messenger | `/workspace/messenger?view=<state>` | `apps/control-center/src/northstar/MessengerSurface.tsx` | No Matrix account/sync/send contract | Preview only | Fifteen reference states are built; no auth, sync, encryption, room mutation, send, media, or call authority. |
| Work Board | `/workspace/work-board` | `PrimarySurfaces.tsx` | `GET /control-center/work-board`; `/work-board` | Partial | Backend columns/cards, local search, exact card-create receipts, exact local-task receipts, and post-receipt refresh are wired. Reorder persistence and completion are skipped; task execution is unavailable. |
...
| Calendar | `/workspace/calendar` | `PrimarySurfaces.tsx` | Source posture from `GET /control-center/founder-loop/source-readiness`; no event read model | Partial | Backend calendar-source readiness is visible; event grid remains a disabled preview. Navigation, proposal edit, assistant handoff, and external writes are unavailable. |
| News | `/workspace/news` | `SecondarySurfaces.tsx` | Source posture from `GET /control-center/founder-loop/source-readiness`; no feed read model | Partial | Backend source posture is visible; articles remain labeled preview data. Search/topic selection are local. Open, save, mute, and assistant actions are disabled. |
...
| Activity & Trust | `/workspace/activity-trust` | `SecondarySurfaces.tsx` | `GET /control-center/trust-authority/matrix`; `GET /control-center/settings/status`; `/trust`, `/settings` | Partial | Domain/tier matrix, active lease, and policy decisions are backend reads. Exact active lease revocation is wired with confirmation, receipt, and settings refresh. Pause, kill switch, and safe-disable mutation are unavailable. |
...
| Decision Review | `/workspace/decisions` | `SecondarySurfaces.tsx` | `GET /control-center/actions/inbox`; `/actions` | Wired | Backend queue and envelopes render directly. Eligible items can record approve/edit/reject/defer receipts, enforce the cost gate, and reconcile against a refreshed backend queue. Read-only/blocked items never gain controls. |
...
| Work Board — columns/cards | Read | `GET /control-center/work-board` | Wired | Refreshes after successful create receipts | Python owns board state; no durable React duplicate | `WiredSurfaces.test.tsx` |
...
| Work Board — Create card | Mutation | `POST /control-center/work-board/cards` | Wired | Shows exact receipt; then reloads `GET /control-center/work-board`; reports refresh-pending safely on failure | Enabled only when backend ownership, contract, route, and approval posture are all true | `WiredSurfaces.test.tsx` |
| Work Board — Create local task record | Mutation | `POST /control-center/work-board/tasks` | Wired | Shows exact receipt and refreshes board | Records a task only; does not execute or complete it | `WiredSurfaces.test.tsx` |
...
| Decision Review — approve/edit/reject/defer | Mutation | `POST /control-center/actions/{action_id}/{decision}` | Wired | Shows decision receipt, action-executed truth, and reconciliation state | Approve is disabled unless backend cost posture is approved | `WiredSurfaces.test.tsx` |
...
| Trust — Revoke active lease | Mutation | `POST /api/runtime/authority-leases/revoke` through typed client | Wired | First click requests confirmation; second records receipt; then settings status refreshes | Only an exact active non-read-only lease is eligible | `WiredSurfaces.test.tsx` |
...
| Developer Tools — Inspect/Review links | Navigation | Canonical `/runtime`, `/actions`, `/models`, `/settings`, `/action-preview` | Wired | Canonical route owns refresh and mutation posture | Avoids duplicating backend state | Typecheck and browser QA |
...
| CRM — v3 table/detail | Read | `GET /control-center/crm/summary` | Wired | Initial app data load | Safe refs/summaries only; all external/write controls remain disabled | `WiredSurfaces.test.tsx`; browser QA |
...
| Communications / Calendar / News source posture | Read | Founder Loop source-readiness contract | Wired | Initial app data load | Message/event/article content still lacks an owning read model | Typecheck and browser QA |
...
| `apps/control-center/src/api/client.ts` | Adds a typed Work Board refresh read using the existing endpoint. |
...
| `apps/control-center/src/northstar/MessengerSurface.tsx` | Keeps preview navigation local and disables unsupported send, invite, call, sync, settings, and connector actions. |
