...
| Founder Loop | `/today` | Today | Daily command deck | non-redundant synthesized briefing; cross-surface attention queue; compact sourced news + weather; mixed article/email-bulletin sources; stale or blocked news/weather; trend-only business pulse; CRM follow-up selected; Work Board blocker selected; no decisions; high-risk blocked item; degraded sources; persistent privacy/authority posture; UAA sidecar asking about board/CRM | yes |
