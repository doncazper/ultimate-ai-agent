...
v0.18.1 hardens M14 by making local backend connection safety more explicit in the existing Web Control Center shell.

M14 remains scoped to local-only API base validation and visible backend connection states. v0.18.1 strengthens rejection coverage for public/private non-loopback hosts, non-loopback hostnames, URL credentials, and broad secret-like query parameters, and adds explicit unknown/checking connection-state copy.

M15 Approval Queue + Receipt/Event Viewer UI remains future work.

v0.18.1 adds no backend API route, production Control Center authority, public execution API, runtime/model/provider call, remote dispatch, mobile/native app, sensor access, plugin enablement, Chrome authenticated profile control, Computer Use automation, iOS/macOS build workflow, analytics/auth/payment/SaaS SDK, production persistence, external API host, dependency, or external action.
