...
def _empty_read_model() -> dict[str, Any]:
    read_model = build_action_inbox_decision_lane_read_model(actions=[])
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Inspect backend-owned Action Inbox decision-lane posture."
    )
    ...
    if db_path.exists():
        try:
            ...
            read_model = dict(
                inbox.get("action_inbox_decision_lane_read_model")
            ...
            inspection_error_ref = (
                "error-ref:action-inbox-decision-lanes:read-failed-redacted"
            )
    else:
        read_model = _empty_read_model()
    ...
    output = {
        "schema_version": "product-loop-005-action-inbox-decision-lanes.inspect.v1",
        "command_ref": "repo-local-command:inspect-action-inbox-decision-lanes",
        "contract_ref": ACTION_INBOX_DECISION_LANE_CONTRACT_REF,
        "storage_state": storage_state,
        "inspection_error_ref": inspection_error_ref,
        "action_inbox_decision_lane_read_model": read_model,
        "safe_refs_only": True,
