...
blocked until later exact scoped milestones exist.
"""
...
def _validate_active_docs(root: Path, failures: list[str]) -> None:
    required_by_doc = {
        CURRENT_BOARD: [
            ...
            "Future Local Model Manager implementation stages require later exact scoped milestones",
        ...
        PRODUCT_TRUTH: [
            "UAA-P1-062 now documents the Local Model Manager / Memory-Aware Runtime Control lane shape",
            "planned/blocked for model manager backend status",
            SCOPE_REF,
        ],
        GAP_MAP: [
