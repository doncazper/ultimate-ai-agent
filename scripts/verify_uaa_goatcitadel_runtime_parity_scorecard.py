#!/usr/bin/env python3
"""Verify the UAA GoatCitadel runtime parity scorecard artifact."""
...
DEFAULT_REPORT = ROOT / "docs" / "runtime" / "UAA_GOATCITADEL_RUNTIME_PARITY_SCORECARD.md"
...
def verify(report_path: Path = DEFAULT_REPORT) -> list[str]:
    ...
    if "GoatCitadel refs are architectural evidence only" not in text:
        failures.append("GoatCitadel reference posture is not explicit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("UAA GoatCitadel runtime parity scorecard verified")
    return 0
