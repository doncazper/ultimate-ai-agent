...
FORBIDDEN_RELEASE_SECURITY_CLAIMS = [
    ...
    "raw prompt logging is enabled",
    "raw prompts are stored",
    "raw response logging is enabled",
    "raw responses are stored",
    "raw provider payload logging is enabled",
    "raw provider payloads are stored",
    "raw provider payload export is enabled",
    "raw path logging is enabled",
    "raw paths are stored",
    "raw local paths are stored",
    "raw log retention is enabled",
    "raw logs are stored",
    "raw environment dumps are stored",
    "credentials are stored in release evidence",
    "secret-like values are stored in release evidence",
]
...
def _verify_operator_runtime_currentness(root: Path) -> list[str]:
    ...
    product_truth_compact = " ".join(product_truth.split())
    ...
    board_required = {
        ...
        "active board must track UAA-P0-002": "uaa-p0-002 product release-truth packet",
    }
    ...
    required_current_fragments = {
        ...
        "active docs must link product release-truth packet": (
            "docs/roadmap/product_release_truth_packet.md"
        ),
    ...
    for message, fragment in {
        ...
        "Operator roadmap must list UAA-P0-002": "uaa-p0-002",
        "Operator roadmap must link the product release-truth packet": (
            "docs/roadmap/product_release_truth_packet.md"
        ),
    }.items():
    ...
    for rel_path, text in {
        "README.md": readme,
        "docs/README.md": docs_readme,
        "docs/DOCUMENTATION_INDEX.md": docs_index,
        "docs/canonical/CANONICAL_DOC_MAP.md": canonical_map,
        "docs/roadmap/README.md": roadmap_index,
    }.items():
        if "docs/roadmap/product_release_truth_packet.md" not in text:
            failures.append(f"{rel_path} must link product release-truth packet")
    ...
    product_truth_required = {
        ...
        "product truth packet must list accepted local model checkpoints": (
            "accepted local model lane checkpoints: checkpoint-m166, checkpoint-m167"
        ),
        "product truth packet must link source plan": (
            "source plan: `docs/roadmap/operator_runtime_excellence_roadmap.md`"
        ),
        "product truth packet must frame peer context as product-shaping only": (
            "product-shaping evidence only"
        ),
        "product truth packet must deny implementation dependency": (
            "not an implementation dependency"
        ),
        "product truth packet must include release truth section": "## release truth",
        "product truth packet must include product excellence matrix": (
            "## product excellence matrix"
        ),
        "product truth packet must include exact matrix columns": (
            "| capability | current uaa state | target production-ready state | "
            "priority | status | evidence source or missing evidence | "
            "blocking gate |"
        ),
        "product truth packet must define shipped status": (
            "shipped: evidence-backed current repository capability only"
        ),
        "product truth packet must define planned status": (
            "planned: named by the operator runtime excellence roadmap"
        ),
        "product truth packet must define blocked status": (
            "blocked: cannot be claimed until the named gate is met"
        ),
        "product truth packet must define future-scoped status": (
            "future-scoped: requires a later accepted milestone"
        ),
        "product truth packet must mark planned product shell": (
            "| product shell |"
        ),
        ...
        "product truth packet must cite local model hardening evidence": (
            "tests/test_m167_live_model_hardening.py"
        ),
        "product truth packet must cite local model E2E smoke harness doc": (
            "docs/production/m167_local_model_e2e_smoke_harness.md"
        ),
        "product truth packet must cite local model E2E smoke harness tests": (
            "tests/test_m151_openwebui_local_gateway_api.py"
        ),
        "product truth packet must cite current board gaps": (
            "docs/kanban/current_board.md"
        ),
        "product truth packet must deny production readiness claims": (
            "do not claim production readiness"
        ),
        "product truth packet must deny public distribution claims": (
            "public distribution"
        ),
        "product truth packet must deny broad autonomy": "broad autonomy",
        "product truth packet must deny unapproved runtime authority": (
            "unapproved runtime authority"
        ),
        "product truth packet must deny shell execution": "shell execution",
        "product truth packet must deny connector writes": "connector writes",
        "product truth packet must deny plugin runtime import": (
            "plugin runtime import"
        ),
        "product truth packet must deny raw durable evidence fields": (
            "raw prompt, raw response, raw provider payload, raw path, raw log"
        ),
        "product truth packet must require scoped milestone for non-goal claims": (
            "accepted scoped milestone"
        ),
    }
    for message, fragment in product_truth_required.items():
        if fragment not in product_truth_compact:
            failures.append(message)

    return failures


def _verify_public_security_posture(root: Path) -> list[str]:
    failures: list[str] = []
    for rel_path in REQUIRED_PUBLIC_SECURITY_DOCS:
        if not (root / rel_path).exists():
            failures.append(f"missing public security posture file: {rel_path}")

    def read_lower(rel_path: str) -> str:
        path = root / rel_path
        return _read(path).lower() if path.exists() else ""

    security = read_lower("SECURITY.md")
    runbook = read_lower("docs/security/SECURITY_TRIAGE_RUNBOOK.md")
    docs_index = read_lower("docs/DOCUMENTATION_INDEX.md")
    docs_readme = read_lower("docs/README.md")
    readme = read_lower("README.md")
    canonical_map = read_lower("docs/canonical/CANONICAL_DOC_MAP.md")
    ...
    security_compact = " ".join(security.split())
    runbook_compact = " ".join(runbook.split())
    active_security_text = "\n".join(
        [
            security,
            runbook,
            docs_index,
    ...
    for rel_path, text in {
        ...
        if "security.md" not in text:
            failures.append(f"{rel_path} must link SECURITY.md")
        if "docs/security/security_triage_runbook.md" not in text:
            failures.append(f"{rel_path} must link security triage runbook")
    ...
    security_required = {
        ...
        "SECURITY.md must include severity definitions": "## severity definitions",
        "SECURITY.md must include unsafe disclosure guidance": (
            "## unsafe disclosure guidance"
        ),
        "SECURITY.md must tell reporters not to post unsafe details": (
            "do not post raw prompt content"
        ),
        "SECURITY.md must tell public reporters to use safe summaries": (
            "public issues and release-facing comments should contain only a safe summary"
        ),
        "SECURITY.md must define critical severity": "| critical |",
        "SECURITY.md must define high severity": "| high |",
        "SECURITY.md must define medium severity": "| medium |",
        "SECURITY.md must define low severity": "| low |",
        "SECURITY.md must include response targets": "## response targets",
        "SECURITY.md must target 3 business day acknowledgement": "within 3 business days",
        "SECURITY.md must target 7 business day severity assessment": (
            "within 7 business days"
        ),
        "SECURITY.md must include security invariants": "## security invariants",
        "SECURITY.md must deny public distribution claim": (
            "public distribution is claimed by this policy"
        ),
        "SECURITY.md must preserve PolicyEngine": "policyengine",
        "SECURITY.md must preserve LocalApprovalAuthority": "localapprovalauthority",
        "SECURITY.md must preserve route side-effect classification": (
            "route side-effect classification"
        ),
        "SECURITY.md must preserve OpenAPI checks": "openapi checks",
        "SECURITY.md must preserve Foundation Gate checks": "foundation gate checks",
        "SECURITY.md must deny unrestricted shell/subprocess execution": (
            "no unrestricted shell/subprocess execution"
        ),
        "SECURITY.md must deny connector writes": "connector writes",
        "SECURITY.md must deny plugin runtime import": "plugin runtime import",
        "SECURITY.md must deny mobile control": "mobile control",
    }
    for message, fragment in security_required.items():
        if fragment not in security_compact:
            failures.append(message)

    runbook_required = {
        "triage runbook must identify UAA-P0-003": "program task: uaa-p0-003",
        "triage runbook must cover secret scanning": "secret scanning",
        "triage runbook must cover dependency alerts": "dependency alerts",
        "triage runbook must cover unsafe logging": "unsafe logging",
        "triage runbook must cover route auth issues": "route auth issues",
        "triage runbook must cover redaction regressions": "redaction regressions",
        "triage runbook must cover no-secret-output failures": (
            "no-secret-output failures"
        ),
        "triage runbook must cover release-blocking findings": (
            "release-blocking security findings"
        ),
        "triage runbook must include release blocking section": "## release blocking",
        "triage runbook must block unsafe release claims": (
            "overclaim production or public readiness"
        ),
        "triage runbook must include focused safe-error test": (
            "tests/test_api_safe_exception_messages.py"
        ),
        "triage runbook must include focused secret redaction test": (
            "tests/test_secret_broker_redaction.py"
        ),
        "triage runbook must include verify_all lane": (
            "scripts/verify_all.py --skip-ruff --skip-pytest"
        ),
        "triage runbook must include rollback": "## remediation and rollback",
        "triage runbook must deny scanner runtime": "does not add scanner runtime",
    }
    for message, fragment in runbook_required.items():
        if fragment not in runbook_compact:
            failures.append(message)

    for message, fragment in {
        "active docs must mention UAA-P0-003": "uaa-p0-003",
        "active docs must mention public security posture": "public security posture",
        "active docs must include security policy link": "security.md",
        "active docs must include security triage runbook link": (
            "docs/security/security_triage_runbook.md"
        ),
        "active docs must include no-secret-output language": "no-secret-output",
    }.items():
        if fragment not in active_security_text:
            failures.append(message)

    for rel_path in RELEASE_FACING_SECURITY_DOCS:
        path = root / rel_path
        if not path.exists():
            failures.append(f"missing release-facing security scan target: {rel_path}")
            continue
        lowered = _read(path).lower()
        for fragment in FORBIDDEN_RELEASE_SECURITY_CLAIMS:
...
def _verify_m167_live_model_evidence_matrix(root: Path) -> list[str]:
    failures: list[str] = []
    matrix_path = root / REQUIRED_M167_EVIDENCE_MATRIX_DOC
    if not matrix_path.exists():
        return [f"missing M167 live evidence matrix: {REQUIRED_M167_EVIDENCE_MATRIX_DOC}"]

    def read_lower(rel_path: str) -> str:
        path = root / rel_path
        return _read(path).lower() if path.exists() else ""

    matrix = read_lower(REQUIRED_M167_EVIDENCE_MATRIX_DOC)
    matrix_compact = " ".join(matrix.split())
    active_links = {
    ...
    matrix_link = REQUIRED_M167_EVIDENCE_MATRIX_DOC.lower()
    for rel_path, text in active_links.items():
        if matrix_link not in text:
            failures.append(f"{rel_path} must link M167 live evidence matrix")

    required_fragments = {
        "matrix must identify UAA-P0-004": "status: active uaa-p0-004 evidence scaffold",
        "matrix must preserve checkpoint-m167": "source checkpoint: checkpoint-m167",
        "matrix must bind authority to M166": (
            "authority source: m166 exact-scope local model production gate"
        ),
        "matrix must deny execution": "does not start llama.cpp",
        "matrix must deny OpenWebUI calls": "call openwebui",
        "matrix must deny downloads": "download a model",
        "matrix must deny routes": "add a backend route",
        "matrix must deny new production authority": "grant new production authority",
        "matrix must define Proven": "proven: reviewed live evidence exists",
        "matrix must define Pending": "pending: the row is scoped",
        "matrix must define Blocked": "blocked: the row is scoped",
        "matrix must define Not-scoped": "not-scoped: the behavior is outside this task",
        "matrix must state no row is proven": "no hardware row is proven in this patch",
        "matrix must include exact columns": (
            "| profile | safe evidence ref | reviewer ref | hardware profile summary | "
            "approved gguf/model ref | llama.cpp/openwebui shell status | "
            "blocker status | verification command/result ref | rollback status | "
            "production-readiness status |"
        ),
        "matrix must include safe evidence refs": "evidence-ref:m167:matrix:",
        "matrix must include reviewer refs": "review-ref:m167:",
        "matrix must include blocker refs": "blocker-ref:m167:",
        "matrix must include verification refs": "verification-ref:m167:",
        "matrix must include rollback refs": "rollback-ref:m167:",
        "matrix must mention approved model refs": "model-ref:m167:approved-gguf:pending",
        "matrix must deny tools/functions": "no tools/functions",
        "matrix must include not-scoped cases": "## not-scoped cases",
        "matrix must forbid raw prompts": "must not contain raw prompts",
        "matrix must forbid raw responses": "raw responses",
        "matrix must forbid raw provider payloads": "raw provider payloads",
        "matrix must forbid credential material": "credentials",
        "matrix must require M166 binding for Proven": "m166 authority-gate binding",
    }
    for message, fragment in required_fragments.items():
        if fragment not in matrix_compact:
            failures.append(message)

    required_rows = {
        "Apple Silicon": "| apple silicon | `evidence-ref:m167:matrix:apple-silicon` |",
        "CPU-only": "| cpu-only | `evidence-ref:m167:matrix:cpu-only` |",
        "low RAM": "| low ram | `evidence-ref:m167:matrix:low-ram` |",
        "discrete GPU": "| discrete gpu | `evidence-ref:m167:matrix:discrete-gpu` |",
        "limited disk": "| limited disk | `evidence-ref:m167:matrix:limited-disk` |",
    }
    for label, fragment in required_rows.items():
        if fragment not in matrix:
            failures.append(f"M167 matrix missing required hardware row: {label}")

    for forbidden in [
        "proven; production-ready",
        "production readiness is claimed",
        "public distribution is available",
        "openwebui admin flows are scoped",
        "tools/functions authority is scoped",
    ]:
        if forbidden in matrix:
            failures.append(f"M167 matrix contains unsafe claim: {forbidden}")

    board = read_lower("docs/kanban/current_board.md")
    if "uaa-p0-004 m167 live model evidence matrix" not in board:
        failures.append("current board must track UAA-P0-004 M167 matrix")
    if "gate met: apple silicon, cpu-only, low ram, discrete gpu, and limited disk rows" not in board:
        failures.append("current board must mark UAA-P0-004 gate met")
    if "uaa-p0-007 control center operator-shell gap map" not in board:
        failures.append("current board must keep UAA-P0-007 operator-shell gap map visible")

    return failures


def _verify_local_model_e2e_smoke_harness(root: Path) -> list[str]:
    failures: list[str] = []
    smoke_path = root / REQUIRED_LOCAL_MODEL_E2E_SMOKE_DOC
    if not smoke_path.exists():
        return [f"missing local model E2E smoke harness doc: {REQUIRED_LOCAL_MODEL_E2E_SMOKE_DOC}"]

    source_path = root / "src/ultimate_ai_agent/core/local_model_management/e2e_smoke.py"
    if not source_path.exists():
        failures.append("missing local model E2E smoke harness source module")

    def read_lower(rel_path: str) -> str:
        path = root / rel_path
        return _read(path).lower() if path.exists() else ""

    smoke_doc = read_lower(REQUIRED_LOCAL_MODEL_E2E_SMOKE_DOC)
    smoke_compact = " ".join(smoke_doc.split())
    smoke_link = REQUIRED_LOCAL_MODEL_E2E_SMOKE_DOC.lower()
    active_links = {
        "README.md": read_lower("README.md"),
        "docs/README.md": read_lower("docs/README.md"),
        "docs/DOCUMENTATION_INDEX.md": read_lower("docs/DOCUMENTATION_INDEX.md"),
    ...
    for rel_path, text in active_links.items():
        if smoke_link not in text:
            failures.append(f"{rel_path} must link local model E2E smoke harness")

    required_fragments = {
        "smoke doc must identify UAA-P0-005": "status: active uaa-p0-005 smoke harness",
        "smoke doc must preserve checkpoint-m167": "source checkpoint: checkpoint-m167",
        "smoke doc must bind authority to M166": (
            "authority gate: checkpoint-m166 exact-scope local llama.cpp/openwebui shell gate"
        ),
        "smoke doc must mention approved GGUF readiness": "approved gguf readiness",
        "smoke doc must mention skipped prerequisite semantics": (
            "skipped because prerequisite unavailable"
        ),
        "smoke doc must mention llama.cpp supervisor": "llama.cpp supervisor",
        "smoke doc must mention /v1 models": "local `/v1/models`",
        "smoke doc must mention /v1 chat": "local `/v1/chat/completions`",
        "smoke doc must mention OpenWebUI shell compatibility": (
            "openwebui shell compatibility"
        ),
        "smoke doc must mention auth failure": "auth failure",
        "smoke doc must mention safe failure": "safe failure",
        "smoke doc must mention rollback": "rollback",
        "smoke doc must mention tools/functions/streaming denial": (
            "tools/functions/streaming denial"
        ),
        "smoke doc must mention pass status": "| pass |",
        "smoke doc must mention fail status": "| fail |",
        "smoke doc must mention blocked status": "| blocked |",
        "smoke doc must mention skipped status": "| skipped |",
        "smoke doc must require safe-ref-only evidence": "safe-ref-only",
        "smoke doc must require redacted summaries": "redacted summary only",
        "smoke doc must name harness entry point": "run_local_model_e2e_smoke_harness",
        "smoke doc must cite required pytest lane": (
            "tests/test_m151_openwebui_local_gateway_api.py"
        ),
        "smoke doc must deny raw prompts": "no raw prompt",
        "smoke doc must deny raw responses": "no raw response",
        "smoke doc must deny raw provider payloads": "no raw provider payload",
        "smoke doc must deny raw paths": "no raw path",
        "smoke doc must deny raw logs": "no raw log",
        "smoke doc must deny credential material": "no credential material",
        "smoke doc must deny broad shell behavior": (
            "does not add unrestricted shell/subprocess execution"
        ),
        "smoke doc must deny public distribution": "public distribution",
    }
    for message, fragment in required_fragments.items():
        if fragment not in smoke_compact:
            failures.append(message)

    test_text = read_lower("tests/test_m151_openwebui_local_gateway_api.py")
    for message, fragment in {
        "M151 pytest lane must call P0-005 harness": "run_local_model_e2e_smoke_harness",
        "M151 pytest lane must cover passed status": "localmodele2esmokestatus.passed",
        "M151 pytest lane must cover skipped status": "localmodele2esmokestatus.skipped",
        "M151 pytest lane must cover blocked status": "localmodele2esmokestatus.blocked",
        "M151 pytest lane must cover rollback": "rollback_plan_ref",
        "M151 pytest lane must cover tools/functions/streaming": (
            "tools_functions_streaming_denial"
        ),
    }.items():
    ...
    board = read_lower("docs/kanban/current_board.md")
    if "uaa-p0-005 local model e2e smoke harness" not in board:
        failures.append("current board must track UAA-P0-005 local model E2E smoke harness")
    if "gate met: llama.cpp supervisor, local /v1 gateway, openwebui shell" not in board:
        failures.append("current board must mark UAA-P0-005 gate met")
    if "pull uaa-p0-005 next" in board:
        failures.append("current board still contains stale UAA-P0-005 next label")
    if "uaa-p0-007 control center operator-shell gap map" not in board:
        failures.append("current board must keep UAA-P0-007 visible after UAA-P0-005")

    return failures


def _verify_performance_baseline_harness(root: Path) -> list[str]:
    failures: list[str] = []
    doc_path = root / REQUIRED_PERFORMANCE_BASELINE_DOC
    if not doc_path.exists():
        return [f"missing performance baseline harness doc: {REQUIRED_PERFORMANCE_BASELINE_DOC}"]

    def read_lower(rel_path: str) -> str:
        path = root / rel_path
        return _read(path).lower() if path.exists() else ""

    perf_doc = read_lower(REQUIRED_PERFORMANCE_BASELINE_DOC)
    perf_compact = " ".join(perf_doc.split())
    perf_link = REQUIRED_PERFORMANCE_BASELINE_DOC.lower()
    active_links = {
        "README.md": read_lower("README.md"),
        "docs/README.md": read_lower("docs/README.md"),
        "docs/DOCUMENTATION_INDEX.md": read_lower("docs/DOCUMENTATION_INDEX.md"),
    ...
    required_doc_fragments = {
        ...
        "performance doc must link benchmark script": (
            "scripts/benchmark_foundation_gate.py"
        ),
        "performance doc must link latency check script": (
            "scripts/check_foundation_gate_latency.py"
        ),
        "performance doc must name reports/performance": "reports/performance",
        "performance doc must define JSON report": (
        ...
        "performance doc must include model route preview budget": (
            "| `/models/route/preview` | 150 ms |"
        ),
        "performance doc must include task classify budget": (
            "| `/task-decomposition/classify` | 100 ms |"
        ),
        "performance doc must include task decompose budget": (
            "| `/task-decomposition/decompose` | 250 ms |"
        ),
        "performance doc must include file preview budget": (
            "| `/files/read/preview` bounded text | 150 ms |"
        ),
        "performance doc must include v1 models budget": (
            "| `/v1/models` local gateway | 100 ms |"
        ),
        "performance doc must include v1 chat budget": (
            "| `/v1/chat/completions` local path | 250 ms |"
        ),
        "performance doc must include Control Center budget": (
    ...
    for message, fragment in required_doc_fragments.items():
        if fragment not in perf_compact:
            failures.append(message)
    ...
    for message, fragment in {
        ...
        "benchmark must measure task classify": "\"task_decomposition_classify\"",
        "benchmark must measure task decompose": "\"task_decomposition_decompose\"",
        "benchmark must measure file preview": "\"file_read_preview_bounded_text\"",
        "benchmark must measure v1 models": "\"v1_models_local_gateway\"",
        "benchmark must measure v1 chat": "\"v1_chat_completions_local_path\"",
    ...
    for fragment in [
        ...
        "reports/performance/latest_*.md",
    ]:
        if fragment not in gitignore:
            failures.append(f".gitignore must ignore generated performance report: {fragment}")

    board = read_lower("docs/kanban/current_board.md")
    if "uaa-p0-006 performance baseline harness" not in board:
        failures.append("current board must track UAA-P0-006 performance baseline harness")
    if "gate met: p50/p95 budgets for health, manifest, model route preview" not in board:
...
def _verify_control_center_operator_shell_gap_map(root: Path) -> list[str]:
    ...
    required_fragments = {
        ...
        "gap map must preserve PolicyEngine": "policyengine",
        "gap map must preserve LocalApprovalAuthority": "localapprovalauthority",
        "gap map must preserve route side-effect classification": (
            "route side-effect classification"
        ),
        "gap map must include exact matrix columns": (
        ...
        "gap map must define no side effect class": "| `none` |",
        "gap map must define validation side effect class": "| `validation_only` |",
        "gap map must define local dev side effect class": (
        ...
        "gap map must require no hidden authority": "no hidden authority",
        ...
        "gap map must mention GGUF selection": "gguf selection",
        "gap map must mention loopback llama.cpp settings": (
            "loopback llama.cpp settings"
        ),
        "gap map must mention safe registered capability": (
            "safe registered capability"
        ),
        "gap map must mention rollback refs": "rollback refs",
        "gap map must include rollback section": "## rollback",
    }
    for message, fragment in required_fragments.items():
    ...
    if "gate met: chat, plans, models, approvals, files, runtime, evidence, and" not in board:
...
def _verify_post_m100_roadmap_reconciliation_docs(root: Path, version: str | None) -> list[str]:
    ...
    if (
        "checkpoint m105" in version_doc_text
        or "m105" in version_doc_text
    ...
    if (
        "checkpoint m110" in version_doc_text
        or "m110" in version_doc_text
    ...
    if (
        "checkpoint m114" in version_doc_text
        or "m114" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m116" in version_doc_text
        or "m116" in version_doc_text
        or "role-based authority model" in version_doc_text
    ):
        implemented_milestones.add("m116")
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m117" in version_doc_text
        or "m117" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m119" in version_doc_text
        or "m119" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m122" in version_doc_text
        or "m122" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m126" in version_doc_text
        or "m126" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m128" in version_doc_text
        or "m128" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m131" in version_doc_text
        or "m131" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m134" in version_doc_text
        or "m134" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m137" in version_doc_text
        or "m137" in version_doc_text
    ...
    if _version_tuple(version) >= (1, 7, 2) and (
        "checkpoint m139" in version_doc_text
        or "m139" in version_doc_text
    ...
    stale_active_fragments = {
        "post-M100 docs must not keep stale M150 beta row": "| v1.54.0 | m150 | ultimate ai agent beta 1 |",
        "post-M100 docs must not keep stale M104 product-minor row": "| v1.8.0 | m104 | notification planning, no push execution |",
        "post-M100 docs must not claim M150 beta target": "m150 | ultimate ai agent beta 1",
    }
...
def _verify_m104_notification_planning_no_push_docs(root: Path, version: str | None) -> list[str]:
    ...
    if implemented_m105_row not in active_text:
        forbidden_fragments.add("m105 is implemented")
    for fragment in forbidden_fragments:
        if fragment in active_text or fragment in text:
...
def _verify_m105_background_task_contract_no_execution_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M105 background task contract doc: {rel_path}")
    text = "\n".join(
        _read(root / rel_path).lower()
        for rel_path in required_paths
        if (root / rel_path).exists()
    ...
    required_fragments = {
        "M105 docs must say Background Task Contract, No Execution": (
            "background task contract, no execution"
        ),
        "M105 docs must say contract-only": "contract-only",
        "M105 docs must say planning-only": "planning-only",
        "M105 docs must say safe refs": "safe refs",
        "M105 docs must say safe task summaries": "safe task summaries",
        "M105 docs must say safe cadence refs": "safe cadence refs",
        "M105 docs must say consent": "consent",
        "M105 docs must say revocation": "revocation",
        "M105 docs must say audit": "audit",
        "M105 docs must deny background worker": "no background worker",
        "M105 docs must deny scheduler": "no scheduler",
        "M105 docs must deny daemon": "no daemon",
        "M105 docs must deny OS background permission prompt": (
            "no os background permission prompt"
        ),
        "M105 docs must deny push trigger": "no push trigger",
        "M105 docs must deny device token handling": "no device token handling",
        "M105 docs must deny external service": "no external service",
        "M105 docs must deny raw task payload": "no raw task payload",
        "M105 docs must deny backend route": "no backend route",
        "M105 docs must deny Control Center control": "no control center control",
        "M105 docs must deny dependency": "no dependency",
        "M105 docs must deny production authority": "no production authority",
        "M105 docs must keep M106 future": "m106 remains future",
    }
    for message, fragment in required_fragments.items():
        if fragment not in text:
            failures.append(message)
...
def _verify_m110_mobile_sensor_hardening_freeze_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M110 sensor hardening freeze doc: {rel_path}")
    text = "\n".join(
        _read(root / rel_path).lower()
        for rel_path in required_paths
        if (root / rel_path).exists()
    ...
    required_fragments = {
        "M110 docs must say Mobile Sensor Hardening Freeze": "mobile sensor hardening freeze",
        "M110 docs must say contract-only": "contract-only",
        "M110 docs must say review-only": "review-only",
        "M110 docs must say freeze-only": "freeze-only",
        "M110 docs must say safe refs": "safe refs",
        "M110 docs must say safe sensor refs": "safe sensor refs",
        "M110 docs must say safe sensor scope refs": "safe sensor scope refs",
        "M110 docs must say hardening checklist refs": "hardening checklist refs",
        "M110 docs must say Mobile Sensor Audit Ledger": "mobile sensor audit ledger",
        "M110 docs must say actor-bound": "actor-bound",
        "M110 docs must say device-bound": "device-bound",
        "M110 docs must say sensor-scope-bound": "sensor-scope-bound",
        "M110 docs must say audit": "audit",
        "M110 docs must say replay": "replay",
        "M110 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M110 docs must deny hardening runtime": "no hardening runtime",
        "M110 docs must deny sensor access": "no sensor access",
        "M110 docs must deny sensor read": "no sensor read",
        "M110 docs must deny raw sensor payload": "no raw sensor payload",
        "M110 docs must deny location access": "no location access",
        "M110 docs must deny camera access": "no camera access",
        "M110 docs must deny photos access": "no photos access",
        "M110 docs must deny microphone access": "no microphone access",
        "M110 docs must deny background collection": "no background collection",
        "M110 docs must deny native mobile UI": "no native mobile ui",
        "M110 docs must deny backend route": "no backend route",
        "M110 docs must deny Control Center control": "no control center control",
        "M110 docs must deny notification delivery": "no notification delivery",
        "M110 docs must deny push trigger": "no push trigger",
        "M110 docs must deny background worker": "no background worker",
        "M110 docs must deny scheduler": "no scheduler",
        "M110 docs must deny daemon": "no daemon",
        "M110 docs must deny device token handling": "no device token handling",
        "M110 docs must deny external service": "no external service",
        "M110 docs must deny network sync": "no network sync",
        "M110 docs must deny raw audit payload": "no raw audit payload",
        "M110 docs must deny dependency": "no dependency",
        "M110 docs must deny memory write": "no memory write",
        "M110 docs must deny context injection": "no context injection",
...
def _verify_m113_secrets_boundary_docs(
    ...
    m114_is_current = (
        "checkpoint m114 is implemented/released" in active_text
...
def _verify_m114_account_connector_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m114 is implemented/released" not in active_text
        and "m114 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M114 account connector doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M114 docs must say account connector contract review": "account connector contract review",
        "M114 docs must say contract-only": "contract-only",
        "M114 docs must say review-only": "review-only",
        "M114 docs must say safe refs": "safe refs",
        "M114 docs must say secrets boundary": "secrets boundary",
        "M114 docs must say connector contract refs": "connector contract refs",
        "M114 docs must say connector scope refs": "connector scope refs",
        "M114 docs must say credential boundary ref": "credential boundary ref",
        "M114 docs must say auth boundary ref": "auth boundary ref",
        "M114 docs must say data access boundary refs": "data access boundary refs",
        "M114 docs must say actor-bound": "actor-bound",
        "M114 docs must say baseline-bound": "baseline-bound",
        "M114 docs must say source-secrets-boundary-bound": "source-secrets-boundary-bound",
        "M114 docs must say user-bound": "user-bound",
        "M114 docs must say workspace-bound": "workspace-bound",
        "M114 docs must say credential-boundary-bound": "credential-boundary-bound",
        "M114 docs must say auth-boundary-bound": "auth-boundary-bound",
        "M114 docs must say audit": "audit",
        "M114 docs must say replay": "replay",
        "M114 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M114 docs must deny production authority": "no production authority",
        "M114 docs must deny production runtime": "no production runtime",
        "M114 docs must deny auth runtime": "no auth runtime",
        "M114 docs must deny login": "no login",
        "M114 docs must deny session cookie": "no session cookie",
        "M114 docs must deny OAuth flow": "no oauth flow",
        "M114 docs must deny token exchange": "no token exchange",
        "M114 docs must deny credential handling": "no credential handling",
        "M114 docs must deny credential storage": "no credential storage",
        "M114 docs must deny credential read": "no credential read",
        "M114 docs must deny credential write": "no credential write",
        "M114 docs must deny secret material access": "no secret material access",
        "M114 docs must deny secret export": "no secret export",
        "M114 docs must deny vault runtime": "no vault runtime",
        "M114 docs must deny account connector runtime": "no account connector runtime",
        "M114 docs must deny account connector": "no account connector",
        "M114 docs must deny network access": "no network access",
        "M114 docs must deny account action": "no account action",
        "M114 docs must deny model call": "no model call",
        "M114 docs must deny memory write": "no memory write",
        "M114 docs must deny context injection": "no context injection",
        "M114 docs must deny execution": "no execution",
        "M114 docs must deny tool execution": "no tool execution",
        "M114 docs must deny shell execution": "no shell execution",
        "M114 docs must deny browser automation": "no browser automation",
        "M114 docs must deny plugin execution": "no plugin execution",
        "M114 docs must deny mobile sensor": "no mobile sensor",
        "M114 docs must deny background worker": "no background worker",
        "M114 docs must deny remote execution": "no remote execution",
        "M114 docs must deny backend route": "no backend route",
...
def _verify_m115_production_audit_retention_docs(
    ...
    if implemented_m116_row not in current_text:
        failures.append("active docs missing implemented Checkpoint M116 row")
    implemented_m117_row = (
        "| checkpoint m117 | pre-alpha checkpoint | m117 | "
        "remote agent coordination contract | implemented/released |"
    )
    planned_m117_row = (
        "| checkpoint m117 | pre-alpha checkpoint | m117 | "
        "remote agent coordination contract | planned/provisional |"
    )
    if implemented_m117_row not in current_text and planned_m117_row not in current_text:
...
def _verify_m116_role_based_authority_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m116 is implemented/released" not in active_text
        and "m116 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M116 role-based authority doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M116 docs must say role-based authority model": "role-based authority model",
        "M116 docs must say contract-only": "contract-only",
        "M116 docs must say review-only": "review-only",
        "M116 docs must say safe refs": "safe refs",
        "M116 docs must say production audit retention policy": "production audit retention policy",
        "M116 docs must say role refs": "role refs",
        "M116 docs must say authority scope refs": "authority scope refs",
        "M116 docs must say permission boundary refs": "permission boundary refs",
        "M116 docs must say separation-of-duty refs": "separation-of-duty refs",
        "M116 docs must say break-glass boundary ref": "break-glass boundary ref",
        "M116 docs must say actor-bound": "actor-bound",
        "M116 docs must say baseline-bound": "baseline-bound",
        "M116 docs must say source-production-audit-retention-bound": "source-production-audit-retention-bound",
        "M116 docs must say user-bound": "user-bound",
        "M116 docs must say workspace-bound": "workspace-bound",
        "M116 docs must say role-bound": "role-bound",
        "M116 docs must say authority-scope-bound": "authority-scope-bound",
        "M116 docs must say permission-boundary-bound": "permission-boundary-bound",
        "M116 docs must say separation-of-duty-bound": "separation-of-duty-bound",
        "M116 docs must say audit": "audit",
        "M116 docs must say replay": "replay",
        "M116 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M116 docs must deny production authority": "no production authority",
        "M116 docs must deny production runtime": "no production runtime",
        "M116 docs must deny authority runtime": "no authority runtime",
        "M116 docs must deny role enforcement": "no role enforcement",
        "M116 docs must deny permission enforcement": "no permission enforcement",
        "M116 docs must deny auth runtime": "no auth runtime",
        "M116 docs must deny login": "no login",
        "M116 docs must deny session cookie handling": "no session cookie handling",
        "M116 docs must deny OAuth flow": "no oauth flow",
        "M116 docs must deny token exchange": "no token exchange",
        "M116 docs must deny credential handling": "no credential handling",
        "M116 docs must deny account action": "no account action",
        "M116 docs must deny network access": "no network access",
        "M116 docs must deny model call": "no model call",
        "M116 docs must deny memory write": "no memory write",
        "M116 docs must deny context injection": "no context injection",
        "M116 docs must deny execution": "no execution",
        "M116 docs must deny backend route": "no backend route",
    ...
    for fragment in future_fragments:
        if fragment in current_text or fragment in text:
            failures.append(f"M116 docs imply forbidden/future capability: {fragment}")
    return failures


def _verify_m117_remote_agent_coordination_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m117 is implemented/released" not in active_text
        and "m117 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M117 remote agent coordination doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M117 docs must say remote agent coordination contract": "remote agent coordination contract",
        "M117 docs must say contract-only": "contract-only",
        "M117 docs must say review-only": "review-only",
        "M117 docs must say safe refs": "safe refs",
        "M117 docs must say role-based authority model": "role-based authority model",
        "M117 docs must say remote agent refs": "remote agent refs",
        "M117 docs must say coordination scope refs": "coordination scope refs",
        "M117 docs must say trust boundary refs": "trust boundary refs",
        "M117 docs must say handoff protocol refs": "handoff protocol refs",
        "M117 docs must say communication channel refs": "communication channel refs",
        "M117 docs must say actor-bound": "actor-bound",
        "M117 docs must say baseline-bound": "baseline-bound",
        "M117 docs must say source-role-authority-model-bound": "source-role-authority-model-bound",
        "M117 docs must say user-bound": "user-bound",
        "M117 docs must say workspace-bound": "workspace-bound",
        "M117 docs must say remote-agent-bound": "remote-agent-bound",
        "M117 docs must say coordination-scope-bound": "coordination-scope-bound",
        "M117 docs must say trust-boundary-bound": "trust-boundary-bound",
        "M117 docs must say handoff-protocol-bound": "handoff-protocol-bound",
        "M117 docs must say audit": "audit",
        "M117 docs must say replay": "replay",
        "M117 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M117 docs must deny production authority": "no production authority",
        "M117 docs must deny remote agent runtime": "no remote agent runtime",
        "M117 docs must deny remote dispatch": "no remote dispatch",
        "M117 docs must deny remote execution": "no remote execution",
        "M117 docs must deny live connection": "no live connection",
        "M117 docs must deny network access": "no network access",
        "M117 docs must deny agent spawn": "no agent spawn",
        "M117 docs must deny background worker": "no background worker",
        "M117 docs must deny credential handling": "no credential handling",
        "M117 docs must deny account action": "no account action",
        "M117 docs must deny model call": "no model call",
        "M117 docs must deny memory write": "no memory write",
        "M117 docs must deny context injection": "no context injection",
        "M117 docs must deny execution": "no execution",
        "M117 docs must deny backend route": "no backend route",
...
def _verify_m118_deployment_mode_matrix_docs(
    ...
    implemented_m119_row = (
        "| checkpoint m119 | pre-alpha checkpoint | m119 | "
        "production red-team harness | implemented/released |"
    )
    if not (
        _roadmap_row_present(current_text, planned_m119_row)
        or _roadmap_row_present(current_text, implemented_m119_row)
    ):
        failures.append("active docs missing planned or implemented Checkpoint M119 row")
    for fragment in {
        "deployment runtime is implemented",
...
def _verify_m119_production_red_team_harness_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m119 is implemented/released" not in active_text
        and "m119 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M119 production red-team harness doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M119 docs must say production red-team harness": "production red-team harness",
        "M119 docs must say contract-only": "contract-only",
        "M119 docs must say review-only": "review-only",
        "M119 docs must say safe refs": "safe refs",
        "M119 docs must say deployment mode matrix": "deployment mode matrix",
        "M119 docs must say red-team scenario refs": "red-team scenario refs",
        "M119 docs must say abuse case refs": "abuse case refs",
        "M119 docs must say threat model refs": "threat model refs",
        "M119 docs must say safety control refs": "safety control refs",
        "M119 docs must say mitigation plan refs": "mitigation plan refs",
        "M119 docs must say actor-bound": "actor-bound",
        "M119 docs must say baseline-bound": "baseline-bound",
        "M119 docs must say source-deployment-mode-matrix-bound": "source-deployment-mode-matrix-bound",
        "M119 docs must say audit": "audit",
        "M119 docs must say replay": "replay",
        "M119 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M119 docs must deny production authority": "no production authority",
        "M119 docs must deny red-team execution": "no red-team execution",
        "M119 docs must deny attack automation": "no attack automation",
        "M119 docs must deny scanner runtime": "no scanner runtime",
        "M119 docs must deny external probing": "no external probing",
        "M119 docs must deny exploit generation": "no exploit generation",
        "M119 docs must deny network access": "no network access",
        "M119 docs must deny credential handling": "no credential handling",
        "M119 docs must deny model call": "no model call",
        "M119 docs must deny memory write": "no memory write",
        "M119 docs must deny context injection": "no context injection",
        "M119 docs must deny execution": "no execution",
        "M119 docs must deny backend route": "no backend route",
...
def _verify_m121_email_connector_contract_refresh_docs(
    ...
    m122_is_current = (
        "checkpoint m122 is implemented/released" in current_text
        or "m122 is implemented/released" in current_text
    )
    ...
    if not m122_is_current and not _roadmap_row_present(current_text, planned_m122_row):
...
def _verify_m122_calendar_connector_contract_refresh_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m122 is implemented/released" not in active_text
        and "m122 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
        ...
        "docs/release_notes/checkpoint_m122.md",
        "docs/archive/checkpoints/m122/README_IMPORT.md",
        "docs/archive/checkpoints/m122/master_plan.md",
        "docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md",
    ]
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M122 calendar connector doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M122 docs must say calendar connector contract refresh": "calendar connector contract refresh",
        "M122 docs must say contract-only": "contract-only",
        "M122 docs must say review-only": "review-only",
        "M122 docs must say safe refs": "safe refs",
        "M122 docs must say email connector contract refresh": "email connector contract refresh",
        "M122 docs must say calendar scope refs": "calendar scope refs",
        "M122 docs must say calendar boundary refs": "calendar boundary refs",
        "M122 docs must say event boundary refs": "event boundary refs",
        "M122 docs must say consent boundary refs": "consent boundary refs",
        "M122 docs must say data classification refs": "data classification refs",
        "M122 docs must say retention boundary refs": "retention boundary refs",
        "M122 docs must say actor-bound": "actor-bound",
        "M122 docs must say baseline-bound": "baseline-bound",
        "M122 docs must say source-email-connector-contract-refresh-bound": "source-email-connector-contract-refresh-bound",
        "M122 docs must say audit": "audit",
        "M122 docs must say replay": "replay",
        "M122 docs must say no-effect receipt plan": "no-effect receipt plan",
        "M122 docs must deny calendar connector runtime": "no calendar connector runtime",
        "M122 docs must deny calendar account auth": "no calendar account auth",
        "M122 docs must deny calendar read": "no calendar read",
        "M122 docs must deny calendar search": "no calendar search",
        "M122 docs must deny calendar event create": "no calendar event create",
        "M122 docs must deny calendar event update": "no calendar event update",
        "M122 docs must deny calendar event delete": "no calendar event delete",
        "M122 docs must deny calendar invite send": "no calendar invite send",
        "M122 docs must deny raw calendar content": "no raw calendar content",
        "M122 docs must deny credential handling": "no credential handling",
        "M122 docs must deny network access": "no network access",
        "M122 docs must deny backend route": "no backend route",
    ...
    implemented_m122_row = (
        "| checkpoint m122 | pre-alpha checkpoint | m122 | "
        "calendar connector contract refresh | implemented/released |"
    )
    planned_m123_row = (
...
def _verify_m125_connector_read_only_runtime_docs(
    ...
    allowed_m126_rows = [
        ...
        (
            "| checkpoint m126 | pre-alpha checkpoint | m126 | "
            "connector approval capture | implemented/released |"
        ),
    ]
    planned_m127_row = (
    ...
    if "checkpoint m127 is implemented/released" in current_text:
        if not (
            _roadmap_row_present(current_text, planned_m128_row)
            or _roadmap_row_present(current_text, implemented_m128_row)
        ):
...
def _verify_m126_connector_approval_capture_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m126 is implemented/released" not in active_text
        and "m126 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M126 connector approval capture doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M126 docs must say connector approval capture": "connector approval capture",
        "M126 docs must say exact-bound": "exact-bound",
        "M126 docs must say review-only": "review-only",
        "M126 docs must say safe refs only": "safe refs only",
        "M126 docs must say approval refs remain identifiers": "approval refs remain identifiers, not authority",
        "M126 docs must deny approval_test_": "approval_test_",
        "M126 docs must say M125 connector read-only runtime": "m125 connector read-only runtime",
        "M126 docs must say actor-bound": "actor-bound",
        "M126 docs must say user-bound": "user-bound",
        "M126 docs must say workspace-bound": "workspace-bound",
        "M126 docs must say replay-safe": "replay-safe",
        "M126 docs must say revocable": "revocable",
        "M126 docs must deny live connector runtime": "no live connector runtime",
        "M126 docs must deny account auth": "no account auth",
        "M126 docs must deny network access": "no network access",
        "M126 docs must deny credential handling": "no credential handling",
        "M126 docs must deny raw connector content": "no raw connector content",
        "M126 docs must deny full content read": "no full content read",
        "M126 docs must deny connector write": "no connector write",
        "M126 docs must deny connector send": "no connector send",
        "M126 docs must deny connector delete": "no connector delete",
        "M126 docs must deny connector export": "no connector export",
        "M126 docs must deny attachment download": "no attachment download",
        "M126 docs must deny model call": "no model call",
        "M126 docs must deny memory write": "no memory write",
        "M126 docs must deny context injection": "no context injection",
        "M126 docs must deny execution": "no execution",
        "M126 docs must deny backend route": "no backend route",
...
def _verify_m128_connector_write_execution_low_risk_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m128 is implemented/released" not in active_text
        and "m128 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(
                f"missing M128 connector write execution low-risk doc: {rel_path}"
            )
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
    ...
    required_fragments = {
        "M128 docs must say connector write execution": "connector write execution",
        "M128 docs must say low-risk-only": "low-risk-only",
        "M128 docs must say local-only": "local-only",
        "M128 docs must say safe-ref-only": "safe-ref-only",
        "M128 docs must say exact-bound": "exact-bound",
        "M128 docs must say injected safe transport": "injected safe transport",
        "M128 docs must say safe result ref": "safe result ref",
        "M128 docs must say safe summary": "safe summary",
        "M128 docs must say exact connector write approval": "exact connector write approval",
        "M128 docs must say approval refs remain identifiers": "approval refs remain identifiers, not authority",
        "M128 docs must say M127 Connector Write Dry-Run Planner": "m127 connector write dry-run planner",
        "M128 docs must say M126 connector approval capture": "m126 connector approval capture",
        "M128 docs must say M125 Connector Read-Only Runtime": "m125 connector read-only runtime",
        "M128 docs must say actor ref": "actor ref",
        "M128 docs must say user ref": "user ref",
        "M128 docs must say workspace ref": "workspace ref",
        "M128 docs must say audit": "audit",
        "M128 docs must say replay": "replay",
        "M128 docs must say revocation": "revocation",
        "M128 docs must say kill-switch": "kill-switch",
        "M128 docs must deny live connector runtime": "no live connector runtime",
        "M128 docs must deny account auth": "no account auth",
        "M128 docs must deny network access": "no network access",
        "M128 docs must deny credential handling": "no credential handling",
        "M128 docs must deny raw connector content": "no raw connector content",
        "M128 docs must deny full content read": "no full content read",
        "M128 docs must deny connector send execution": "no connector send execution",
        "M128 docs must deny connector delete execution": "no connector delete execution",
        "M128 docs must deny connector export": "no connector export",
        "M128 docs must deny connector bulk export": "no connector bulk export",
        "M128 docs must deny attachment download": "no attachment download",
        "M128 docs must deny model call": "no model call",
        "M128 docs must deny memory write": "no memory write",
        "M128 docs must deny context injection": "no context injection",
        "M128 docs must deny backend route": "no backend route",
...
def _verify_m131_autonomy_mode4_scoped_work_session_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m131 is implemented/released" not in active_text
        and "m131 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M131 autonomy mode4 doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M131 docs must say autonomy mode 4": "autonomy mode 4",
        "M131 docs must say scoped work session": "scoped work session",
        "M131 docs must say contract-only": "contract-only",
        "M131 docs must say review-only": "review-only",
        "M131 docs must say scoped-work-session-only": "scoped-work-session-only",
        "M131 docs must say deterministic": "deterministic",
        "M131 docs must say local-only": "local-only",
        "M131 docs must say safe-ref-only": "safe-ref-only",
        "M131 docs must say exact scope": "exact scope",
        "M131 docs must say actor-bound": "actor-bound",
        "M131 docs must say resource-bound": "resource-bound",
        "M131 docs must say capability-bound": "capability-bound",
        "M131 docs must say allowlist-bound": "allowlist-bound",
        "M131 docs must say duration-bound": "duration-bound",
        "M131 docs must say policy decision ref": "policy decision ref",
        "M131 docs must say approval bundle ref": "approval bundle ref",
        "M131 docs must say risk decision ref": "risk decision ref",
        "M131 docs must say audit ref": "audit ref",
        "M131 docs must say replay ref": "replay ref",
        "M131 docs must say revocation ref": "revocation ref",
        "M131 docs must say kill-switch ref": "kill-switch ref",
        "M131 docs must say no-effect receipt": "no-effect receipt",
        "M131 docs must deny session start": "no session start",
        "M131 docs must deny autonomous actions": "no autonomous actions",
        "M131 docs must deny execution": "no execution",
        "M131 docs must deny tool execution": "no tool execution",
        "M131 docs must deny shell execution": "no shell execution",
        "M131 docs must deny network access": "no network access",
        "M131 docs must deny browser automation": "no browser automation",
        "M131 docs must deny browser forms": "no browser forms",
        "M131 docs must deny authenticated browser": "no authenticated browser",
        "M131 docs must deny download": "no download",
        "M131 docs must deny upload": "no upload",
        "M131 docs must deny plugin execution": "no plugin execution",
        "M131 docs must deny connector runtime": "no connector runtime",
        "M131 docs must deny account auth": "no account auth",
        "M131 docs must deny background worker": "no background worker",
        "M131 docs must deny scheduler": "no scheduler",
        "M131 docs must deny model call": "no model call",
        "M131 docs must deny memory write": "no memory write",
        "M131 docs must deny context injection": "no context injection",
        "M131 docs must deny backend route": "no backend route",
    ...
    planned_m134_row = (
        ...
        "human checkpoint scheduling | implemented/released |"
    )
    planned_m135_row = (
    ...
    for fragment in {
        ...
        "autonomous recovery planner is implemented",
...
def _verify_m134_human_checkpoint_scheduling_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m134 is implemented/released" not in active_text
        and "m134 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M134 human checkpoint scheduling doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M134 docs must say human checkpoint scheduling": "human checkpoint scheduling",
        "M134 docs must say contract-only": "contract-only",
        "M134 docs must say review-only": "review-only",
        "M134 docs must say human-checkpoint-scheduling-only": "human-checkpoint-scheduling-only",
        "M134 docs must say deterministic": "deterministic",
        "M134 docs must say local-only": "local-only",
        "M134 docs must say safe-ref-only": "safe-ref-only",
        "M134 docs must say exact scope": "exact scope",
        "M134 docs must say Mode 5": "mode 5",
        "M134 docs must say M133 supervisor decision": "m133 supervisor decision",
        "M134 docs must say M132 trusted workflow decision": "m132 trusted workflow decision",
        "M134 docs must say checkpoint plan": "checkpoint plan",
        "M134 docs must say schedule plan": "schedule plan",
        "M134 docs must say checkpoint window": "checkpoint window",
        "M134 docs must say reviewer ref": "reviewer ref",
        "M134 docs must say consent": "consent",
        "M134 docs must say expiration": "expiration",
        "M134 docs must say reminder plan": "reminder plan",
        "M134 docs must say escalation plan": "escalation plan",
        "M134 docs must say pause condition": "pause condition",
        "M134 docs must say stop condition": "stop condition",
        "M134 docs must say risk decision": "risk decision",
        "M134 docs must say audit": "audit",
        "M134 docs must say replay": "replay",
        "M134 docs must say revocation": "revocation",
        "M134 docs must say kill-switch": "kill-switch",
        "M134 docs must say no-effect receipt": "no-effect receipt",
        "M134 docs must deny checkpoint scheduled state": "no checkpoint scheduled",
        "M134 docs must deny scheduling": "no scheduling",
        "M134 docs must deny prompt": "no prompt",
        "M134 docs must deny notification delivery": "no notification delivery",
        "M134 docs must deny reminder runtime": "no reminder runtime",
        "M134 docs must deny calendar write": "no calendar write",
        "M134 docs must deny approval capture": "no approval capture",
        "M134 docs must deny escalation runtime": "no escalation runtime",
        "M134 docs must deny supervisor runtime": "no supervisor runtime",
        "M134 docs must deny recovery execution": "no recovery execution",
        "M134 docs must deny scheduler": "no scheduler",
        "M134 docs must deny background worker": "no background worker",
        "M134 docs must deny autonomous actions": "no autonomous actions",
        "M134 docs must deny execution": "no execution",
        "M134 docs must deny tool execution": "no tool execution",
        "M134 docs must deny shell execution": "no shell execution",
        "M134 docs must deny network access": "no network access",
        "M134 docs must deny browser automation": "no browser automation",
        "M134 docs must deny plugin execution": "no plugin execution",
        "M134 docs must deny connector runtime": "no connector runtime",
        "M134 docs must deny account auth": "no account auth",
        "M134 docs must deny model call": "no model call",
        "M134 docs must deny memory write": "no memory write",
        "M134 docs must deny context injection": "no context injection",
        "M134 docs must deny backend route": "no backend route",
...
def _verify_m136_cross_tool_dependency_execution_docs(
    ...
    m137_implemented = (
        "checkpoint m137 is implemented/released" in current_text
        or "m137 is implemented/released" in current_text
    ...
    elif not _roadmap_row_present(current_text, planned_m137_row):
...
def _verify_m137_browser_connector_combined_workflow_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m137 is implemented/released" not in active_text
        and "m137 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M137 browser connector workflow doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M137 docs must say autonomous browser + connector combined workflows": "autonomous browser + connector combined workflows",
        "M137 docs must say contract-only": "contract-only",
        "M137 docs must say review-only": "review-only",
        "M137 docs must say browser-connector-combined-workflow-only": "browser-connector-combined-workflow-only",
        "M137 docs must say deterministic": "deterministic",
        "M137 docs must say local-only": "local-only",
        "M137 docs must say safe-ref-only": "safe-ref-only",
        "M137 docs must say exact scope": "exact scope",
        "M137 docs must say Mode 5": "mode 5",
        "M137 docs must say M136 cross-tool dependency execution decision": "m136 cross-tool dependency execution decision",
        "M137 docs must say M135 autonomous recovery planner decision": "m135 autonomous recovery planner decision",
        "M137 docs must say M134 human checkpoint scheduling decision": "m134 human checkpoint scheduling decision",
        "M137 docs must say M133 supervisor decision": "m133 supervisor decision",
        "M137 docs must say M132 trusted workflow decision": "m132 trusted workflow decision",
        "M137 docs must say browser workflow": "browser workflow",
        "M137 docs must say browser observation": "browser observation",
        "M137 docs must say browser action plan refs": "browser action plan refs",
        "M137 docs must say connector workflow": "connector workflow",
        "M137 docs must say connector account scope": "connector account scope",
        "M137 docs must say connector action plan refs": "connector action plan refs",
        "M137 docs must say workflow step refs": "workflow step refs",
        "M137 docs must say combined dependency graph": "combined dependency graph",
        "M137 docs must say dependency order refs": "dependency order refs",
        "M137 docs must say safe handoff": "safe handoff",
        "M137 docs must say dry-run plan": "dry-run plan",
        "M137 docs must say approval bundle": "approval bundle",
        "M137 docs must say human checkpoint": "human checkpoint",
        "M137 docs must say audit": "audit",
        "M137 docs must say replay": "replay",
        "M137 docs must say revocation": "revocation",
        "M137 docs must say kill-switch": "kill-switch",
        "M137 docs must say no-effect receipt": "no-effect receipt",
        "M137 docs must deny browser action": "no browser action",
        "M137 docs must deny connector action": "no connector action",
        "M137 docs must deny account auth": "no account auth",
        "M137 docs must deny dependency execution": "no dependency execution",
        "M137 docs must deny tool execution": "no tool execution",
        "M137 docs must deny shell execution": "no shell execution",
        "M137 docs must deny network access": "no network access",
        "M137 docs must deny plugin execution": "no plugin execution",
        "M137 docs must deny model call": "no model call",
        "M137 docs must deny memory write": "no memory write",
        "M137 docs must deny context injection": "no context injection",
        "M137 docs must deny backend route": "no backend route",
...
def _verify_m138_autonomous_error_handling_guardrails_docs(
    ...
    implemented_m139_row = (
        "| checkpoint m139 | pre-alpha checkpoint | m139 | "
        "autonomy abuse/loop detection | implemented/released |"
    )
    m139_implemented = (
        "checkpoint m139 is implemented/released" in current_text
        or "m139 is implemented/released" in current_text
    ...
    elif not _roadmap_row_present(current_text, planned_m139_row):
    ...
    if m139_implemented:
        forbidden_fragments.discard("autonomy abuse/loop detection is implemented")
        forbidden_fragments.discard("m139 autonomy abuse/loop detection is implemented")
        forbidden_fragments.discard("m139 dependency is added")
    for fragment in forbidden_fragments:
        if fragment in current_text or fragment in text:
...
def _verify_m139_autonomy_abuse_loop_detection_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m139 is implemented/released" not in active_text
        and "m139 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M139 abuse/loop detection doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M139 docs must say autonomy abuse/loop detection": "autonomy abuse/loop detection",
        "M139 docs must say contract-only": "contract-only",
        "M139 docs must say review-only": "review-only",
        "M139 docs must say autonomy-abuse-loop-detection-only": "autonomy-abuse-loop-detection-only",
        "M139 docs must say deterministic": "deterministic",
        "M139 docs must say local-only": "local-only",
        "M139 docs must say safe-ref-only": "safe-ref-only",
        "M139 docs must say exact scope": "exact",
        "M139 docs must say Mode 5": "mode 5",
        "M139 docs must say M138 error guardrail decision": "m138 error handling guardrail decision",
        "M139 docs must say M137 browser connector decision": "m137 browser connector combined workflow decision",
        "M139 docs must say M136 cross-tool dependency decision": "m136 cross-tool dependency execution decision",
        "M139 docs must say M135 recovery planner decision": "m135 autonomous recovery planner decision",
        "M139 docs must say M134 human checkpoint decision": "m134 human checkpoint scheduling decision",
        "M139 docs must say M133 supervisor decision": "m133 supervisor decision",
        "M139 docs must say M132 trusted workflow decision": "m132 trusted workflow decision",
        "M139 docs must say abuse signal refs": "abuse signal refs",
        "M139 docs must say loop signal refs": "loop signal refs",
        "M139 docs must say pattern policy refs": "pattern policy refs",
        "M139 docs must say threshold policy refs": "threshold policy refs",
        "M139 docs must say intervention plan refs": "intervention plan refs",
        "M139 docs must say escalation plan refs": "escalation plan refs",
        "M139 docs must say human checkpoint": "human checkpoint",
        "M139 docs must say audit": "audit",
        "M139 docs must say replay": "replay",
        "M139 docs must say revocation": "revocation",
        "M139 docs must say kill-switch": "kill-switch",
        "M139 docs must say no-effect receipt": "no-effect receipt",
        "M139 docs must deny abuse detection runtime": "no abuse detection runtime",
        "M139 docs must deny loop detection runtime": "no loop detection runtime",
        "M139 docs must deny loop monitor": "no loop monitor",
        "M139 docs must deny detector runtime": "no detector runtime",
        "M139 docs must deny loop intervention": "no loop intervention",
        "M139 docs must deny autonomous recovery execution": "no autonomous recovery execution",
        "M139 docs must deny retry execution": "no retry execution",
        "M139 docs must deny rollback execution": "no rollback execution",
        "M139 docs must deny resume execution": "no resume execution",
        "M139 docs must deny dependency execution": "no dependency execution",
        "M139 docs must deny browser action": "no browser action",
        "M139 docs must deny connector action": "no connector action",
        "M139 docs must deny tool execution": "no tool execution",
        "M139 docs must deny shell execution": "no shell execution",
        "M139 docs must deny network access": "no network access",
        "M139 docs must deny plugin execution": "no plugin execution",
        "M139 docs must deny model call": "no model call",
        "M139 docs must deny memory write": "no memory write",
        "M139 docs must deny context injection": "no context injection",
        "M139 docs must deny backend route": "no backend route",
...
def _verify_m145_enterprise_pro_safety_modes_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m145 is implemented/released" not in active_text
        and "m145 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M145 Enterprise/Pro safety modes doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M145 docs must say Enterprise/Pro Safety Modes": "enterprise/pro safety modes",
        "M145 docs must say contract-only": "contract-only",
        "M145 docs must say review-only": "review-only",
        "M145 docs must say deterministic": "deterministic",
        "M145 docs must say local-only": "local-only",
        "M145 docs must say safe-ref-only": "safe-ref-only",
        "M145 docs must say safety-modes-only": "safety-modes-only",
        "M145 docs must say disabled by default": "disabled by default",
        "M145 docs must say route-free": "route-free",
        "M145 docs must say no-effect": "no-effect",
        "M145 docs must say accepted M101-M144": "accepted m101-m144",
        "M145 docs must say enterprise safety mode refs": "enterprise safety mode refs",
        "M145 docs must say pro safety mode refs": "pro safety mode refs",
        "M145 docs must say workspace boundary refs": "workspace boundary refs",
        "M145 docs must say role policy refs": "role policy refs",
        "M145 docs must say authority ceiling refs": "authority ceiling refs",
        "M145 docs must say feature availability refs": "feature availability refs",
        "M145 docs must say escalation policy refs": "escalation policy refs",
        "M145 docs must deny enterprise runtime": "no enterprise runtime",
        "M145 docs must deny pro runtime": "no pro runtime",
        "M145 docs must deny plan enforcement": "no plan enforcement",
        "M145 docs must deny billing runtime": "no billing runtime",
        "M145 docs must deny billing plan boundary": "no billing plan boundary",
        "M145 docs must deny account tenant runtime": "no account tenant runtime",
        "M145 docs must deny auth runtime": "no auth runtime",
        "M145 docs must deny backend route": "no backend route",
        "M145 docs must deny Control Center control": "no control center control",
        "M145 docs must deny dependency": "no dependency",
...
def _verify_m148_external_security_review_docs(
    root: Path, version: str | None
) -> list[str]:
    failures: list[str] = []
    if _version_tuple(version) < (1, 7, 2):
    ...
    if (
        "checkpoint m148 is implemented/released" not in active_text
        and "m148 is implemented/released" not in active_text
    ):
        return failures
    required_paths = [
    ...
    for rel_path in required_paths:
        if not (root / rel_path).exists():
            failures.append(f"missing M148 external security review doc: {rel_path}")
    text = " ".join(
        "\n".join(
            _read(root / rel_path).lower()
            for rel_path in required_paths
    ...
    required_fragments = {
        "M148 docs must say External Security Review": "external security review",
        "M148 docs must say contract-only": "contract-only",
        "M148 docs must say review-only": "review-only",
        "M148 docs must say deterministic": "deterministic",
        "M148 docs must say local-only": "local-only",
        "M148 docs must say safe-ref-only": "safe-ref-only",
        "M148 docs must say external-security-review-only": (
            "external-security-review-only"
        ),
        "M148 docs must say disabled by default": "disabled by default",
        "M148 docs must say route-free": "route-free",
        "M148 docs must say no-effect": "no-effect",
        "M148 docs must say accepted M101-M147": "accepted m101-m147",
        "M148 docs must say security review refs": "security review refs",
        "M148 docs must say threat model refs": "threat model refs",
        "M148 docs must say review scope refs": "review scope refs",
        "M148 docs must say evidence index refs": "evidence index refs",
        "M148 docs must say finding summary refs": "finding summary refs",
        "M148 docs must say disclosure review refs": "disclosure review refs",
        "M148 docs must say remediation plan refs": "remediation plan refs",
        "M148 docs must deny external vendor handoff": (
            "no external vendor handoff"
        ),
        "M148 docs must deny security vendor handoff": (
            "no security vendor handoff"
        ),
        "M148 docs must deny external review automation": (
            "no external review automation"
        ),
        "M148 docs must deny scanner runtime": "no scanner runtime",
        "M148 docs must deny vulnerability scan": "no vulnerability scan",
        "M148 docs must deny repository export": "no repository export",
        "M148 docs must deny artifact export": "no artifact export",
        "M148 docs must deny issue export": "no issue export",
        "M148 docs must deny security review runtime": (
            "no security review runtime"
        ),
        "M148 docs must deny auth runtime": "no auth runtime",
        "M148 docs must deny backend route": "no backend route",
        "M148 docs must deny Control Center control": "no control center control",
        "M148 docs must deny dependency": "no dependency",
