...
def _route_status_manifest_failures(root: Path) -> list[str]:
    ...
    if manifest.get("openapi_path_count") != 130:
        failures.append("route status manifest must record the 130-path OpenAPI boundary")
...
def _operator_shell_gap_map_failures(root: Path) -> list[str]:
    ...
    required_fragments = {
        ...
        "operator-shell gap map must include current API count": (
            "api boundary: current fastapi manifest has 130 openapi paths"
        ),
