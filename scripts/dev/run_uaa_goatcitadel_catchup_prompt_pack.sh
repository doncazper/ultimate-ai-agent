...
PROMPT="$ROOT/docs/prompts/uaa_goatcitadel_catchup/00_execute_uaa_goatcitadel_catchup_end_to_end.prompt.md"
VERIFY="$ROOT/scripts/verify_uaa_goatcitadel_catchup_prompt_pack.py"
OUTPUT="${UAA_GOATCITADEL_CATCHUP_OUTPUT:-/tmp/uaa-goatcitadel-catchup-prompt-pack.md}"
CODEX_BIN="${CODEX_BIN:-codex}"
SANDBOX="${UAA_GOATCITADEL_CATCHUP_SANDBOX:-workspace-write}"
DRY_RUN=0
LIST_ONLY=0
MODEL_ARG=()

if [[ -n "${UAA_GOATCITADEL_CATCHUP_MODEL:-}" ]]; then
  MODEL_ARG=(--model "$UAA_GOATCITADEL_CATCHUP_MODEL")
fi
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/dev/run_uaa_goatcitadel_catchup_prompt_pack.sh [options]
...
Environment:
  ...
  UAA_GOATCITADEL_CATCHUP_MODEL     Optional Codex model argument.
  UAA_GOATCITADEL_CATCHUP_SANDBOX   Codex sandbox. Default: workspace-write
  UAA_GOATCITADEL_CATCHUP_OUTPUT    Combined prompt output path.
USAGE
}
...
echo "UAA GoatCitadel catch-up prompt pack:"
echo "  repo: $ROOT"
echo "  prompt: $PROMPT"
