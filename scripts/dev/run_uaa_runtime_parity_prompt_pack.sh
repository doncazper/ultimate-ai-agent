...
PROMPT="$ROOT/docs/prompts/uaa_runtime_parity/00_execute_runtime_parity_end_to_end.prompt.md"
VERIFY="$ROOT/scripts/verify_uaa_runtime_parity_prompt_pack.py"
OUTPUT="${UAA_RUNTIME_PARITY_OUTPUT:-/tmp/uaa-runtime-parity-prompt-pack.md}"
CODEX_BIN="${CODEX_BIN:-codex}"
SANDBOX="${UAA_RUNTIME_PARITY_SANDBOX:-workspace-write}"
DRY_RUN=0
LIST_ONLY=0
MODEL_ARG=()

if [[ -n "${UAA_RUNTIME_PARITY_MODEL:-}" ]]; then
  MODEL_ARG=(--model "$UAA_RUNTIME_PARITY_MODEL")
fi
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/dev/run_uaa_runtime_parity_prompt_pack.sh [options]
...
Environment:
  ...
  UAA_RUNTIME_PARITY_MODEL   Optional Codex model argument.
  UAA_RUNTIME_PARITY_SANDBOX Codex sandbox. Default: workspace-write
  UAA_RUNTIME_PARITY_OUTPUT  Combined prompt output path.
USAGE
}
...
echo "UAA runtime parity prompt pack:"
echo "  repo: $ROOT"
echo "  prompt: $PROMPT"
