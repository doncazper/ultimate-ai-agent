...
PROMPT="$ROOT/docs/prompts/uaa_runtime_capability_foundation/00_execute_uaa_runtime_capability_foundation_end_to_end.prompt.md"
VERIFY="$ROOT/scripts/verify_uaa_runtime_capability_foundation_prompt_pack.py"
OUTPUT="${UAA_RUNTIME_CAPABILITY_FOUNDATION_OUTPUT:-/tmp/uaa-runtime-capability-foundation-prompt-pack.md}"
CODEX_BIN="${CODEX_BIN:-codex}"
SANDBOX="${UAA_RUNTIME_CAPABILITY_FOUNDATION_SANDBOX:-workspace-write}"
DRY_RUN=0
LIST_ONLY=0
MODEL_ARG=()

if [[ -n "${UAA_RUNTIME_CAPABILITY_FOUNDATION_MODEL:-}" ]]; then
  MODEL_ARG=(--model "$UAA_RUNTIME_CAPABILITY_FOUNDATION_MODEL")
fi
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/dev/run_uaa_runtime_capability_foundation_prompt_pack.sh [options]
...
Environment:
  ...
  UAA_RUNTIME_CAPABILITY_FOUNDATION_MODEL     Optional Codex model argument.
  UAA_RUNTIME_CAPABILITY_FOUNDATION_SANDBOX   Codex sandbox. Default: workspace-write
  UAA_RUNTIME_CAPABILITY_FOUNDATION_OUTPUT    Combined prompt output path.
USAGE
}
