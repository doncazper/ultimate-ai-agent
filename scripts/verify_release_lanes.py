    ...
    return (
        ...
        ReleaseLane(
            ...
            commands=(
                ...
                LaneCommand(
                    command_ref="command:route-module.ownership",
                    argv=(py, "-m", "pytest", "tests/test_route_module_ownership.py"),
                    env={"PYTHONPATH": "src"},
                    purpose="Validate route group, owner, service module, operation id, auth, release, and side-effect ownership coverage.",
                ),
            ),
            required_for_release_candidate=True,
            skipped_policy="Not skippable for a release candidate.",
        ...
        ReleaseLane(
            ...
            commands=(
                ...
                LaneCommand(
                    command_ref="command:security.artifact-redaction",
                    argv=(py, "scripts/verify_security_redaction_artifacts.py"),
                    purpose="Verify release-facing artifacts, reports, docs, and frontend build output remain redacted.",
                    report_ref="report:security-redaction:artifact-scan",
                ),
            ),
            required_for_release_candidate=True,
            skipped_policy="Not skippable for a release candidate.",
            blocked_policy="Blocked by raw prompt, raw response, provider payload, raw path, raw log, username, hostname, serial, environment dump, credential-like output, or unsafe release claim.",
