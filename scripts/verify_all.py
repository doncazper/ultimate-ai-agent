...
def verify_m32_filesystem_metadata_tool_safety():
    ...
    runtime_source_files = [
        ROOT / "src" / "ultimate_ai_agent" / "core" / "tools" / "runtime" / "filesystem_metadata.py",
        ROOT / "src" / "ultimate_ai_agent" / "core" / "tools" / "runtime" / "file_preview.py",
        ROOT / "src" / "ultimate_ai_agent" / "core" / "tools" / "runtime" / "invocation.py",
        ROOT / "src" / "ultimate_ai_agent" / "core" / "tools" / "runtime" / "policy.py",
    ]
    forbidden_source_fragments = [
        "read_text(",
        "read_bytes(",
    ...
    if manifest.allowlisted_tool_refs[: len(expected_m32_allowlist)] != expected_m32_allowlist:
        print("FAIL: M32/M33 manifest allowlist does not preserve the expected safe tool prefix")
        sys.exit(1)
    unsafe_flags = [
        policy.arbitrary_tool_execution_enabled,
        policy.side_effecting_tools_enabled,
...
def verify_m33_redacted_file_preview_tool_safety():
    ...
    expected_m33_allowlist = [NOOP_TOOL_REF, FILESYSTEM_METADATA_TOOL_REF, REDACTED_FILE_PREVIEW_TOOL_REF]
    if manifest.allowlisted_tool_refs[: len(expected_m33_allowlist)] != expected_m33_allowlist:
        print("FAIL: M33 manifest allowlist does not preserve no-op, metadata, and redacted preview")
        sys.exit(1)
    policy = manifest.policy
    unsafe_flags = [
        policy.arbitrary_tool_execution_enabled,
...
def verify_m72_read_only_http_fetch_tool():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M72 read-only HTTP fetch file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M72 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m72_openapi_route_failures
        from ultimate_ai_agent.core.tools.runtime import (
        ...
        print(f"FAIL: M72 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m72_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    request = ToolInvocationRequest(
        invocation_id="tool-runtime-invocation:verify-all-m72",
        ...
        replay_key="tool-runtime-replay:verify-all-m72",
        safe_summary="Allowlisted read-only HTTP fetch.",
        metadata={
            "request_ref": "http-fetch-request:verify-all-m72",
    ...
    if (
        ...
        print("FAIL: M72 read-only HTTP fetch granted unsafe authority or returned unsafe content")
        sys.exit(1)
    ...
    if no_transport.invocation_allowed or "HTTP_FETCH_TRANSPORT_REQUIRED" not in no_transport.reason_codes:
        print("FAIL: M72 HTTP fetch without explicit transport was not denied")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        if denied.invocation_allowed or reason not in denied.reason_codes:
            print(f"FAIL: M72 unsafe HTTP fetch request was not denied with {reason}")
            sys.exit(1)

    try:
        ReadOnlyHttpFetchPolicy(allowed_hosts=("*",))
        print("FAIL: M72 wildcard allowlist host was not denied")
        sys.exit(1)
    except ValueError as exc:
        if "WILDCARD_HOST_DENIED" not in str(exc):
            print(f"FAIL: M72 wildcard allowlist reason drifted: {exc}")
            sys.exit(1)
...
def verify_m75_browser_action_dry_run_planner():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M75 browser action dry-run file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M75 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        print(f"FAIL: M75 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m75_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    request = BrowserActionDryRunPlannerRequest(
        plan_ref="browser-action-plan:verify-all-m75",
        actor_ref="actor:verify-all-m75",
        target_ref="browser-target:verify-all-m75",
        source_observation_ref="browser-observe-output:verify-all-m74",
        safe_url_ref="browser-url:verify-all-m75",
        safe_summary="Plan a browser action without executing browser automation.",
        steps=[
            BrowserActionDryRunStep(
                step_ref="browser-action-step:verify-all-m75-click",
                action_kind=BrowserActionDryRunActionKind.click,
                safe_target_ref="browser-target:verify-all-m75-button",
                safe_intent="Dry-run plan to click a safe reviewed button.",
            )
        ],
    )
    ...
    if (
        ...
        print("FAIL: M75 browser action dry-run plan is unsafe or executing")
        sys.exit(1)

    for update, reason in [
        ({"browser_action_execution_requested": True}, "BROWSER_ACTION_EXECUTION_DENIED"),
        ({"browser_session_start_requested": True}, "BROWSER_SESSION_START_DENIED"),
        ({"navigation_execution_requested": True}, "BROWSER_NAVIGATION_EXECUTION_DENIED"),
...
def verify_m79_plugin_install_review():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M79 plugin install review file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M79 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m79_openapi_route_failures
        ...
        print(f"FAIL: M79 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m79_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    manifest_decision = build_plugin_manifest_security_decision(
        PluginManifestSecurityReviewRequest(
            review_request_ref="plugin-manifest-review-request:verify-all-m79",
            manifest_ref="plugin-manifest:verify-all-m79",
            plugin_ref="plugin:verify-all-m79",
            plugin_name="verify-all-install-review-disabled-plugin",
            plugin_version="1.0.0",
            actor_ref="actor:verify-all-m79",
            source_ref="plugin-source:verify-all-m79",
            provenance_ref="plugin-provenance:verify-all-m79",
            declared_permissions=[
                PluginManifestDeclaredPermission(
                    permission_ref="plugin-permission:verify-all-m79",
                    kind=PluginManifestPermissionKind.read_only_local_docs,
                    risk_level=PluginManifestRiskLevel.low,
                    safe_purpose="Review plugin install metadata only.",
                    tool_broker_capability_ref="tool-broker-capability:verify-all-m79",
                )
            ],
            static_review_ref="plugin-static-review:verify-all-m79",
            sandbox_test_plan_ref="plugin-sandbox-test-plan:verify-all-m79",
            tool_broker_mapping_ref="plugin-tool-broker-map:verify-all-m79",
            event_ledger_plan_ref="event-ledger-plan:verify-all-m79",
            version_pin_ref="plugin-version-pin:verify-all-m79-1.0.0",
            revocation_plan_ref="plugin-revocation-plan:verify-all-m79",
            human_approval=PluginManifestApprovalBinding(
                approval_ref="approval:verify-all-m79-manifest",
                approved_manifest_ref="plugin-manifest:verify-all-m79",
                approved_plugin_ref="plugin:verify-all-m79",
                approved_version="1.0.0",
                approved_actor_ref="actor:verify-all-m79",
            ),
            safe_manifest_summary="Reviewed disabled plugin manifest metadata.",
        )
    )
    request = PluginInstallReviewRequest(
        install_review_request_ref="plugin-install-review-request:verify-all-m79",
        ...
        source_package_ref="plugin-package:verify-all-m79-reviewed",
        provenance_ref="plugin-provenance:verify-all-m79",
        static_review_ref="plugin-static-review:verify-all-m79",
        sandbox_test_plan_ref="plugin-sandbox-test-plan:verify-all-m79",
        tool_broker_mapping_ref="plugin-tool-broker-map:verify-all-m79",
        event_ledger_plan_ref="event-ledger-plan:verify-all-m79-install-review",
        version_pin_ref="plugin-version-pin:verify-all-m79-1.0.0",
        revocation_plan_ref="plugin-revocation-plan:verify-all-m79",
        approval=PluginInstallReviewApprovalBinding(
            approval_ref="approval:verify-all-m79-install-review",
            approved_install_review_request_ref="plugin-install-review-request:verify-all-m79",
    ...
    if (
        decision.status != PluginInstallReviewDecisionStatus.install_review_ready_disabled
        ...
        or "M79_PLUGIN_INSTALL_REVIEW_DISABLED_BY_DEFAULT" not in decision.reason_codes
        or "M80_REMAINS_FUTURE" not in decision.reason_codes
    ):
        print("FAIL: M79 plugin install review decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            build_plugin_install_review_decision(request.model_copy(update=update))
            print(f"FAIL: M79 unsafe plugin install review request was not denied with {reason}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M79 unsafe plugin install review request raised {exc!s}, expected {reason}")
                sys.exit(1)
...
def verify_m80_network_browser_openwebui_hardening_freeze():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M80 hardening freeze file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M80 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m80_openapi_route_failures
        ...
        print(f"FAIL: M80 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m80_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    request = NetworkBrowserOpenWebUIFreezeRequest(
        request_ref="network-browser-openwebui-freeze-request:verify-all-m80",
        freeze_ref="network-browser-openwebui-freeze:verify-all-m80",
        baseline_ref="baseline:v0.83.0",
        actor_ref="actor:verify-all-m80",
    ...
    if (
        ...
        print("FAIL: M80 hardening freeze report is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M80 unsafe freeze request was not denied with {reason}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M80 unsafe freeze request raised {exc!s}, expected {reason}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M80 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m82_command_proposal_contracts():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M82 command proposal file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M82 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m82_openapi_route_failures
        from ultimate_ai_agent.core.sandbox import (
        ...
        print(f"FAIL: M82 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m82_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    request = CommandProposalRequest(
        request_ref="command-proposal-request:verify-all-m82",
        proposal_ref="command-proposal:verify-all-m82",
        sandbox_spec_ref="runtime-sandbox-spec:m81",
        baseline_ref="baseline:v0.85.0",
        actor_ref="actor:verify-all-m82",
        prior_milestone_refs=["milestone:M57", "milestone:M58", "milestone:M80", "milestone:M81"],
        command_ref="command-ref:verify-noop",
        safe_purpose="Verify-all no-effect command proposal contract.",
    ...
    if (
        ...
        print("FAIL: M82 command proposal decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M82 unsafe command proposal request was not denied with {reason}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M82 unsafe command proposal request raised {exc!s}, expected {reason}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M82 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m83_shell_dry_run_classifier():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M83 shell dry-run classifier file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M83 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m83_openapi_route_failures
        from ultimate_ai_agent.core.sandbox import (
            CommandProposalRequest,
        ...
        print(f"FAIL: M83 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m83_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    proposal = build_command_proposal(
        CommandProposalRequest(
            request_ref="command-proposal-request:verify-all-m83",
            proposal_ref="command-proposal:verify-all-m83",
            sandbox_spec_ref="runtime-sandbox-spec:m81",
            baseline_ref="baseline:v0.86.0",
            actor_ref="actor:verify-all-m83",
            prior_milestone_refs=["milestone:M57", "milestone:M58", "milestone:M80", "milestone:M81"],
            command_ref="command-ref:verify-noop",
            safe_purpose="Verify-all no-effect command proposal for shell dry-run classification.",
            safe_command_label="verify noop metadata",
            argv_preview=["verify-noop", "--dry-summary"],
        )
    )
    request = ShellDryRunClassifierRequest(
        request_ref="shell-dry-run-classifier-request:verify-all-m83",
        classifier_ref="shell-dry-run-classifier:verify-all-m83",
    ...
    if (
        ...
        or "M83_SHELL_DRY_RUN_CLASSIFIER_CONTRACT_ONLY" not in decision.reason_codes
        or "M83_NO_SHELL_EXECUTION" not in decision.reason_codes
        or "M84_REMAINS_FUTURE" not in decision.reason_codes
    ):
        print("FAIL: M83 shell dry-run classifier decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        ({"contains_secret": True}, "SECRET_LIKE_SHELL_DRY_RUN_CLASSIFIER_CONTENT_DENIED"),
    ]:
        try:
            build_shell_dry_run_classification(request.model_copy(update=update))
            print(f"FAIL: M83 unsafe shell dry-run classifier request was not denied with {reason}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M83 unsafe shell dry-run classifier raised {exc!s}, expected {reason}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M83 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m85_read_only_command_allowlist():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M85 read-only command allowlist file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M85 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m85_openapi_route_failures
        from ultimate_ai_agent.core.sandbox import (
            CommandProposalRequest,
        ...
        print(f"FAIL: M85 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m85_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    proposal = build_command_proposal(
        CommandProposalRequest(
            request_ref="command-proposal-request:verify-all-m85",
            proposal_ref="command-proposal:verify-all-m85",
            sandbox_spec_ref="runtime-sandbox-spec:m81",
            baseline_ref="baseline:v0.88.0",
            actor_ref="actor:verify-all-m85",
            prior_milestone_refs=["milestone:M57", "milestone:M58", "milestone:M80", "milestone:M81"],
            command_ref="command-ref:verify-noop",
            safe_purpose="Verify-all no-effect command proposal for read-only allowlist review.",
            safe_command_label="verify noop review",
            argv_preview=["verify-noop", "--dry-summary"],
        )
    )
    classification = build_shell_dry_run_classification(
        ShellDryRunClassifierRequest(
            request_ref="shell-dry-run-classifier-request:verify-all-m85",
            classifier_ref="shell-dry-run-classifier:verify-all-m85",
            command_proposal_ref=proposal.proposal_ref,
            sandbox_spec_ref=proposal.sandbox_spec_ref,
            baseline_ref="baseline:v0.88.0",
            actor_ref=proposal.actor_ref,
    ...
    sandboxed = build_sandboxed_echo_noop_command(
        SandboxedEchoNoOpCommandRequest(
            request_ref="sandboxed-echo-noop-command-request:verify-all-m85",
            sandboxed_command_ref="sandboxed-echo-noop-command:verify-all-m85",
            shell_dry_run_classifier_ref=classification.classifier_ref,
            shell_dry_run_decision_ref=classification.decision_ref,
            command_proposal_ref=classification.command_proposal_ref,
            sandbox_spec_ref=classification.sandbox_spec_ref,
            ...
            safe_echo_text="Verify-all M85 upstream safe no-op text.",
            shell_dry_run_classification=classification,
        )
    )
    request = ReadOnlyCommandAllowlistRequest(
        request_ref="read-only-command-allowlist-request:verify-all-m85",
        allowlist_ref="read-only-command-allowlist:verify-all-m85",
        ...
        entries=[
            ReadOnlyCommandAllowlistEntry(
                entry_ref="read-only-command-allowlist-entry:verify-all-m85",
                command_ref="command-ref:verify-noop",
                safe_command_label="verify noop review",
                safe_argument_profile_ref="safe-argument-profile:verify-all-m85",
                reviewed_by_actor_ref=sandboxed.actor_ref,
            )
        ],
        requested_command_ref="command-ref:verify-noop",
        safe_purpose="Verify-all read-only command allowlist review.",
    )
    ...
    if (
        ...
        print("FAIL: M85 read-only command allowlist decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M85 unsafe command allowlist request was not denied with {reason}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M85 unsafe command allowlist raised {exc!s}, expected {reason}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M85 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m90_shell_subprocess_hardening_freeze():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M90 shell/subprocess hardening freeze file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M90 docs missing fragment: {fragment}")
            sys.exit(1)

    try:
        sys.path.insert(0, str(ROOT))
        sys.path.insert(0, str(ROOT / "src"))
        from tests.test_m90_shell_subprocess_hardening_freeze import _request
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m90_openapi_route_failures
        from ultimate_ai_agent.core.sandbox import (
        ...
        print(f"FAIL: M90 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m90_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    decision = build_shell_subprocess_hardening_freeze(_request())
    if (
        decision.status != ShellSubprocessHardeningFreezeStatus.frozen_for_review
        or not decision.contract_only
        or not decision.review_only
        ...
        or decision.receipt_plan.store_raw_pid
        or decision.receipt_plan.store_raw_signal
        or "M90_SHELL_SUBPROCESS_HARDENING_FREEZE_REVIEW_ONLY" not in decision.reason_codes
        ...
        print("FAIL: M90 shell/subprocess hardening freeze decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    try:
        ...
        print("FAIL: M90 mutated shell execution authority flag was not denied")
        sys.exit(1)
    except ValueError as exc:
        if "SHELL_EXECUTION_DENIED" not in str(exc):
            print(f"FAIL: M90 mutated shell execution authority flag raised {exc!s}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M90 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m91_autonomous_tool_execution_contract():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M91 autonomous tool execution contract file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M91 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m91_openapi_route_failures
        from ultimate_ai_agent.core.tools import (
        ...
        print(f"FAIL: M91 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m91_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    if (
        ...
        or decision.receipt_plan.store_secret
        or decision.receipt_plan.execution_performed
        or decision.receipt_plan.tool_execution_performed
        or decision.receipt_plan.autonomous_execution_performed
        or decision.receipt_plan.session_start_performed
        ...
        print("FAIL: M91 autonomous tool execution contract decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M91 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M91 mutated authority flag raised {exc!s}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        "autonomous_tool_execution_requested=True",
        "autonomous_tool_execution_authorized=True",
        ...
        "store_secret=True",
        "contains_raw_tool_payload=True",
        "contains_raw_provider_payload=True",
        "contains_raw_prompt=True",
        "contains_secret=True",
    ]:
        if fragment in source_text:
            print(f"FAIL: M91 forbidden source fragment present: {fragment}")
            sys.exit(1)
    ...
    for forbidden_route in [
        ...
        if forbidden_route in source_text:
            print(f"FAIL: M91 forbidden backend/frontend route fragment present: {forbidden_route}")
            sys.exit(1)
...
def verify_m92_low_risk_tool_autonomy_single_session():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M92 low-risk tool autonomy file: {rel_path}")
            sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment not in docs_text:
            print(f"FAIL: M92 docs missing fragment: {fragment}")
            sys.exit(1)
    ...
    try:
        ...
        from ultimate_ai_agent.core.gate.evaluators import m92_openapi_route_failures
    except Exception as exc:
        print(f"FAIL: M92 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m92_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)

    decision = build_low_risk_tool_autonomy_single_session_decision(_request())
    if (
        decision.status != LowRiskToolAutonomySingleSessionStatus.single_session_ready_for_review
        or not decision.review_only
        ...
        or decision.receipt_plan.store_secret
        or decision.receipt_plan.execution_performed
        or "M92_LOW_RISK_TOOL_AUTONOMY_SINGLE_SESSION_REVIEW_ONLY" not in decision.reason_codes
        ...
        print("FAIL: M92 low-risk tool autonomy decision is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M92 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M92 mutated authority flag raised {exc!s}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M92 forbidden source fragment present: {fragment}")
            sys.exit(1)
    ...
    for forbidden_route in [
        ...
        if forbidden_route in source_text:
            print(f"FAIL: M92 forbidden backend/frontend route fragment present: {forbidden_route}")
            sys.exit(1)
...
def verify_post_m100_roadmap_reconciliation():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing post-M100 reconciliation file: {rel_path}")
            sys.exit(1)

    roadmap_text = (ROOT / "docs/roadmap/M101_M150_CAPABILITY_CHARTERS.md").read_text(
        encoding="utf-8"
    ).lower()
    expected_labels = [
    ...
    for fragment in stale_fragments:
        if fragment in roadmap_text:
            print(f"FAIL: Stale fast-version roadmap fragment remains active: {fragment}")
            sys.exit(1)
    ...
    for fragment in forbidden_fragments:
        if contains_affirmative_fragment(active_text, fragment) or contains_affirmative_fragment(
            roadmap_text,
            fragment,
        ):
...
def verify_m109_mobile_sensor_audit_ledger():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M109 sensor audit ledger file: {rel_path}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m109_openapi_route_failures
        from ultimate_ai_agent.core.mobile_companion import (
        ...
        print(f"FAIL: M109 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m109_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    record = build_mobile_sensor_audit_ledger_record(source_record=source_record)
    if (
        ...
        print("FAIL: M109 mobile sensor audit ledger record is unsafe or over-authoritative")
        sys.exit(1)

    for update, reason in [
        ({"sensor_access_performed": True}, "SENSOR_ACCESS_DENIED"),
        ({"sensor_access_enabled": True}, "SENSOR_ACCESS_DENIED"),
        ...
        try:
            validate_mobile_sensor_audit_ledger_record(record.model_copy(update=update))
            print(f"FAIL: M109 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M109 mutated authority flag raised {exc!s}")
                sys.exit(1)
    ...
    for fragment in [
        ...
        if fragment in source_text:
            print(f"FAIL: M109 forbidden source fragment present: {fragment}")
            sys.exit(1)
...
def verify_m111_production_threat_model():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M111 production threat model file: {rel_path}")
            sys.exit(1)
    ...
    try:
        ...
        sys.path.insert(0, str(ROOT / "src"))
        from ultimate_ai_agent.api.app import app
        from ultimate_ai_agent.core.gate.evaluators import m111_openapi_route_failures
        from ultimate_ai_agent.core.mobile_companion import (
            build_mobile_approval_renewal_ux_report,
            build_mobile_kill_switch_revocation_record,
            build_mobile_sensor_audit_ledger_record,
        ...
        print(f"FAIL: M111 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m111_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    if (
        ...
        print("FAIL: M111 production threat model record is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        ({"dependency_added": True}, "DEPENDENCY_DENIED"),
        ({"source_freeze_ref": "mobile-sensor-hardening-freeze:other"}, "M111_SOURCE_FREEZE_BINDING_MISMATCH"),
    ]:
        try:
            validate_production_threat_model_record(record.model_copy(update=update))
            print(f"FAIL: M111 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M111 mutated authority flag raised {exc!s}")
                sys.exit(1)
...
def verify_m113_secrets_boundary_credential_vault():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M113 secrets boundary file: {rel_path}")
            sys.exit(1)
    ...
    try:
        ...
        print(f"FAIL: M113 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m113_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    if (
        ...
        print("FAIL: M113 secrets boundary record is unsafe or over-authoritative")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M113 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M113 mutated authority flag raised {exc!s}")
                sys.exit(1)
...
def verify_m121_email_connector_contract_refresh():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M121 email connector file: {rel_path}")
            sys.exit(1)
    ...
    try:
        ...
        print(f"FAIL: M121 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m121_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    record = build_email_connector_contract_refresh_record(source_record=source_record)
    if (
        record.status
        ...
        print("FAIL: M121 email connector refresh record is unsafe")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M121 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M121 mutated authority flag raised {exc!s}")
                sys.exit(1)
...
def verify_m122_calendar_connector_contract_refresh():
    ...
    for rel_path in required_files:
        if not (ROOT / rel_path).exists():
            print(f"FAIL: Missing M122 calendar connector file: {rel_path}")
            sys.exit(1)
    ...
    try:
        ...
        print(f"FAIL: M122 guard imports could not load: {exc}")
        sys.exit(1)

    for failure in m122_openapi_route_failures(app.openapi().get("paths", {})):
        print(f"FAIL: {failure}")
        sys.exit(1)
    ...
    record = build_calendar_connector_contract_refresh_record(source_record=source_record)
    if (
        record.status
        ...
        print("FAIL: M122 calendar connector refresh record is unsafe")
        sys.exit(1)
    ...
    for update, reason in [
        ...
        try:
            ...
            print(f"FAIL: M122 mutated authority flag was not denied: {update}")
            sys.exit(1)
        except ValueError as exc:
            if reason not in str(exc):
                print(f"FAIL: M122 mutated authority flag raised {exc!s}")
                sys.exit(1)
