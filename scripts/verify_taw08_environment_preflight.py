...
def _verify_environment_census(
    *,
    ...
    def requires_preimport_authentication(name: str) -> bool:
        folded = name.casefold()
        return any(
            folded.endswith(suffix.casefold()) for suffix in importable_suffixes
        ) or ".so." in folded
    ...
    for path in site_packages.rglob("*"):
        ...
        if requires_preimport_authentication(name):
    ...
    for relative_ref, expected in authenticated_files.items():
        if not requires_preimport_authentication(Path(relative_ref).name):
            continue
