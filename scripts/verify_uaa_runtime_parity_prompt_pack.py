#!/usr/bin/env python3
"""Validate and emit the UAA runtime parity prompt pack."""
...
PROMPT_REF_PATTERN = re.compile(
    r"^docs/prompts/uaa_runtime_parity/"
...
REQUIRED_PHRASES = (
    ...
    "Do not copy external runtime code wholesale",
...
def verify_manifest(allow_placeholder_hash: bool = False) -> dict[str, Any]:
    ...
    if manifest.get("bundle_id") != "uaa-runtime-parity-001":
...
def emit_combined_prompt(refs: list[str], output: Path) -> None:
    ...
    manifest = _load_manifest()
    header = [
        "# UAA Runtime Parity Prompt Pack Combined Run",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.list or args.print_hash or args.json):
            print(
                "UAA runtime parity prompt pack verified: "
