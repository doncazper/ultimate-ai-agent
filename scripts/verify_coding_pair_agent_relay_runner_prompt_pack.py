#!/usr/bin/env python3
"""Validate and emit the Coding Pair Agent Relay Runner prompt pack."""
...
WRAPPER_PROMPT = (
    "docs/prompts/coding_pair_agent_relay_runner/"
    "00_execute_coding_pair_agent_relay_runner_end_to_end.prompt.md"
)
PROMPT_REF_PATTERN = re.compile(
    r"^docs/prompts/coding_pair_agent_relay_runner/"
    r"(0[0-7]_[a-z0-9_]+\.prompt\.md)$"
)
HASH_PREFIX = "sha256:"
...
FORBIDDEN_PHRASES = (
    ...
    "background autonomy is enabled",
    "provider sdk calls are allowed",
)
REQUIRED_PHRASES = (
    "Foreground Paired Agent Relay Runner",
    "coding_pair_agent_foreground_relay_runner",
    "Do not implement a generic agent bus",
    "Fail-Closed Rule",
    "agent output is untrusted proposal text",
...
def verify_manifest(allow_placeholder_hash: bool = False) -> dict[str, Any]:
    ...
    if manifest.get("bundle_id") != "coding-pair-agent-relay-runner-001":
    ...
    if len(refs) != 8:
        raise VerificationError(f"expected 8 prompt refs, found {len(refs)}")
...
def emit_combined_prompt(refs: list[str], output: Path) -> None:
    ...
    manifest = _load_manifest()
    header = [
        "# Coding Pair Agent Relay Runner Prompt Pack Combined Run",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.list or args.print_hash or args.json):
            print(
                "Coding Pair Agent Relay Runner prompt pack verified: "
