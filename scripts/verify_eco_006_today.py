...
PROJECTION_SOURCE = "src/ultimate_ai_agent/core/ecosystem/today.py"
REQUIRED_FILES = (
    PROJECTION_SOURCE,
...
def verify() -> list[str]:
    ...
    source_path = ROOT / PROJECTION_SOURCE
    ...
    try:
        ...
        statuses = payload["today"]["source_statuses"]
        if len(statuses) != 1 or statuses[0]["freshness"] != "missing":
            failures.append("ECO-006 missing-source posture was not preserved")
    except TodayProjectionError as exc:  # stable safe-ref-only error code
        failures.append(f"ECO-006 operational verification failed: {exc}")
