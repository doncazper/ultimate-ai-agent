...
EXPECTED_CAPABILITY_REFS = (
    ...
    "matrix.room_ai_policy.read",
    "matrix.room_ai_policy.write",
    "matrix.context.materialize",
    "matrix.proposal.read",
    "matrix.proposal.persist",
    "matrix.proposal.delete",
    "matrix.provider.invoke",
    "matrix.attachment.analyze",
...
def _verify_matrix(text: str, failures: list[str]) -> None:
    ...
    allowed_phase_postures = {
        ...
        "exact request-scoped local lane implemented",
        "exact request-scoped local lane implemented; no model call, hidden context, or Memory write",
        "exact request-scoped local lane implemented; proposal only, no execution path",
        "blocked missing exact authority and prohibited by current runtime invariant",
        "blocked missing exact composite authority",
    }
    ...
    _require(
        ...
        (
            ...
            "fresh exact LocalApprovalAuthority validation bound to the complete request fingerprint and target set immediately before start",
            "catalog `unsupported`",
