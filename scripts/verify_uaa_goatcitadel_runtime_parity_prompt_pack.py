#!/usr/bin/env python3
"""Validate and emit the UAA GoatCitadel runtime parity prompt pack."""
...
PACK_DIR = ROOT / "docs" / "prompts" / "uaa_goatcitadel_runtime_parity"
MANIFEST_PATH = PACK_DIR / "prompt_bundle_manifest.json"
README_PATH = PACK_DIR / "README.md"
WRAPPER_PROMPT = (
    "docs/prompts/uaa_goatcitadel_runtime_parity/"
    "00_execute_runtime_parity_end_to_end.prompt.md"
)
PROMPT_REF_PATTERN = re.compile(
    r"^docs/prompts/uaa_goatcitadel_runtime_parity/"
    r"(0[0-8]_[a-z0-9_]+\.prompt\.md)$"
)
HASH_PREFIX = "sha256:"
...
REQUIRED_PHRASES = (
    ...
    "Do not copy GoatCitadel code wholesale",
    "route-decision binding",
    "Turn -> Durable Run -> Approval",
...
def verify_manifest(allow_placeholder_hash: bool = False) -> dict[str, Any]:
    ...
    if manifest.get("bundle_id") != "uaa-goatcitadel-runtime-parity-001":
    ...
    if len(refs) != 9:
        raise VerificationError(f"expected 9 prompt refs, found {len(refs)}")
    ...
    readme_text = combined_text_parts[0]
    _validate_text_safety(README_PATH, readme_text)
...
def emit_combined_prompt(refs: list[str], output: Path) -> None:
    ...
    manifest = _load_manifest()
    header = [
        "# UAA GoatCitadel Runtime Parity Prompt Pack Combined Run",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not (args.list or args.print_hash or args.json):
            print(
                "UAA GoatCitadel runtime parity prompt pack verified: "
