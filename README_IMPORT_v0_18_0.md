...
v0.18.0 implements M14 by stabilizing the existing Web Control Center local backend connection.

M14 adds local-only API base URL validation and clear backend online, degraded, offline-safe, and mock fallback states. M15 Approval Queue + Receipt/Event Viewer UI remains future work.

v0.18.0 adds no backend API route, production Control Center authority, public execution API, runtime/model/provider call, remote dispatch, mobile/native app, sensor access, plugin enablement, Chrome authenticated profile control, Computer Use automation, iOS/macOS build workflow, analytics/auth/payment/SaaS SDK, production persistence, external API host, dependency, or external action.
