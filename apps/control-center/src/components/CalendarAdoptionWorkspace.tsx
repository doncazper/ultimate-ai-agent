...
export function CalendarAdoptionWorkspace() {
  ...
  const conflictedOccurrenceRefs = useMemo(() => {
    const refs = new Set<string>();
    for (const conflict of workspace?.conflict_items ?? []) {
      refs.add(conflict.first_occurrence_ref);
      refs.add(conflict.second_occurrence_ref);
    }
    return refs;
  }, [workspace?.conflict_items]);
  ...
  return <section className="page-section calendar-adoption" aria-labelledby="calendar-adoption-title">
    ...
    {!workspace ? <div className="panel">Loading your local Calendar…</div> : workspace.status === "projection_limited" ? <div className="panel warning"><strong>This Calendar period is too dense to display safely</strong><p>{workspace.next_safe_action}</p></div> : workspace.status === "recovery_required" || workspace.status === "setup_incomplete" ? <div className="panel warning"><strong>Ordinary Calendar changes are paused</strong><p>{workspace.next_safe_action}</p></div> : null}
    ...
    {workspace?.status === "ready" || workspace?.status === "projection_limited" ? <div className="calendar-adoption-layout">
      <section className="calendar-adoption-main">
        ...
        <div className="calendar-adoption-days">{grouped.map(([day, items]) => <section key={day}><header><strong>{day}</strong><span>{items.length}</span></header>{items.map((item) => {
          const conflicted = conflictedOccurrenceRefs.has(item.occurrence.occurrence_ref);
    ...
    {pendingRestore ? <div className="calendar-adoption-confirm" role="dialog" aria-modal="true" aria-labelledby="calendar-restore-review"><div className="panel warning"><p className="eyebrow">Exact restore approval</p><h3 id="calendar-restore-review">Review encrypted restore</h3><p>Restore {pendingRestore.preview.calendar_count} calendar{pendingRestore.preview.calendar_count === 1 ? "" : "s"} and {pendingRestore.preview.event_count} event{pendingRestore.preview.event_count === 1 ? "" : "s"}. Backup revision {pendingRestore.preview.backup_revision}; backup fingerprint {pendingRestore.backup.ciphertext_fingerprint_ref}; target state {pendingRestore.preview.current_state_ref}; target revision {pendingRestore.preview.expected_revision} → {pendingRestore.preview.resulting_revision}. {pendingRestore.preview.rollback_available ? "Undo will remain available." : pendingRestore.preview.impact_status === "empty_target" ? "The target is empty, so there is no earlier local state to undo." : "This will replace existing local Calendar state, and undo will not be available."}</p><div><button type="button" onClick={() => setPendingRestore(null)} disabled={busy}>Cancel</button><button type="button" onClick={() => void confirmRestore()} disabled={busy}>Confirm private restore</button></div></div></div> : null}
