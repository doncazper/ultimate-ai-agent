...
export function LegacyRenderSurface({ activePath, data }: { activePath: string; data: ControlCenterData }) {
  ...
  if (!definition) return <main className="legacy-route-unavailable" role="alert"><h1>Reference route unavailable</h1><p>No accepted reference surface matches this route. No fallback content or authority was inferred.</p><code>{activePath}</code></main>;
