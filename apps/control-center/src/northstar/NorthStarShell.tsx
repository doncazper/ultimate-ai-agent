  ...
  return (
    <div className={`ns-app ${sidecarOpen ? "sidecar-open" : ""}`}>
      ...
      <div className="ns-workspace">
        <GlobalPostureBar data={data} previewOnly={representationNonAuthoritative} />
        {representationNonAuthoritative ? (
          <div className="ns-preview-banner" role="status">
            <Icon name="info" size={15} />
            <span>
              <strong>{fixtureSurface ? "Fixture-only surface" : mixedSurface ? "Mixed representation" : "Preview data"}</strong> · {fixtureSurface ? "Synthetic render data only; no account or source content is connected." : mixedSurface ? "Backend reads are shown alongside explicitly labeled unavailable or fixture panels." : "Render-aligned fixture surface; compatible backend contract is deferred."}
            </span>
          </div>
        ) : null}
        <main className="ns-main">{children}</main>
        <ComposerRail activeSurface={activeSurface} backendReady={backendReady} onOpenSidecar={() => setSidecarOpen(true)} safeRefCount={safeRefCount} />
      </div>
  ...
  return (
    <footer className="ns-composer-rail">
      ...
      <span className="ns-safe-ref-count">
        <Icon name="shield-check" size={17} /> {safeRefCount} safe refs
      </span>
      ...
      <div className="ns-privacy" aria-label="Operator shell posture">
        <Icon name="shield-check" size={18} />
        <span>{backendReady ? "Local operator shell · External lanes remain exact and request-scoped" : "Non-authoritative preview · No external authority inferred"}</span>
      </div>
    </footer>
  );
}
...
function UaaSidecar({
  ...
  const authoritative = data.connection.state === "online" && !data.connection.usingMockData && data.routeStates["/today"]?.state === "backend_owned" && thread.backend_owned && thread.safe_refs_only;
