...
beforeEach(() => {
  ...
  mocked.loadDecisions.mockReset();
});
...
describe("useControlCenterData", () => {
  ...
  it("loads only the bounded Action Inbox contract for North Star Decisions", async () => {
    mocked.loadDecisions.mockResolvedValueOnce(backendData("decisions only"));
    const binding = {
    ...
    await waitFor(() => expect(result.current.status).toBe("ready"));
    expect(result.current.data?.connection.safeMessage).toBe("decisions only");
