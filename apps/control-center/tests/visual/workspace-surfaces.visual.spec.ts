...
const messengerStates = [
  "founder", "personal", "dm", "group", "threads", "search", "room-info",
  "invite", "room-settings", "sessions", "intelligence", "recovery", "dark",
  "calling", "setup",
] as const;
...
for (const state of messengerStates) {
  test(`messenger ${state} reference state renders`, async ({ page }) => {
    await page.goto(`/workspace/messenger?view=${state}`);
    await expect(page.locator(".ns-messenger-app")).toBeVisible();
    await expect(page.getByText("No message sent", { exact: true })).toBeVisible();
  });
}
