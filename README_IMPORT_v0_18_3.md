...
v0.18.3 is a docs/roadmap clarification patch only.

OpenWebUI is the preferred conversational web shell. CCC means Control Center Clients: CCC Web, CCC iOS, CCC Android, and CCC macOS. Python Agent Core remains the brain and authority layer. Open Design governs custom CCC surfaces and does not replace OpenWebUI.

M15 Approval Queue + Receipt/Event Viewer UI remains future work. Native CCC clients require dedicated future milestones and explicit plugin/tooling approval.

v0.18.3 adds no OpenWebUI integration, OpenWebUI deployment config, backend API route, frontend feature, native CCC implementation, Android app, iOS app, macOS app, native build workflow, mobile sensor access, OS permission integration, signing, keystore, provisioning, App Store workflow, Play Store workflow, dependency, plugin enablement, runtime execution, model/provider call, network call, remote execution, or production authority.
