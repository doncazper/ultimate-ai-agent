...
v0.16.0 adds backend Control Center contracts, read-only dashboard snapshots, preview-only action decisions, and read-only/preview-only API routes.

v0.16.0 adds no frontend app, TypeScript UI, React/Next/Vite/shadcn/Tailwind project, Browser/Chrome/Computer Use automation, iOS/macOS build workflow, runtime execution, model/provider call, network call, remote execution, mobile sensor access, plugin enablement, native build, dependency, production persistence, or production Control Center.
