...
EXPECTED_M63_OPENAPI_PATH_COUNT = 75
...
EXPECTED_M65_OPENAPI_PATH_COUNT = 75
...
EXPECTED_M66_OPENAPI_PATH_COUNT = 75
...
EXPECTED_M67_OPENAPI_PATH_COUNT = 75
...
EXPECTED_M68_OPENAPI_PATH_COUNT = 75
...
class FoundationGateEvaluator:
    ...
    def evaluate(self, criteria: Optional[List[FoundationGateCriterion]] = None) -> FoundationGateReport:
        ...
        evaluator_map: Dict[str, Callable[[FoundationGateCriterion], FoundationGateResult]] = {
            ...
            "m63_autonomy_policy_engine_contract_review": (
                self.check_m63_autonomy_policy_engine_contract_review
            ),
            "m63_autonomy_policy_engine_static_safety": (
                self.check_m63_autonomy_policy_engine_static_safety
            ),
            ...
            "m65_autonomy_audit_replay_viewer_contract_review": (
                self.check_m65_autonomy_audit_replay_viewer_contract_review
            ),
            "m65_autonomy_audit_replay_viewer_static_safety": (
                self.check_m65_autonomy_audit_replay_viewer_static_safety
            ),
            "m65_autonomy_audit_replay_viewer_route_boundary": (
                self.check_m65_autonomy_audit_replay_viewer_route_boundary
            ),
            "m65_roadmap_currentness": self.check_m65_roadmap_currentness,
            "m66_scoped_approval_bundles_contract_review": (
                self.check_m66_scoped_approval_bundles_contract_review
            ),
            "m66_scoped_approval_bundles_static_safety": (
                self.check_m66_scoped_approval_bundles_static_safety
            ),
            "m66_scoped_approval_bundles_route_boundary": (
                self.check_m66_scoped_approval_bundles_route_boundary
            ),
            "m66_roadmap_currentness": self.check_m66_roadmap_currentness,
            "m67_revocation_kill_switch_contract_review": (
                self.check_m67_revocation_kill_switch_contract_review
            ),
            "m67_revocation_kill_switch_static_safety": (
                self.check_m67_revocation_kill_switch_static_safety
            ),
            "m67_revocation_kill_switch_route_boundary": (
                self.check_m67_revocation_kill_switch_route_boundary
            ),
            "m67_roadmap_currentness": self.check_m67_roadmap_currentness,
            "m68_autonomy_risk_classifier_contract_review": (
                self.check_m68_autonomy_risk_classifier_contract_review
            ),
            "m68_autonomy_risk_classifier_static_safety": (
                self.check_m68_autonomy_risk_classifier_static_safety
            ),
    ...
    def check_m63_autonomy_policy_engine_contract_review(
        ...
        try:
            ...
            if (
                ...
                failures.append("M63 autonomy policy decision granted authority or side effects")
            for update, reason in [
                ...
                try:
                    ...
                    failures.append(f"M63 unsafe policy request mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M63 unsafe policy request reason drifted for {reason}: {exc}")
            for update, reason in [
                ({"policy_activation_enabled": True}, "AUTONOMY_POLICY_ACTIVATION_DENIED"),
                ({"session_start_enabled": True}, "AUTONOMY_SESSION_START_DENIED"),
                ...
                try:
                    ...
                    failures.append(f"M63 unsafe policy rule mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M63 unsafe policy rule reason drifted for {reason}: {exc}")
        except Exception as exc:
    ...
    def check_m63_autonomy_policy_engine_static_safety(
        ...
        for root in source_roots:
            ...
            for path in sorted(candidate_files):
                ...
                for fragment in forbidden_source_fragments:
                    if fragment in text:
                        failures.append(f"M63 forbidden autonomy policy fragment in {rel}: {fragment}")
        return self._result(criterion, failures, [])
    ...
    def check_m63_autonomy_policy_engine_route_boundary(
        ...
        try:
            ...
            failures.extend(m63_openapi_route_failures(app.openapi().get("paths", {})))
        except Exception as exc:
            failures.append(f"M63 OpenAPI route validation failed: {exc}")
        return self._result(criterion, failures, [])

    def check_m63_roadmap_currentness(self, criterion: FoundationGateCriterion) -> FoundationGateResult:
        required_docs = [
            "README.md",
            "VERSION.md",
            "docs/canonical/09_roadmap.md",
        ...
        if "v0.67.0" not in text or "m63" not in text or "autonomy policy engine v1" not in text:
            failures.append("active docs do not identify v0.67.0/M63 Autonomy Policy Engine v1")
        if "m63 is implemented/released" not in text and "v0.67.0 implements m63" not in text:
    ...
    def check_m65_autonomy_audit_replay_viewer_contract_review(
        ...
        try:
            ...
            if (
                ...
                failures.append("M65 autonomy audit replay viewer granted authority or side effects")
            if view.simulation_result_ref != simulation_result.simulation_result_ref:
                failures.append("M65 replay view does not bind exact simulation result ref")
            if view.replay_steps[0].step_ref != simulation_result.simulated_step_refs[0]:
                failures.append("M65 replay view does not bind exact simulated step refs")
            for update, reason in [
                ...
                try:
                    ...
                    failures.append(f"M65 unsafe replay view mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M65 unsafe replay view reason drifted for {reason}: {exc}")
            for update, reason in [
                ({"authority_granted": True}, "AUTONOMY_POLICY_AUTHORITY_DENIED"),
                ({"session_started": True}, "AUTONOMY_SESSION_START_DENIED"),
                ({"execution_performed": True}, "EXECUTION_DENIED"),
                ...
                try:
                    validate_autonomy_audit_replay_view(
                        view.model_copy(update={"simulation_result": simulation_result.model_copy(update=update)})
                    )
                    failures.append(f"M65 unsafe simulation-result mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M65 unsafe simulation-result reason drifted for {reason}: {exc}")
        except Exception as exc:
            failures.append(f"M65 autonomy audit replay viewer validation failed: {exc}")
    ...
    def check_m65_autonomy_audit_replay_viewer_static_safety(
        ...
        for root in source_roots:
            ...
            for path in sorted(candidate_files):
                ...
                for fragment in forbidden_source_fragments:
                    if fragment in text:
                        failures.append(f"M65 forbidden autonomy audit replay fragment in {rel}: {fragment}")
        return self._result(criterion, failures, [])
    ...
    def check_m65_autonomy_audit_replay_viewer_route_boundary(
        ...
        try:
            ...
            failures.extend(m65_openapi_route_failures(app.openapi().get("paths", {})))
        except Exception as exc:
            failures.append(f"M65 OpenAPI route validation failed: {exc}")
        return self._result(criterion, failures, [])

    def check_m65_roadmap_currentness(self, criterion: FoundationGateCriterion) -> FoundationGateResult:
        required_docs = [
            "README.md",
            "VERSION.md",
            "docs/canonical/09_roadmap.md",
        ...
        if "v0.69.0" not in text or "m65" not in text or "autonomy audit + replay viewer" not in text:
            failures.append("active docs do not identify v0.69.0/M65 Autonomy Audit + Replay Viewer")
        if "m65 is implemented/released" not in text and "v0.69.0 implements m65" not in text:
        ...
        for version_label, milestone, title in [
            ...
            if version_label.lower() not in text or milestone.lower() not in text or title.lower() not in text:
                failures.append(f"active docs missing planned M65-M100 row: {version_label} / {milestone} — {title}")
        forbidden_fragments = [
            "m66 is implemented",
        ...
        if self._active_version_tuple() >= (0, 70, 0):
            forbidden_fragments = [
                fragment
                for fragment in forbidden_fragments
                if fragment not in {"m66 is implemented", "scoped approval bundles are implemented"}
            ]
        for fragment in forbidden_fragments:
            if fragment in text:
                failures.append(f"M65 docs imply forbidden/future capability: {fragment}")
        return self._result(criterion, failures, required_docs)
    ...
    def check_m66_scoped_approval_bundles_contract_review(
        ...
        try:
            ...
            if (
                ...
                failures.append("M66 scoped approval bundle granted authority or side effects")
            for update, reason in [
                ({"approval_test_ref": "approval_test_:m66"}, "APPROVAL_TEST_REF_DENIED"),
                ({"approval_refs": ["approval:m66-gate-review", "approval:m66-gate-review"]}, "APPROVAL_BUNDLE_DUPLICATE_REF_DENIED"),
                ...
                try:
                    ...
                    failures.append(f"M66 unsafe approval bundle mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M66 unsafe approval bundle reason drifted for {reason}: {exc}")
            try:
                validate_scoped_approval_bundle(
    ...
    def check_m66_scoped_approval_bundles_static_safety(
        ...
        for root in source_roots:
            ...
            for path in sorted(candidate_files):
                ...
                for fragment in forbidden_source_fragments:
                    if fragment in text:
                        failures.append(f"M66 forbidden scoped approval bundle fragment in {rel}: {fragment}")
        return self._result(criterion, failures, [])
    ...
    def check_m66_scoped_approval_bundles_route_boundary(
        ...
        try:
            ...
            failures.extend(m66_openapi_route_failures(app.openapi().get("paths", {})))
        except Exception as exc:
            failures.append(f"M66 OpenAPI route validation failed: {exc}")
        return self._result(criterion, failures, [])

    def check_m66_roadmap_currentness(self, criterion: FoundationGateCriterion) -> FoundationGateResult:
        required_docs = [
            "README.md",
            "VERSION.md",
            "docs/canonical/09_roadmap.md",
        ...
        if "v0.70.0" not in text or "m66" not in text or "scoped approval bundles" not in text:
            failures.append("active docs do not identify v0.70.0/M66 Scoped Approval Bundles")
        if "m66 is implemented/released" not in text and "v0.70.0 implements m66" not in text:
        ...
        for version_label, milestone, title in [
            ...
            if version_label.lower() not in text or milestone.lower() not in text or title.lower() not in text:
                failures.append(f"active docs missing planned M66-M100 row: {version_label} / {milestone} — {title}")
        forbidden_fragments = [
            "m67 is implemented",
        ...
        if self._active_version_tuple() >= (0, 71, 0):
            forbidden_fragments = [
                fragment
                for fragment in forbidden_fragments
                if fragment not in {"m67 is implemented", "revocation + kill switch is implemented"}
            ]
        for fragment in forbidden_fragments:
            if fragment in text:
                failures.append(f"M66 docs imply forbidden/future capability: {fragment}")
        return self._result(criterion, failures, required_docs)
    ...
    def check_m67_revocation_kill_switch_contract_review(
        ...
        try:
            ...
            if (
                ...
                failures.append("M67 revocation kill switch record granted authority or side effects")
            for update, reason in [
                ...
                try:
                    ...
                    failures.append(f"M67 unsafe revocation kill switch mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M67 unsafe revocation kill switch reason drifted for {reason}: {exc}")
            try:
                validate_revocation_kill_switch_record(
                    record.model_copy(update={"bundle_ref": "scoped-approval-bundle:other"})
                )
                failures.append("M67 bundle binding drift was not denied")
            except ValueError as exc:
                if "REVOCATION_KILL_SWITCH_BUNDLE_BINDING_MISMATCH_DENIED" not in str(exc):
    ...
    def check_m67_revocation_kill_switch_static_safety(
        ...
        for root in source_roots:
            ...
            for path in sorted(candidate_files):
                ...
                for fragment in forbidden_source_fragments:
                    if fragment in text:
                        failures.append(f"M67 forbidden revocation kill switch fragment in {rel}: {fragment}")
        return self._result(criterion, failures, [])
    ...
    def check_m67_revocation_kill_switch_route_boundary(
        ...
        try:
            ...
            failures.extend(m67_openapi_route_failures(app.openapi().get("paths", {})))
        except Exception as exc:
            failures.append(f"M67 OpenAPI route validation failed: {exc}")
        return self._result(criterion, failures, [])

    def check_m67_roadmap_currentness(self, criterion: FoundationGateCriterion) -> FoundationGateResult:
        required_docs = [
            "README.md",
            "VERSION.md",
            "docs/canonical/09_roadmap.md",
        ...
        if "v0.71.0" not in text or "m67" not in text or "revocation + kill switch" not in text:
            failures.append("active docs do not identify v0.71.0/M67 Revocation + Kill Switch")
        if "m67 is implemented/released" not in text and "v0.71.0 implements m67" not in text:
        ...
        for version_label, milestone, title in [
            ...
            if version_label.lower() not in text or milestone.lower() not in text or title.lower() not in text:
                failures.append(f"active docs missing planned M67-M100 row: {version_label} / {milestone} — {title}")
        for fragment in (
            "production authority is implemented",
            "broad autonomy is implemented",
            "tool execution is implemented",
            ...
            if fragment in text:
                failures.append(f"M67 docs imply forbidden/future capability: {fragment}")
        return self._result(criterion, failures, required_docs)
    ...
    def check_m68_autonomy_risk_classifier_contract_review(
        ...
        try:
            ...
            if (
                ...
                failures.append("M68 risk classifier failed to derive highest risk without authority")
            for update, reason in [
                ({"derived_risk_class": AutonomyRiskClass.low}, "AUTONOMY_RISK_DOWNGRADE_DENIED"),
                ({"authority_granted": True}, "AUTONOMY_RISK_CLASSIFIER_AUTHORITY_DENIED"),
                ...
                try:
                    validate_autonomy_risk_classification_decision(decision.model_copy(update=update))
                    failures.append(f"M68 unsafe risk classifier mutation was not denied: {reason}")
                except ValueError as exc:
                    if reason not in str(exc):
                        failures.append(f"M68 unsafe risk classifier reason drifted for {reason}: {exc}")
        except Exception as exc:
    ...
    def check_m68_autonomy_risk_classifier_static_safety(
        ...
        for root in source_roots:
            ...
            for path in sorted(candidate_files):
                ...
                for fragment in forbidden_source_fragments:
                    if fragment in text:
                        failures.append(f"M68 forbidden autonomy risk classifier fragment in {rel}: {fragment}")
        return self._result(criterion, failures, [])
    ...
    def check_m68_autonomy_risk_classifier_route_boundary(
        ...
        try:
            ...
            failures.extend(m68_openapi_route_failures(app.openapi().get("paths", {})))
        except Exception as exc:
            failures.append(f"M68 OpenAPI route validation failed: {exc}")
        return self._result(criterion, failures, [])

    def check_m68_roadmap_currentness(self, criterion: FoundationGateCriterion) -> FoundationGateResult:
        required_docs = [
            "README.md",
            "VERSION.md",
            "docs/canonical/09_roadmap.md",
        ...
        if "v0.72.0" not in text or "m68" not in text or "autonomy risk classifier" not in text:
            failures.append("active docs do not identify v0.72.0/M68 Autonomy Risk Classifier")
        if "m68 is implemented/released" not in text and "v0.72.0 implements m68" not in text:
