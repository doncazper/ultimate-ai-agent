...
class MemoryFeedbackRequest(BaseModel):
    memory_record_ref: str | None = Field(default=None, min_length=1, max_length=220)
    target_ref: str | None = Field(default=None, min_length=1, max_length=220)
    target_kind: str | None = Field(default=None, min_length=1, max_length=120)
    feedback_kind: str = Field(..., min_length=1, max_length=120)
    ...
    def validate_request(self) -> "MemoryFeedbackRequest":
        if self.memory_record_ref is None:
            from ultimate_ai_agent.core.memory.diagnostics import (
                MEMORY_FEEDBACK_QUALITY_BLOCKED_STATE_REFS,
            )

            if self.target_ref is None or self.target_kind is None:
                raise ValueError(
                    "memory feedback requires memory_record_ref or target_ref"
                )
            self.target_ref = _safe_ref(self.target_ref, "target_ref")
            ...
            self.reason_refs = _safe_refs(self.reason_refs, "reason_refs")
            self.metadata_refs = _safe_refs(self.metadata_refs, "metadata_refs")
            ...
            if missing_quality_refs:
                raise ValueError(
                    "memory feedback request missing quality blocked state refs"
                )
            return self

        self.memory_record_ref = _safe_ref(self.memory_record_ref, "memory_record_ref")
