...
def validate_openwebui_runtime_bridge_policy(
    policy: OpenWebUIRuntimeBridgePolicy,
) -> OpenWebUIRuntimeBridgePolicy:
    _assert_safe_text(policy.runtime_bridge_policy_ref)
    _assert_metadata_refs_only(policy.docs_refs)
    _assert_metadata_refs_only(policy.metadata_refs)
    _assert_safe_metadata(policy.metadata)
    if policy.status not in {
        ...
        raise ValueError("M76 OpenWebUI runtime bridge policy must remain review-only")
    if not policy.safe_summary_only:
        raise ValueError("SAFE_SUMMARY_ONLY_REQUIRED")
...
def validate_openwebui_runtime_bridge_request(
    request: OpenWebUIRuntimeBridgeRequest,
) -> OpenWebUIRuntimeBridgeRequest:
    _assert_safe_text(request.bridge_request_ref)
    _assert_safe_text(request.session_ref)
    _assert_safe_text(request.safe_conversation_ref)
    _assert_safe_text(request.actor_ref)
    _assert_safe_text(request.safe_intent_summary)
    _assert_metadata_refs_only(request.metadata_refs)
    _assert_safe_metadata(request.metadata)
    _assert_allowed_content_mode(request.content_mode)
    if request.approval_ref is not None:
    ...
    if request.openwebui_runtime_call_requested:
        raise ValueError("OPENWEBUI_RUNTIME_CALL_DENIED")
    if request.openwebui_handoff_requested:
        raise ValueError("OPENWEBUI_HANDOFF_DENIED")
...
def validate_openwebui_runtime_bridge_receipt_plan(
    plan: OpenWebUIRuntimeBridgeReceiptPlan,
) -> OpenWebUIRuntimeBridgeReceiptPlan:
    _assert_safe_text(plan.receipt_plan_ref)
    _assert_safe_text(plan.bridge_request_ref)
...
def validate_openwebui_runtime_bridge_envelope(
    envelope: OpenWebUIRuntimeBridgeEnvelope,
) -> OpenWebUIRuntimeBridgeEnvelope:
    _assert_safe_text(envelope.bridge_envelope_ref)
    _assert_safe_text(envelope.bridge_request_ref)
    _assert_safe_text(envelope.session_ref)
    _assert_safe_text(envelope.safe_conversation_ref)
    _assert_safe_text(envelope.actor_ref)
    _assert_safe_text(envelope.safe_bridge_summary)
    _assert_metadata_refs_only(envelope.reason_codes)
    _assert_metadata_refs_only(envelope.side_effects_performed)
    _assert_metadata_refs_only(envelope.docs_refs)
    _assert_metadata_refs_only(envelope.metadata_refs)
    _assert_safe_metadata(envelope.metadata)
    _assert_allowed_content_mode(envelope.content_mode)
    validate_openwebui_runtime_bridge_receipt_plan(envelope.receipt_plan)
    flag_reasons = [
        ("raw_prompt_returned", "RAW_PROMPT_DENIED"),
        ("raw_provider_payload_returned", "RAW_PROVIDER_PAYLOAD_DENIED"),
        ("raw_content_returned", "RAW_CONTENT_DENIED"),
        ...
        ("approval_granted", "APPROVAL_REF_NOT_AUTHORITY"),
        ("handoff_executed", "OPENWEBUI_HANDOFF_DENIED"),
        ("production_authority_granted", "PRODUCTION_AUTHORITY_DENIED"),
    ]
    for field_name, reason in flag_reasons:
