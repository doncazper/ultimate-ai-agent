...
class ToolRuntimeAdapterDescriptor(BaseModel):
    ...
    mode: ToolRuntimeMode = ToolRuntimeMode.read_only_http_fetch_allowlisted
    tool_ref: str = NOOP_TOOL_REF
    tool_name: str = NOOP_TOOL_NAME
    safe_description: str = (
        "Deterministic no-op, filesystem metadata, redacted file preview, and allowlisted "
        "read-only HTTP fetch runtime adapter."
    )
    ...
    def validate_descriptor(self) -> Any:
        ...
        if self.tool_ref not in {
            ...
            raise ValueError("runtime adapter may describe only allowlisted safe tools")
        ...
        if self.tool_ref == READ_ONLY_HTTP_FETCH_TOOL_REF and self.tool_name != READ_ONLY_HTTP_FETCH_TOOL_NAME:
            raise ValueError("M72 runtime adapter HTTP fetch descriptor has mismatched tool name")
        return self
...
class ToolRuntimePolicy(BaseModel):
    ...
    read_only_http_fetch_tool_enabled: bool = True
    ...
    def validate_policy(self) -> Any:
        ...
        if not self.read_only_http_fetch_tool_enabled:
            raise ValueError("M72 allowlisted read-only HTTP fetch tool must be enabled")
        blocked_fields = [
...
class ToolRuntimeManifest(BaseModel):
    baseline_version: str = "0.76.0"
    contract_version: str = "m72-tool-runtime-read-only-http-fetch"
    ...
    capabilities: List[ToolRuntimeCapability] = Field(
        default_factory=lambda: [
            ...
            ToolRuntimeCapability.read_only_http_fetch_allowlisted,
            ...
            ToolRuntimeCapability.no_unrestricted_network,
            ToolRuntimeCapability.no_authenticated_network,
    ...
    def validate_manifest(self) -> Any:
        if self.allowlisted_tool_refs != [
            ...
            raise ValueError(
                "M72 manifest allowlist must contain only no-op, metadata, redacted preview, and read-only HTTP fetch tools"
            )
        return self
...
class ToolInvocationReceiptPlan(BaseModel):
    ...
    def validate_receipt(self) -> Any:
        ...
        if self.tool_ref not in {
            ...
            raise ValueError("M72 receipt plans may reference only allowlisted safe tools")
...
class ToolInvocationResult(BaseModel):
    ...
    output: Union[NoOpToolOutput, FilesystemMetadataOutput, RedactedFilePreviewOutput, ReadOnlyHttpFetchOutput]
    execution_performed: bool = True
    side_effects_performed: List[str] = Field(default_factory=list)
    raw_input_echoed: bool = False
    raw_content_stored: bool = False
    ...
    def validate_result(self) -> Any:
        ...
        expected_status_by_tool = {
            ...
            READ_ONLY_HTTP_FETCH_TOOL_REF: ToolInvocationStatus.http_fetch_completed,
        }
...
class ToolInvocationDecision(BaseModel):
    ...
    def validate_decision(self) -> Any:
        ...
        if self.tool_ref not in {
            ...
            raise ValueError("M72 decisions may reference only allowlisted safe tools")
        if self.execution_performed and self.status not in {
            ...
            ToolInvocationStatus.http_fetch_completed,
        }:
