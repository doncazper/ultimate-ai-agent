...
- M9 local loopback runtime support is dev-only, loopback-only, approval-gated, and must default to validation or simulated fallback. Tests and Foundation Gate must use fake transport and must not make real network/model calls.
