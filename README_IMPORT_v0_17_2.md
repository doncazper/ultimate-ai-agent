...
v0.17.2 hardens the local React/Vite/TypeScript Web Control Center verification path.

v0.17.2 adds frontend CI coverage, local browser smoke readiness documentation, a static browser smoke readiness verifier, stronger frontend safety checks, and Foundation Gate coverage for CI and browser-smoke readiness. Browser smoke remains manual, local-only, unauthenticated-profile-free, and non-authoritative.

v0.17.2 adds no production Control Center authority, public execution API, runtime/model/provider call, remote dispatch, mobile/native app, sensor access, plugin enablement, Chrome authenticated profile control, Computer Use automation, iOS/macOS build workflow, analytics/auth/payment/SaaS SDK, production persistence, external action, or new backend OpenAPI path.
