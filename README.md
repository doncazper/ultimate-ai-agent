...
- UAA-P1-091 v0.105.0 Governed Runtime Pilot Phase 07 is the active scoped internal runtime-authority lane: configured loopback local-model calls, one exact read-only status command, exact Action Inbox approved focused pytest and repo-verifier execution, and an approved-runtime-command staged orchestration step for the exact focused pytest lane are governed through RuntimeGateway receipts; browser automation, connector writes, plugin import, remote execution, arbitrary shell/subprocess work, frontend-check runtime execution outside a future exact lane, public beta, public release, production authority, and broad autonomy remain blocked.
