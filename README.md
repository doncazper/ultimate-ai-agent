...
FCC-V1-003 Founder Loop V1 Vertical Slice is complete for creating a
reviewable Action envelope from a Today item, producing exact
approval/edit/reject/defer receipts, updating the Evidence Timeline, and
inspecting the same state through `scripts/dev/uaa_founder_loop.py`. It does
not execute approved actions or grant connector, shell/subprocess,
provider/model, memory-write, public beta, or production authority.
FCC-V1-004 Chat Durable Receipt And Handoff is complete for durable safe Chat
turn receipts and reviewable Actions/Plans handoff receipts. It does not treat
...
Governed Cognitive Memory Spine Phase 2 implements a read-only L1 hot local
memory index at `GET /control-center/memory/l1-index` for reviewed recall-only
...
- reviewed recall-only memory records plus read-only L1/L2 derived indexes
...
- memory recall, L1 previews, or L2 projections as truth, approval, connector,
  CRM, action, or hidden context authority
...
Python Agent Core
        ...
        +-- reviewed recall-only memory records and read-only L1/L2 indexes
...
| Control Center status | [gap map](docs/control_center/OPERATOR_SHELL_GAP_MAP.md), [route status manifest](docs/control_center/route_status_manifest.json), [language rules](docs/control_center/PRODUCT_LANGUAGE_RULES.md) |
...
| Governed memory | [memory spine contract](docs/memory/GOVERNED_COGNITIVE_MEMORY_SPINE_V1.md), [memory spine roadmap](docs/memory/GOVERNED_COGNITIVE_MEMORY_SPINE_ROADMAP.md), [handoff](docs/codex/GOVERNED_COGNITIVE_MEMORY_SPINE_HANDOFF.md) |
