...
| What is usable now? | Exact route-surface proof for `/actions`, `/chat`, `/memory`, and `/evidence`; backend-owned partial inspection surfaces for Start Here, Today, Action Inbox, Proof, Evidence, Memory, Trust, Settings, and local CRM; a local CLI Knowledge Workbench for rights-gated ingest, lifecycle/removal, lexical retrieval, and selected cited context; a bounded Python Core for typed local links and exact approval-bound Task/Board/Calendar ChangeSets with rollback; status surfaces for Runtime and local model readiness; and blocked-state visibility for connector, send/write, and production authority. |
...
| Chat workspace and handoff | Content-free draft checkpoints, conversation archive/recovery, durable safe Chat turn receipts, and reviewable Actions/Plans handoff receipts. Draft bodies stay tab-local and model output is not authority. | [docs/control_center/Q33_FOUNDER_OPERATING_LOOP_ADOPTION.md](docs/control_center/Q33_FOUNDER_OPERATING_LOOP_ADOPTION.md), [docs/control_center/FCC_V1_004_CHAT_DURABLE_RECEIPT_HANDOFF.md](docs/control_center/FCC_V1_004_CHAT_DURABLE_RECEIPT_HANDOFF.md) |
...
| [docs/product/UAA_FINANCE_FIN001_ACTIVATION_RECORD.md](docs/product/UAA_FINANCE_FIN001_ACTIVATION_RECORD.md) | Active exact FIN-001 claim and bounded synthetic implementation evidence |
| [docs/product/UAA_FINANCE_FIN001_SYNTHETIC_KERNEL.md](docs/product/UAA_FINANCE_FIN001_SYNTHETIC_KERNEL.md) | Synthetic protected-book contracts, encrypted repository, authority gate, CLI, and exclusions |
